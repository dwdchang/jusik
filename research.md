...
| `indices/GlobalTile` | Server | 글로벌 지표 타일 하나 (§89) — 라벨(+국기 이모지·단위) / **값 큼직** / 전일 대비율 작게, 세로 4줄 왼쪽 정렬. 값 글자 크기는 **`--tile-value-size` 상속**으로 받는다(CSS Modules는 파일마다 해시가 달라 부모가 자식 클래스를 못 고르지만 CSS 변수는 상속된다). 값은 **포맷된 문자열**로 받는다(소수 0·2·4자리 + 상단은 `formatIndex`도 씀). 이어받은 항목은 **값 뒤** `*`(기준일 열이 없어져 옮긴 자리) |
...
| `indices/GlobalTileSection` | Server | 글로벌 지표 구획 (§88의 `GlobalTableSection` 4열 표를 §89에서 대체·삭제) — `<details open>`/`<summary>` 네이티브라 JS 0·서버 컴포넌트. **기본 펼침이지만 접기는 남겼다**(요청은 "펼친 상태"였지 "접기 제거"가 아니고 비용이 0). 값 자리 수는 행의 `decimals`(`formatFixed`). **기준일 열 제거** — 기준일이 알려주던 것(미국·유럽은 항상 전일 종가)과 단위 축약으로 잘라낸 거래소·기준 설명은 전부 **구획 각주**로 옮겼다 |
...
QStash 스케줄 4개 (평일 09:00~15:30 10분 간격 / 15:40 / 18:15 KST)
  ...
  → refreshMarketData(trigger)  [lib/jobs/refreshMarketData.ts]
      ...
      1a''. refreshGlobalTable: **하루 3회차만**(isGlobalTableRound — 09:00·15:40·18:15 시작
           9분 창) 카탈로그 5구획 27종 순차 조회 → market:globalTable (§88·§89).
...
- **지표 상세**: `IndexDetailScreen` → `getIndexDetail`/`getOverseasDetail` → detail 1건.
  ...
  글로벌 지표(`/indices/market`, §88·§89)는 detail 2키(us10y·btcUsd) MGET 1회 +
  `market:globalTable` 1회를 병렬로 읽는다 — 27종은 항목별 키 없이 이 스냅샷 하나로 끝나고,
...
| `market:globalTable` | `StoredGlobalTable` (구획 5개 × 행: label·unit·**flag?**·decimals·close·changeRate·direction·basDt(+`staleAt?`) + fetchedAt) — 차트·일별 없음. basDt는 화면에서 뺐지만(§89) 이어받기 판정에 남는다 (§88·§89) | ✕ | 시세 잡(하루 3회차) | 글로벌 지표 화면 |
...
- **회차 안에서 다시 갈라지는 갱신도 있다** (§88·§89) — 글로벌 지표 27종은
