...
| `dividends/rounds.ts` | **배당 회차 병합** (Phase 83·94) — 예탁원(KIS `market:stockInfo`의 `rounds`)을 본 소스로 두고 **예탁원에 없는 값만** 배당결정 공시(`market:dividendDecision`)로 메운다. 예탁원이 이사회 결의를 며칠 늦게 반영해 확정 배당이 화면에서 통째로 빠지던 문제를 푼다(실측 2026-07-30 삼성전자 26.2Q: DART 374원 vs 예탁원 `per_sto_divi_amt=0`). 보완 단위는 **회차가 아니라 필드**(Phase 94) — 같은 기준일이면 예탁원 회차를 살리되 **비어 있는 지급일만** 공시 값으로 채우고 `source: "mixed"`로 표시한다(금액·종류는 정정 경로인 예탁원 우선). 회차 단위로 우선순위를 매기던 Phase 83 규칙은 예탁원이 **금액만 채우고 지급일은 공란**으로 준 중간 상태(실측 2026-07-31 같은 회차)에서 공시가 이미 알던 지급일을 버려, 지급일로 거르는 홈 배당 카드에서 회차가 나왔다 사라지게 했다. 보완 대상이 지급일뿐인 것은 나머지에 구멍이 없기 때문이다(기준일=대조 키, 0원 금액은 예탁원 회차 자체가 미생성, 종류는 항상 존재). 공시끼리 겹치면 접수번호가 큰 정정본. 주당배당금·기준일이 없는 건(현물배당만·파싱 실패)은 버린다. 순수 함수 `mergeDividendRounds` + 리더 `getDividendRoundsMap`(두 MGET 병렬, 공시 읽기 실패는 격리). **한계**: 배당결정 공시는 보통주 법인 명의라 우선주(DART 고유번호 없음)엔 이 보완이 안 걸린다 |
...
| `indices/GlobalTileSection` | Server | 글로벌 지표 구획 (§88의 `GlobalTableSection` 4열 표를 §89에서 대체·삭제) — `<details open>`/`<summary>` 네이티브라 JS 0·서버 컴포넌트. **기본 펼침이지만 접기는 남겼다**(요청은 "펼친 상태"였지 "접기 제거"가 아니고 비용이 0). 값 자리 수는 행의 `decimals`(`formatFixed`). **기준일 열 제거** — 기준일이 알려주던 것(미국·유럽은 항상 전일 종가)과 단위 축약으로 잘라낸 거래소·기준 설명은 전부 **구획 각주**로 옮겼다 — 그 각주는 §91에서 `NoteDisclosure` 접힘 토글이 되어 `<details>`가 중첩된다(부모 `<summary>` 바깥이라 클릭 영역은 안 겹친다). **§96.2 — 제목 옆 「N종」 배지 제거**(타일이 눈앞에 다 보여 세어 줄 이유가 없다), 제목은 **13px regular·secondary**로 물러났다(같은 화면 타일 라벨은 13px regular·primary — 크기가 같아 **색으로 앞뒤를 가른다**) |
...
| `feeds/EarningsFocusPanel` | Server | **실적 탭 상단 블록** (Phase 82) — 최신 분기 요약 3칸(매출액·영업이익·순이익 × 전분기·전년동기 대비) + 차트 + 최근 9분기 표 + IR 일정 카드 + **「실적 발표 안내」·「실적 관련 보도」**(Phase 84 — 보도는 제목(원문 링크)+발췌 4줄 클램프+발행일, **본문은 담지 않고 출처 각주를 단다**. 발췌는 §97에서 `tidyNewsSummary`로 마지막 온전한 문장까지만 남긴다). **각주 2곳(재무 기준·뉴스 출처)은 §98에서 `NoteDisclosure` 접힘 토글로** — `.caption` 클래스는 사용처가 0이 되어 삭제했다. 확정 재무 첫 조회가 느려 **본문만 `<Suspense key={code}>`** (Phase 78 패턴) — 선택 줄과 아래 게시판은 기다리지 않는다. **Phase 83에서 종목 선택 칩이 여기서 빠져 `EarningsStockPicker`로 갔다** (props도 `options` → `hasOptions` 축소) |
...
| `ui/NoteDisclosure` | Server | **각주 「설명」 접힘 토글 공용 컴포넌트** (Phase 91) — `label`(기본 "설명")·`className`·`children`. 네이티브 `<details>`라 클라이언트 번들 0(§87·§89 관례)이고 키보드·스크린리더는 브라우저가 준다. 설명형 각주 **12지점**이 전부 이것을 쓴다 — 지표 상세 10지점(글로벌 지표 주요 지표·구획 4개·갱신 주기 푸터 / 지수 상세 3종 공용 푸터 / 달러 인덱스 / 코스피 변동성 / 수출입 상세) + **§98에서 실적 탭 2지점**(`EarningsFocusPanel`의 재무 기준·뉴스 출처 — §91이 지표 화면만 훑어 빠져 있었다. 이쪽은 부모가 `flex`+`gap`이라 여백 `className`을 넘기지 않는다). 폰트·색·펼침 회전은 컴포넌트가 쥐고 **여백만 `className`으로 받는다**(카드 안 각주와 푸터 첫 줄의 간격이 달라서). `GlobalTileSection`처럼 이미 `<details open>`인 카드 안에 중첩되지만 부모 `<summary>` 바깥이라 클릭 영역이 겹치지 않는다. §6.4 참고 |
...
- `src/styles/tokens.css` — 디자인 토큰(색·타이포·간격·radius). `html[data-theme="dark"]`
  ...
  `--text-page-title` 20(§36 페이지 h1 전용) / **`--text-body` 16** / `--text-caption` 13 /
  ...
  그대로 명시했으므로 동작은 불변이다(같은 함정이 또 있는지 볼 때의 단서). §96.2에서
  두 곳이 `--text-caption`으로 옮겨 가 지금 사용처는 `FeedTabsClient`뿐이다.
  굵기: `--weight-regular` 400(§36 페이지 h1 · §96.2 구획 제목·타일 라벨) /
  medium 500 / semibold 600 / bold 700.
  **섹션 제목 관례**: `--text-caption` + `--weight-semibold` + `--color-text-secondary`가
  6곳 이상에 복붙돼 있다(지수 상세·종목 상세·배당·보유·달러 인덱스·변동성). 글로벌 지표
  화면만 §96.2에서 **굵기를 regular로** 낮췄다(그 화면의 주인공이 타일 값이라 제목을
  뒤로 물렸다) — 나머지는 semibold 그대로다.
...
18. **뉴스는 소비처마다 수집 규칙이 다르다** (Phase 84) — `market:news:{code}`(뉴스 탭)와
    ...
    **필수로 걸지 않은 건** "삼전 2분기 영업익"·"하이닉스 2분기 영업이익" 같은 축약형
    ...
    ⚠️ **네이버 `description`은 요약문이 아니라 검색어 주변 스니펫**이라 문장 중간에서
    시작·종료한다. 본문을 수집하지 않는 이상(저작권 선) **요약을 만들 재료가 없으므로**,
    §97은 요약 대신 `tidyNewsSummary`로 **끝의 불완전한 조각만** 버린다(앞쪽은 한국어에
    문장 시작 판정 단서가 없어 손대지 않는다). 적용은 **저장이 아니라 읽기 시점**
    (`visibleEarningsNews`)이라 기존 스냅샷에도 즉시 걸리고 선별 점수는 원문으로 매겨진다.
    실데이터 말줄임표는 `…`가 아니라 **마침표 3개**라 `썼다....`가 흔하다 — 먼저 걷어내지
    않으면 마지막 점이 문장 종결로 잡혀 아무것도 안 잘린다.
