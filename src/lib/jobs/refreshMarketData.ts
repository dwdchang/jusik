...
export interface RefreshMarketDataReport {
  ...
  /**
   * 글로벌 지표 28종(§88·§89·§90) 저장 결과 — 하루 3회차만 갱신하므로 그 밖의 회차는 skipped.
...
/**
 * 글로벌 지표 28종 → market:globalTable 저장 (§88 신설 · §89에서 32종 → 27종 · §90에서 28종).
 * 하루 3회차(09:00·15:40·18:15 KST)만 갱신한다 — 해외 지수·상품은 전일 종가가 하루 한 번
 * 바뀔 뿐이어서 10분 주기로 23콜을 태우면 +966콜/일이 된다. 그 밖의 회차는 그대로 통과.
...
export async function refreshMarketData(
  ...
  // 1a''. 글로벌 지표 28종(§88·§89·§90) → market:globalTable — 하루 3회차만, 부수 데이터·실패 격리.
  // 위 dxy의 통화쌍 응답과 oil·gold 스냅샷을 재사용하므로 신규 호출은 23콜이다.
