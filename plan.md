...
- **상태**: **구현 완료(2026-07-24)**. tsc·eslint·build 통과. **실 DART 응답 필드(sj_div·account_nm·idx_val 등) 실측 검증 완료(2026-07-25, 사용자 확인)** — 배포 후 실종목 열람에서 재무제표 계정×5개년·재무지표 4분류가 정상 표시됨. 표준 필드명 기준 파싱이 실응답과 일치.
...
### Phase 66 — 토요일 기준가 승격 잡 (금요일 등록분 공백 단축) (2026-07-25, **폐기**)
...
- **선결 실측 결과 → 전제 붕괴**: "영업일+1"이 **달력 다음날이 아니라 다음 영업일**로 확정됐다.
  - **공식 문서**(공공데이터포털 15094808): "데이터 갱신은 기준일자로부터 **영업일 하루 뒤** 오후 1시 이후에 업데이트됩니다. 예를 들어, **금요일 데이터는 차주 월요일에 제공됩니다**"(월요일이 공휴일이면 다음 영업일). 우리 사례가 문서에 그대로 예시로 적혀 있다.
  - **실측 뒷받침**: 2026-07-25(토) 10:54~13:30 5분 간격 폴링 내내 `resultCode=00`·`totalCount=8`로 최신이 `20260723` 고정 — 13시가 지나도 금요일(`20260724`) 종가 미공개.
- **폐기 사유**: 금요일 종가는 **월요일 13:00 이후**에야 나오는데 KIS 갱신 잡이 **월요일 09:00**에 이미 실시간 시세를 채운다. 4시간 늦게 도착하는 데이터라 쓸 곳이 없다. 주말에 가져올 데이터 자체가 없으므로 토요일·일요일 어느 시각에 잡을 돌려도 무의미하며 **대안도 없다**.
- **남는 것**: Phase 65 폴백만으로 「등록 즉시 확인」이라는 원래 목적은 달성된다(금요일 밤 등록분도 등록 시점 종가를 즉시 표시). 금요일 등록분의 실시간 시세 공백이 월요일 09:00까지인 것은 **KIS 시간창의 구조적 제약**이라 이 경로로는 줄일 수 없다.
- **AGENTS.md §3 영향 없음**: 잡은 **6종 그대로 유지**(7종 개정 불필요 — 사용자 승인은 받았으나 폐기로 실행하지 않음).
...
### Phase 69 — 코스피/코스닥 상세 상단 「거래대금 · 수급」 요약 블록 (2026-07-28, 구현 완료)

- **요청 근거**: 사용자 — 코스피/코스닥 요약에 **거래대금** 정보를 넣고, **개인·외국인·기관계 3열**로 "얼마(전일 대비 증감액)"를 표시할 수 있는지 확인 후 구현.
- **조사 실측 (2026-07-28 15:00 KST 라이브) — 투자자별 *거래대금*은 KIS로 확보 불가**:
  - `FHPTJ04040000`(시장별 투자자매매동향 일별, 현재 사용 중): **39필드 전부 `_ntby_`(순매수) 계열**. 매수대금(`_shnu_tr_pbmn`)·매도대금(`_seln_tr_pbmn`) 필드가 스키마에 **아예 없다**.
  - `FHPTJ04030000`(시장별 투자자매매동향 **시세**, `inquire-investor-time-by-market`): 응답 **72필드에 `prsn_shnu_tr_pbmn`·`prsn_seln_tr_pbmn` 등 매수/매도 대금이 존재**하지만 **값이 전부 0**. `FID_INPUT_ISCD`(0001·1001·2001·0000) × `FID_INPUT_ISCD_1`(KSP·KSQ·KTS·공란) × 부가 파라미터(무·`MRKT_DIV=U`·`=V`·`INPUT_DATE_1`·`SCR_DIV_CODE`·`DIV_CLS_CODE`) **96개 조합 전수 시도 결과 비영 값 0건**(장중 조회). Phase 42의 `ISCD_2` 함정과 달리 **어떤 조합으로도 실데이터가 나오지 않는다**.
  - `FHKST01010900`(**종목별** 투자자매매동향): 매수/매도 대금 **정상 수신**(개인·외국인·기관 3분류, 당일 행은 빈값 → 전일까지 확정치). 단 **종목 단위**라 시장 전체를 만들려면 전 종목 수천 콜이 필요 — 배제.
  - **시장 전체 거래대금(투자자 구분 없음)은 이미 확보돼 있다** — `FHPUP02120000` output1의 `acml_tr_pbmn`(당일 누적)·`prdy_tr_pbmn`(전일). 저장된 `dailyRows[].tradingValue`가 같은 값이라 **추가 조회 없이** 전일 대비 산출 가능.
- **정책(사용자 확정)**: 거래대금 한 줄(시장 전체) + 그 아래 개인·외국인·기관계 3열을 **순매수 금액** 기준으로 표시하고, 3열이 거래대금이 아님을 화면 주석으로 명시한다. 위치는 **지수 상세 현황 카드 아래**(홈 요약 카드는 폭이 좁아 제외).
- **구현 (추가 KIS 콜 0 · Redis 스키마 무변경 · 잡 무변경 — 순수 UI)**:
  - `components/indices/MarketFlowSummary.tsx`(+`.module.css`, 신규) — **서버 컴포넌트**. 거래대금은 `dailyRows[0].tradingValue − dailyRows[1].tradingValue`, 3열은 `investorRows[0] − investorRows[1]`의 `individual`·`foreign`·`institution`. 데이터가 둘 다 없으면 `null` 반환(초기 시딩 전 빈 카드 방지) → 자체 `margin-bottom`을 갖고 화면 쪽 빈 래퍼를 두지 않는다.
  - 3열 표기는 `formatEokwon(백만원 ÷ 100, true)`(억원 반올림) — 480px 레이아웃에서 셀 폭이 약 133px뿐이라 만원 단까지 적는 `formatEokFromMillion`("8,507억 5,400만원")은 넘친다. 거래대금 줄은 폭이 넉넉해 기존 `formatEokFromMillion` 유지(일별 시세 탭 거래대금 열과 동일 표기).
  - `components/indices/IndexDetailScreen.tsx` — `IndexCard` 섹션과 차트 섹션 사이에 KOSPI/KOSDAQ일 때만 렌더.
- **실측 검증(2026-07-28 스냅샷)**: 코스피 거래대금 28조 7,947억원(전일 대비 +4조 5,000억원) · 개인 +3조 8,148억(+1조 8,360억) · 외국인 −4조 6,494억(−1조 7,683억) · 기관계 +8,508억(−88억). 코스닥도 정상. `dailyRows[0].basDt`와 `investorRows[0].basDt`가 같은 거래일로 정렬됨을 확인.
- **한계(화면 주석·이 문서에 기록)**: 3열은 **순매수**이지 거래대금이 아니다. 장중에는 당일 누적 거래대금과 전일 종일 거래대금을 비교하게 되므로 전일 대비 값이 낮게 나온다.
- **상태**: **구현 완료(2026-07-28)**. tsc·eslint·build 통과.

### Phase 70 — 「거래대금 · 수급」 전일 **같은 시각** 대비 (장중 시각 슬롯 자체 축적) (2026-07-28, 구현 완료)

- **요청 근거**: 사용자 — §69의 "전일 대비"는 **당일 누적 vs 전일 종일** 비교라 아침에 보면 갭이 지나치게 크다. 시간별(1시간 등) **같은 시간대끼리** 비교가 가능한지 확인 후 구현.
- **조사 실측 (2026-07-28 라이브)**:
  - **당일 행은 조회 시점까지의 누적** — 같은 날 KOSPI 개인 순매수(백만원): 15:00 `3,652,442` → 15:20(잡 스냅샷) `3,903,606` → 15:27 `3,895,491`. 거래대금도 15:20 `30,411,778`로 계속 증가. 즉 지적대로 장 초반 비교는 기준이 어긋난다.
  - **과거 거래일의 시간대별 시장 수급을 주는 KIS API는 없다** — `FHPTJ04030000`(시세)은 §69대로 전 필드 0, `HHPTJ04160200`(외인기관 추정가집계)은 시간대 5구간을 주지만 **종목 단위 + 외국인·기관만(개인 없음) + 수량만(금액 없음) + 당일만**(실측: `bsop_hour_gb` 1~5, `frgn_fake_ntby_qty`/`orgn_fake_ntby_qty`/`sum_fake_ntby_qty`)이라 시장 요약에 못 쓴다.
  - **그러나 자체 축적이 가능하다** — 시세 잡이 이미 평일 09:00~15:30 **10분 간격**(+15:40·18:15)으로 당일 누적치를 받아온다(스냅샷 `fetchedAt`=15:20 확인). 회차마다 시각 슬롯으로 한 벌 더 적재하면 **다음 거래일부터 같은 시각 비교**가 성립한다.
- **정책(사용자 확정)**: **10분 슬롯**(잡 회차와 1:1 — 비교 오차 최대 10분, 1시간 슬롯은 장초반 오차가 남음) · baseline이 없는 첫 거래일은 **전일 종일 대비로 폴백**하고 라벨로 구분("전일 종일 대비" vs "전일 10:20 대비").
- **구현 (KIS 추가 콜 0 · 신규 잡/라우트 없음)**:
  - `types/indices.ts` — `IntradayFlowSlot`(hhmm·individual·foreign·institution·tradingValue?) · `IndexDetailData.intradayBaseline?`.
  - `lib/date/kst.ts` — `kstHhmm(ms)`("HHMM").
  - `lib/market/store.ts` — `market:investorIntraday:{kospi|kosdaq}`(`StoredIntradayFlows`: tradingDate·slots·fetchedAt) + `:baseline` 2종, get/set 4개.
  - `lib/jobs/refreshMarketData.ts` — `refreshInvestorFlows`가 이번 회차 최신 행을 함께 반환하도록 바꾸고, 새 스텝 `refreshIntradayFlowSlots`(1b''-2)가 **그 응답 + 방금 저장된 지수 스냅샷의 거래대금을 재사용**해 현재 슬롯을 upsert. 거래일이 바뀐 첫 회차에 직전 스냅샷을 `:baseline`으로 승격(Phase 68 시총 baseline과 동일 패턴). **당일 행이 아직 없는 회차(휴장일·장 시작 전)는 건너뛴다** — 전일 누계를 오늘 슬롯으로 오인하지 않기 위함. 부수 데이터·실패 격리, 리포트에 `intradayFlows` 추가.
  - `lib/indices/getIndexDetail.ts` — `Promise.all`에 `getIntradayBaseline(market)` 합류(슬롯이 0개면 생략 → 화면 폴백).
  - `components/indices/MarketFlowSummary.tsx` — `pickBaselineSlot`(현재 시각 이하의 마지막 슬롯, 없으면 가장 이른 슬롯) + 라벨 분기. 거래대금과 3열은 **각각** 기준을 정한다(슬롯에 `tradingValue`가 없던 회차면 거래대금만 종일 폴백). 헤더에 `07/28 15:20`처럼 스냅샷 시각 표기.
- **검증**: 42슬롯 시뮬레이션으로 분기 확인 — 09:10→09:10, 10:20→10:20, 12:00→12:00, 15:40→15:40, 18:15→18:15, 개장 전 08:50→가장 이른 09:00 슬롯. baseline 없음/슬롯에 거래대금 없음 폴백도 확인. tsc·eslint·build 통과.
- **비용**: KIS 콜 **0**(기존 응답 재사용) · Redis 키 2개/시장(하루 42슬롯 ≈ 수 KB) · 회차당 Redis 읽기 2·쓰기 2 추가.
- **한계**: **다음 거래일부터** 유효(배포 당일은 baseline이 없어 종일 대비 폴백). 장 마감 후에는 전일 마지막 슬롯과 비교하게 돼 자연히 종일 대비로 수렴한다.
- **상태**: **구현 완료(2026-07-28)**. 실데이터 확인은 배포 후 다음 거래일.

### Phase 71 — 변동성 카드를 「최근 거래일 값 + 전일 대비 / 월평균 · 전월 대비」 2줄로 (2026-07-28, 구현 완료)

- **요청 근거**: 사용자 — 홈 「코스피 변동성 지수」 카드의 요약이 **전월 대비**인데 **전일 대비**로 바꾸고 싶다. 두 줄(전일 대비 위 / 전월 대비 아래)이 나은지 추천 요청.
- **조사·판단**:
  - 기존 카드의 대표값은 `currentMonthAvg`(**당월 평균**)이라 전월 대비와 기준이 맞는 구조였다. **라벨만 "전일 대비"로 바꾸면 월 평균값에 일 단위 증감을 붙이는 꼴**이라 숫자가 어긋난다 → 대표값도 함께 내려야 한다고 회신.
  - 데이터는 이미 있다 — `kospiVolatility:history`가 일별 `{date, dailyGapPercent}` 전량 보유. **KIS 추가 콜 0**, 잡·스케줄 무변경.
  - **전일 대비의 함정(사용자에게 사전 고지)**: 일별 기록은 장중 10분 간격 회차마다 당일 값을 덮어쓰므로 **오늘 값은 고가·저가 폭이 계속 벌어지는 진행 값**이다. 09:10엔 전일 대비가 크게 마이너스였다가 마감에 가까워지며 좁혀져 **하루 안에 부호가 뒤집힌다**. 선택지 ① 확정된 직전 거래일끼리만 비교(정확하나 "전일"이 실제로는 어제 vs 그저께) ② **오늘 값 사용 + 장중 표시**.
- **정책(사용자 확정)**: 추천안(대표값 교체 + 2줄) 승인, 함정 처리는 **②(오늘 값 + 장중 표시)** 승인.
- **구현**:
  - `types/indices.ts` — `VolatilityCardSummary`를 일 지표 4(`latestGapPercent`·`latestDate`·`dayOverDayDiff`·`latestIntraday`) + 월 지표 2(`currentMonthAvg`·`monthOverMonthDiff`, **둘 다 nullable로 완화**)로 확장.
  - `lib/indices/volatility.ts` — `getVolatilityCardSummary`가 history 1건에서 `records.at(-1)/.at(-2)`로 전일 대비를 잡는다(**배열 직전 항목이라 휴장일·월 경계와 무관하게 직전 거래일**). `latestIntraday` = 최신 기록이 KST 오늘 && `kstHhmm(now) < "1530"`(정규장 마감). null 반환 조건을 "당월 기록 없음"에서 **"기록이 하나도 없음"**으로 바꿔, 달 초에도 카드가 placeholder로 떨어지지 않는다.
  - `components/indices/SummaryCard.tsx`+`.module.css` — `note?: string` 보조줄 추가(`--text-caption-sm`·tertiary, 색상 없음). §35에서 폐지한 `footnote`(각주)와 달리 **기준이 다른 부가 지표용**. `.change`의 `margin-bottom`을 12px→8px로(기존 카드는 전부 `:last-child`라 영향 없음).
  - `components/indices/IndexDashboard.tsx` — 인라인 JSX를 `volatilitySummaryProps` 헬퍼로 분리(`indexSummaryProps` 관례). 1행 `장중 · 전일 대비 +3.22%p`(장 마감 후엔 접두 없음), 2행 `월평균 6.34% · 전월 −0.12%p`(각 값이 null이면 그 조각만 빠지고, 둘 다 없으면 줄 자체가 없음).
- **표기 결정 근거**: 2행에 **월평균 값을 같이 적는다** — `전월 −0.12%p`만 두면 위의 대표값(최근 거래일 값)에 대한 전월 대비로 오독된다. 기준값을 한 번 밝혀야 두 줄이 각각 무엇의 증감인지 분명해진다.
- **검증**: tsc·eslint·build 통과. 라이브 Redis 실측(2026-07-28) — 기록 112건, 최근 `07/27 3.795%` → `07/28 7.019%`(전일 대비 **+3.22%p**), 당월 평균 6.34%(19일). 값이 큰 것은 계산 오류가 아니라 **당일 코스피가 −10.84%(6755.75→6023.66)인 급락장**이기 때문임을 스냅샷으로 확인.
- **비용**: KIS 콜 0 · Redis 읽기 증가 0(같은 키 1건 재사용).
- **상태**: **구현 완료(2026-07-28)**.

### Phase 72 — 종목분석 상세 개편: 통합지표·차트 6종 + 재무제표 전문 라우트 분리 (2026-07-28, 구현 완료)

- **요청 근거**: 사용자 — 종목분석 상세에 **통합지표**(주요 재무지표 표 · 투자지표 3열×5행 · 주가 변동률 막대 4열)와 **차트 6종**(실적 / 배당금·시가배당률 / 배당성향·배당금 / 분기배당금 / 현금흐름표 / 주당현금흐름, 각각 **연환산·연간·분기** 전환)을 넣고 싶다. 기존 재무제표 데이터는 양이 많으니 **「상세보기」 버튼**으로 옮겨달라.
- **가능성 조사(2026-07-28 실측, 코드 작성 전)** — 요청 항목별로 원천을 대조했다:
  - **DART 분기 재무제표 지원 확인** — `fnlttSinglAcntAll`에 `reprt_code` 11013/11012/11014를 넣으면 분기가 온다. 금액 기준이 재무제표별로 다르다: 손익 `thstrm_amount`=당분기 3개월·`thstrm_add_amount`=누적, **현금흐름은 `add_amount`가 없고 `thstrm_amount`가 이미 누적**, 재무상태표는 기말 잔액.
  - **배당(신규 `alotMatter`)** — 주당 현금배당금·현금배당성향·현금배당수익률을 **한 콜에 당기·전기·전전기 3개년**으로 준다(005930 2025: 1,668원 / 25.10% / 1.50% — 사용자 제시 표와 일치). 분기보고서로 부르면 **연초부터 누적**이라 차분하면 분기 배당이 나온다(1Q 365 → 반기 732 → 3Q 1,102 → 연 1,668).
  - **자사주(신규 `stockTotqySttus`)** — `istc_totqy`(발행주식총수)·`tesstk_co`(자기주식수)로 비중 계산. 주당 지표(BPS·주당현금흐름)의 분모로도 쓴다.
  - **한계 3종(사용자에게 사전 고지)**: ① 재무제표 API는 **2015 사업연도부터**(2013년 조회 시 status 013) → 요청한 15개년 불가 ② **재무지표 API(`fnlttSinglIndx`)는 2023년부터**(2017~2022 전부 013) → ROE를 옛 연도까지 채우려면 재무제표 원본으로 직접 계산해야 함 ③ **금융위 시세는 2020년부터**(2019년 이전 totalCount 0) → 과거 PER·PBR은 6개년까지만.
- **정책(사용자 확정)**: 표시 연수 **최근 6개년**(2015 제약·화면 폭·금융위 시세 시작연도가 모두 이 선에서 맞물린다) · 재무제표 전문은 **별도 라우트** · 화면 순서는 **투자지표 → 주가 변동률 → 차트 6종 → 주요 재무지표 표**(주요 재무지표만 차트 아래로 내리고 나머지는 요청 순서 유지).
- **시세 원천을 KIS가 아닌 금융위로 정한 이유**: 화면은 Redis 스냅샷만 읽는 게 원칙인데(§3) `market:stock:*`에는 **보유·관심 종목만** 있다. 종목분석은 임의 종목을 검색해 여는 화면이라 대부분 스냅샷이 없다. 금융위 주식시세정보는 호출 시간창이 없고 이미 종목 등록 액션이 같은 예외로 직접 호출 중(Phase 63)이라 경로를 맞췄다. **1콜로 1년치(262행) 확보**돼 52주 최고·최저, 1개월·3개월·1년·올해 변동률이 한 번에 나온다.
- **구현**:
  - `lib/api/dart/finance.ts` — 분기 보고서 코드 4종(`DART_QUARTER_REPRTS`)·`fetchDartFinancialStatements`에 `reprtCode` 인자(기본값 연간이라 기존 호출부 무변경)·`fetchDartDividendMatters`(alotMatter)·`fetchDartStockTotalQty`(stockTotqySttus) 추가.
  - `lib/api/fsc/stockPrice.ts` — 봉투 검증 공통부를 `fetchFscItems`로 추출(기존 `fetchStockCloseAsOf`가 재사용)하고 **`fetchStockDailySeries`**(기간 일별 시세, 오름차순, `hipr`/`lopr`/`lstgStCnt`/`mrktTotAmt` 포함) 신설.
  - `lib/analysis/overview.ts`(신규) — 통합지표 read-through(`analysis:overview:v1:{corpCode}`, **TTL 30일**). 연간 6개년(CFS+OFS)·분기 최근 3년(12분기)·배당·주식총수를 모아 **연간/분기/연환산 세 벌 시계열**로 환산. 핵심 규칙 2가지: ① 재무제표별 금액 기준이 달라 **모든 항목을 누적으로 모은 뒤 인접 분기 차분**(예외를 분기 계산에 끌고 들어가지 않음) ② 계정은 **표준 코드 우선·계정명 폴백**하되 `sj_div`로 범위를 좁힘.
  - `lib/analysis/quote.ts`(신규) — 시세 파생 read-through(`analysis:quote:v1:{code}`, **TTL 6시간** — 재무와 갱신 주기가 달라 키를 분리). 52주 최고·최저, 변동률 4종, **연말·분기말 종가**(PER·PBR·연환산 시가배당률 분자). 이미 받아둔 시계열로 해결되는 기간말은 재조회하지 않는다.
  - `lib/analysis/view.ts`(신규) — 재무+시세 결합 순수 함수. `withAnnualValuation`(연말 종가) / `withTtmValuation`(분기말 종가) / `withQuarterValuation`(배수 미산출) + 투자지표 15칸 `buildInvestmentIndicators`.
  - `lib/analysis/financials.ts` — 조회 연수 5→6, 캐시 키 `analysis:financials:**v2:**`(구 키는 자연 만료), `mapLimit`·`currentKstYear`·`DART_CONCURRENCY`를 통합지표와 공유하도록 export.
  - `components/analysis/` — `InvestmentIndicators`(Server, 3열 그리드 15칸) · `AnalysisCharts`(Client, 변동률 막대 + 6종 ComposedChart, 차트마다 독립 탭) · `AnalysisChartsClient`(dynamic·`ssr:false`) · `KeyMetricsTable`(Client, 11행 표 + 탭, **최신이 왼쪽**).
  - `app/analysis/[symbolCode]/page.tsx` 재작성 + `statements/page.tsx`·`page.module.css` 신설(Phase 64 화면을 그대로 이설, 뒤로가기는 통합지표로). `analysis/loading.tsx`가 하위 라우트까지 상속돼 prefetch 유지(§2.1).
- **지표 정의 결정(실측 대조로 확정)**:
  - **ROE 분모는 기말 지배주주 자본**. 기초·기말 평균이 교과서적이지만, 사용자 제시 표(국내 증권사 화면)와 대조하니 기말 기준이 **2021~2025 5개년 전부 일치**(13.25 / 15.86 / 4.10 / 8.58 / 10.43)했다. 연환산 ROE도 같은 규칙이라 탭을 바꿔도 값이 튀지 않는다.
  - **연환산 시가배당률은 각 분기말 종가로 계산**. 처음엔 현재가를 썼더니 전 구간이 0.57%로 눌려 연간 탭(1.8~4.0%)과 어긋났다 — 과거 지점에 현재 주가를 적용한 탓. 분기말 종가로 바꿔 1.39~2.72%로 정합해졌다.
  - **분기 탭은 PER·PBR을 매기지 않는다**. 분기 EPS는 3개월치라 연 단위 배수와 섞이면 4배 부풀려 읽힌다.
- **계정 매칭 함정(실측)**: ① **카카오(035720)는 영업수익을 `ifrs-full_GrossProfit` 코드로 태깅**해 표준 코드만 보면 2020~2022 매출이 통째로 빈다 → 이름 폴백 필요 ② 그렇다고 이름만 보면 **"지배기업 소유주지분"이 손익·포괄손익·재무상태표에 모두 있어** 총포괄손익을 순이익으로 착각한다 → `sj_div` 제한 필요 ③ **카카오·SK하이닉스는 손익계산서(IS)를 따로 내지 않고 포괄손익계산서(CIS)만 제출**한다 → 순이익 탐색 범위를 IS로 좁히면 안 된다. 세 조건을 동시에 만족시키려고 "코드 → 이름 → 예비 코드" 순서 + 구분 제한 구조를 썼다.
- **검증**: tsc·eslint·build 통과. **실 API 파이프라인 실측(2026-07-28)** — 005930(3.7~7.9초)·000660·035720(포괄손익만 제출)·247540(코스닥·적자 연도 포함) 4종 전 구간 정상. 005930 대조: 52주 67,200~374,500 / 변동률 −25.18%·13.14%·285.43% / 매출 3,336,059억 / DPS 1,668원 / 배당성향 25.1% / 5년 배당수익률 2.1% 모두 사용자 제시 표와 일치. 분기 배당 차분도 실제 회차와 일치(23~24년 361원씩, 25.4Q 566원).
- **비용**: 첫 열람만 DART ≈ 41콜 + 금융위 ≈ 7~19콜(동시성 5, 3.7~8.6초 실측), 이후 캐시. DART 일 한도 20,000콜 대비 여유. **KIS 콜 0 · 잡 6종·스케줄 무변경.**
- **상태**: **구현 완료(2026-07-28)**.

### Phase 73 — 알림 종류별 on/off(배당 분리·음소거 예외) + 시세 조건 3을 「지수 대비 상대 열위」로 (2026-07-29, 구현 완료)

- **요청 근거**: 사용자 — ① 「내 배당」 탭에 배당 알림 설정을 붙이고 싶다. **전체 푸시 알림은 꺼도 배당 소식만큼은 따로 받고 싶은데 가능한지** 확인 요청 ② 시세 알림 조건 3을 "소속 지수 −2% + 종목 −12% 동반 하락"에서 **두 값의 차이가 −10%p 이상**일 때 울리도록 변경.
- **사전 조사(코드 작성 전 회신)**: 현행 구조로는 **불가능**했다 — 발송 경로가 `sendPushToEmail` 하나뿐이고 페이로드에 종류 구분자가 없으며, on/off 축이 ①기기 구독(해지=발송 주소 파기) ②종목 음소거(4종 알림이 공유) 둘뿐이라 "종류별"이라는 축 자체가 없었다. 따라서 **"전체 끄기"의 의미를 구독 해지에서 앱 내 종류 설정으로 옮기는 것**이 전제(구독을 해지하거나 OS 권한을 거부하면 서버가 무엇을 해도 못 보내는 게 웹푸시 스펙).
- **정책(사용자 확정)**: 음소거 우선순위는 **배당만 예외**(종목을 꺼도 배당은 발송) · 배당 카테고리는 **공시-배당 + 지급일 알림을 하나로 묶음**. → ⚠️ **음소거 예외는 Phase 79에서 폐기**(사용자 확정 번복 — 종목별 알림을 끄면 4종 전부 차단). 아래 구현 서술 중 "음소거 건너뜀"·"배당은 예외" 부분은 이력이며 현행 아님. **배당 카테고리 묶음은 그대로 유효**.
- **구현**:
  - `lib/alerts/categories.ts`(신규) — `AlertCategory` 4종·기본값(전부 켬)·화면 메타·`isAlertCategory`. Redis를 물지 않는 순수 모듈이라 서버·클라이언트 공용.
  - `lib/alerts/store.ts` — `alerts:{email}:prefs` read/write. **평문 저장**(보유종목을 드러내지 않는 취향 값 — 쿨다운 키와 같은 기준), 읽기는 `{...기본값, ...저장값}` 병합(카테고리가 늘어도 기존 사용자가 새 알림을 놓치지 않음), 전부 켬이면 키 삭제(muted 관례).
  - `lib/alerts/evaluate.ts` — 조건 3 교체(아래) + `prefs.price` 게이팅. 게이팅은 **발송에만** 걸고 신고가 갱신·저장은 계속한다(꺼 둔 동안 신고가가 멈추면 다시 켰을 때 낡은 기준으로 오탐).
  - `lib/alerts/feedAlerts.ts` — 매칭 라벨에 「배당」이 있으면 **배당 이벤트로 배타 분기**(같은 공시 중복 발송 방지)해 제목 `배당 공시 — {종목}`으로 발송, **음소거 건너뜀**. 링크는 공시 원문이 있는 `/feeds` 유지. 리포트에 `dividendDisclosures` 그룹·각 그룹 `prefSkipped` 추가.
  - `lib/alerts/dividendAlerts.ts` — 음소거 조회 제거(`getMutedSymbols` → `getAlertPrefs`), `mutedSkipped` → `prefSkipped`. 이메일당 Redis 읽기 수는 그대로.
  - `app/alerts/actions.ts` — `setAlertCategoryEnabledAction`(화이트리스트 검증 후 저장, `revalidatePath`는 `/alerts`+`/dividends` 둘 다).
  - `components/alerts/CategoryAlertToggles.tsx`(+`.module.css`, 신규) — 항목 배열을 받는 범용 토글. `/alerts`는 4종, 「내 배당」 탭은 `dividend` 1종만 넘겨 **같은 컴포넌트·같은 키 재사용**.
  - `app/alerts/page.tsx` — 「알림 종류」 카드 신설(구독 카드와 종목별 카드 사이) + 종목별 토글 아래 「배당은 예외」 안내 한 줄. 세 블록 각각 try/catch 격리.
  - `app/dividends/page.tsx`(+CSS) — 「내 배당」 탭 목록 아래 「배당 알림」 카드. `getAlertPrefs` 실패는 토글만 숨기고 목록은 그대로.
- **조건 3 변경의 근거**: 구 규칙은 임계 2개의 **AND**라 지수가 −1.9%면 종목이 −20%여도 안 울리는 사각지대가 있었다. 두 값의 **차이** 1개로 바꾸면 그 구멍이 사라지고, **구 임계쌍의 간격이 정확히 −10%p라 기존 발동 지점은 그대로 포함**된다. 신·구 대조(실행 검증): `(−2.0, −12.0)` 구·신 모두 울림 / `(−1.9, −20.0)` 구 침묵→신 울림(gap −18.1%p) / `(+1.5, −9.0)` 구 침묵→신 울림(−10.5%p) / `(−3.0, −8.0)`·`(−5.0, −11.0)` 양쪽 침묵. **특성 고지**: 순수 차이라 지수가 크게 오른 날은 종목이 보합이어도 발동할 수 있으나(지수 +11%/종목 0%), 코스피 역대 최대 일간 상승률이 그 수준이라 실동작에서는 하락 국면 판정과 같다.
- **알림 문구**: `코스피 −2.10% 대비 종목 −12.30% (−10.2%p 열위)` — 지수·종목 값을 둘 다 보여줘야 "무엇에 비해 밀렸는지"가 드러난다.
- **검증**: tsc·eslint·build 통과. 조건 3은 신·구 규칙 8케이스 대조 스크립트로 확인(위). 잡 6종·스케줄·KIS 콜 **불변**, 새 Redis 키는 사용자당 1개(`:prefs`, 기본값이면 아예 생성 안 됨).
...
### Phase 75 — 종목분석 차트 6종 → 4종 정리 + 현금흐름 계열 꺾은선 전환 (2026-07-29, 구현 완료)

- **요청 근거**: 사용자(Phase 72 배포 후 실화면 확인 중) — **「배당성향·배당금」·「분기배당금」 두 차트를 삭제**하고, **「현금흐름표」·「주당현금흐름」은 실선(꺾은선) 차트로** 바꿔달라.
- **판단**: 삭제 2종은 모두 **다른 화면에 같은 값이 이미 있다** — 배당성향은 「주요 재무지표 표」 행(`KeyMetricsTable`)과 투자지표(`view.ts`)에 그대로 있고, 분기배당금은 「배당금·시가배당률」 차트의 **분기 탭**이 같은 `dps` 계열을 그린다(그 차트만 `defaultMode="quarter"`로 열어 두었을 뿐 데이터가 다르지 않다). 따라서 **표시 계층에서 차트 2장만 걷어내면 되고 파이프라인은 손댈 필요가 없다** — `payoutRatio`·분기 `dps` 계산은 그대로 둔다(표·투자지표가 계속 쓴다).
- **구현**(`components/analysis/AnalysisCharts.tsx` 1파일):
  - 「배당성향 · 배당금」·「분기배당금」 `MetricChart` 2블록 삭제 → 차트 **6종 → 4종**(실적 / 배당금·시가배당률 / 현금흐름표 / 주당현금흐름).
  - 현금흐름 2종의 계열 4개(`cfo`·`cfi`·`cff`·`cfps`)에 `type: "line"` 지정. `MetricChart`는 이미 spec별로 `Line`/`Bar`를 갈라 그리므로 **렌더 로직 변경 없음**(`connectNulls`라 결측 분기에서 선이 끊기지 않는다).
  - `MetricChart`의 `defaultMode` prop 제거 — 유일한 사용처가 삭제한 「분기배당금」이었다. 남은 4종은 전부 "연간"으로 열린다.
- **범위**: 표시 계층만. `overview.ts`·`view.ts`·캐시 키·DART/금융위 호출·잡 6종 전부 불변(재시딩 불필요).
- **검증**: tsc·eslint·build 통과.
- **상태**: **구현 완료(2026-07-29)**.
...
### Phase 77 — 클라이언트 캐시 TTL 30초 → 600초 + 갱신 회차 자동 새로고침 (2026-07-29, 구현 완료)

- **요청 근거**: 사용자 — "prefetch 시간이 30초인데 어차피 10분마다 갱신인데 30초가 의미있나. 첫 접속 시 다음 갱신까지 몇 분 남았는지 체크하고 그때까지 유지하는 식은 어때? 그리고 갱신되면 자동 새로고침 가능할까?" → 가능성 조사 후 **B안(타이머 + 경량 확인) 승인**, "무단 화면 변경은 상관없다, 그냥 새로고침"으로 확정.
- **먼저 바로잡은 용어**: `staleTimes.dynamic`(Phase 48)은 **prefetch 시점**이 아니라 이미 받아둔 RSC 페이로드의 **클라이언트 라우터 캐시 TTL**이다(prefetch 시점 제어는 별건으로 `HoverPrefetchLink`가 담당). 지적대로 10분 주기 데이터에 30초 TTL은 갱신 간격의 1/20만 덮어 캐시 이득 대부분을 버리고 있었다.
- **아이디어 1(남은 시간만큼 유지)은 불가 확정**: `staleTimes`는 `next.config.ts`의 **빌드타임 고정값**이고, 요청·시각별로 계산해 넣을 훅도 런타임 API도 없다(`staleTimes.md` — `static`/`dynamic` 두 개의 초 값이 전부, 여전히 `version: experimental`). 대신 **TTL은 갱신 간격과 같은 600초로 고정하고, 갱신 시점에 캐시를 능동적으로 버리는** 아이디어 2로 같은 결과를 얻는다.
- **알림에 미치는 영향: 없음 (구현 전 확인)** — 공시·10% 하락 등 알림 4종은 **전부 서버측 QStash 잡에서 판정·발송**되고 브라우저 상태와 무관하다: 시세 3조건은 `refreshMarketData` → `evaluatePriceAlerts`, 공시·시장경보는 `refreshFeeds` → `evaluateFeedAlerts`, 배당은 `evaluateDividendAlerts`. 화면이 열려 있든 닫혀 있든, 라우터 캐시가 묵었든 발송 시점·내용이 달라지지 않는다. 푸시 클릭 진입도 `sw.js`의 `client.navigate(url)` = **브라우저 네비게이션**이라 라우터 캐시를 우회해 항상 최신을 받는다. 즉 자동 새로고침은 **이미 도착한 알림을 화면에서 확인하는 경로만** 빨라지게 하고 알림 자체는 건드리지 않는다.
- **구현**:
  - `next.config.ts` — `staleTimes.dynamic` 30 → **600**.
  - `api/market/last-refresh/route.ts` (신규, GET) — `market:lastRefreshAt`의 성공 시각 하나만 반환(**Redis GET 1회·KIS 호출 0**). 잡 라우트와 달리 **세션 인증**(호출 주체가 브라우저) — proxy matcher에 더해 라우트에서도 `isEmailAllowed`까지 확인(Phase 76 이중 방어 방침). 응답 `Cache-Control: no-store`(캐시되면 갱신 감지 자체가 무의미).
  - `app/actions.ts` (신규) — `invalidateMarketRouterCache()`: `revalidatePath("/", "layout")`. **`router.refresh()`만으로는 현재 라우트의 캐시만 지워** 갱신 직후 다른 화면으로 이동하면 600초 안의 묵은 세그먼트가 그대로 보인다. 이 앱은 화면이 전부 동적 렌더(`auth()`가 쿠키 접근)라 무효화할 서버 캐시가 없고, 노리는 효과는 클라이언트 캐시 전량 무효화 하나뿐이다.
  - `components/ui/AutoRefresh.tsx` (신규, 클라이언트) — 렌더 출력 없음. 루트 `layout.tsx`에 배치해 전 화면 커버.
- **AutoRefresh 상태머신**: 마운트 시 기준값(`at`) 확보 → 예정 회차(`nextScheduledRefreshMs`, §52에서 이미 만든 함수 재사용)까지는 **서버를 부르지 않음** → 회차가 지나면 30초 간격 확인 → `at`이 기준값과 달라지면 `invalidateMarketRouterCache()` + `router.refresh()` 후 다음 회차 대기 → 5분 안에 안 바뀌면 그 회차는 포기하고 다음 회차로.
  - **예정 시각만 보고 곧바로 새로고침하지 않는 이유**: 잡 실행·Redis 반영에 편차가 있다(같은 이유로 staleness 판정도 20분 유예를 둔다). 실제 반영을 확인하고 나서 버린다.
  - **경로는 의존성이 아니라 ref로 읽는다** — 화면을 옮길 때마다 타이머를 다시 걸면 예정 회차 대기 상태가 초기화돼 그 회차를 통째로 놓친다(`nextScheduledRefreshMs`는 늘 "지금 이후"를 주므로 다음 회차로 밀린다). 타이머는 마운트 때 한 번만 건다.
  - 백그라운드 탭은 타이머가 스로틀링되므로 `visibilitychange` 복귀 시 한 번 더 확인하고, `visible`이 아니면 확인 자체를 건너뛴다. `/login`도 제외.
- **검증**: tsc·eslint·build 통과(라우트 29개). 회차 계산 8케이스 실측(장중 10분 간격·15:40·18:15·야간→익일 09:00·금요일 밤/토/일→월요일 09:00 전부 정확). 상태머신 시뮬레이션(30초 tick, 장중 6.5시간): **잡 60초 반영 시 요청 120회·새로고침 40회로 회차 후 1분 내 화면 반영**, 4분 반영 시 360회·4분 내 반영, **잡 전면 실패 시 480회·새로고침 0회**(회차당 5분 창 12회 후 포기), **주말 24시간 요청 0회**(예정 회차가 없어 확인 자체를 안 함).
- **범위**: 캐시·표시 계층만. 알림 판정·발송, KIS 호출, Redis 쓰기, 잡 6종·스케줄 전부 불변. 새 Redis 키 0(기존 `market:lastRefreshAt` 읽기만).
- **상태**: **구현 완료(2026-07-29)**.

---

### Phase 78 — 종목분석 첫 열람 20초 진단 + 확정 종가 캐시·조회 동시성·시세 블록 스트리밍 (2026-07-29, 구현 완료)

- **요청 근거**: 사용자 — "종목분석 상세에서 분석 버튼 누르면 최장 20초 걸리는 것 같은데 연환산 같은 항목 때문이야? 확인해줘." → 원인 실측 후 **캐시 분리 + 동시성 상향 승인**, 이어 "5초 걸리든 10초 걸리든 사용자가 오해하지 않게 진행 표시가 가능한지" 확인 요청 → **A안(Suspense 분할) + B안(경과 시간) 승인**.
- **지목된 원인은 아니었다 — 실측으로 특정**: 연환산(TTM)은 `buildTtmPoints`·`withTtmValuation` **순수 함수**라 밀리초다. 단계별 실측(005930, 캐시 미스 재현):

  | 단계 | 콜 | 시간 |
  | --- | --- | --- |
  | `getAnalysisOverview` — DART 전부(연간·분기·연환산 원자료) | 45 | **1.0초** |
  | `getAnalysisQuote` — 금융위 본 시계열 | 1 | 0.8~5초 |
  | `getAnalysisQuote` — **기간말 종가 재조회** | 15 | **5~15초** |
  | `getStockMaster`(161KB) — 중복 호출 | 2 | 1.2~1.5초 |

- **진짜 원인 — 금융위 API의 불규칙 지연**: 같은 쿼리 반복 측정에서 응답이 **두 값으로 갈린다** — 130~170ms 아니면 **3.7~5.2초**. 순차 호출에서도 나타나므로 동시성 스로틀이 아니라 백엔드 편차다(`순차 5콜: 163, 142, 3868, 5444, 178ms`). 여기에 동시성 5가 곱해져 15콜이 **3라운드**로 쪼개지고, 각 라운드가 자기 라운드의 최악 콜에 묶여 15초. 앞의 본 시계열 1콜을 **순차로** 먼저 받는 것까지 더해 20초가 된다.
- **구현 ①(캐시) — `analysis:closes:v1:{종목코드}`, TTL 1년**: `{"2023Q2": 71500, ...}` 꼴로 종목당 한 키. 지나간 기간말 종가는 **무수정주가라 소급 조정이 없어 불변**인데, quote TTL 6시간이 만료될 때마다 2020년치까지 전부 다시 받고 있었다. 종목당 1키로 묶은 이유는 라운드트립(20키면 GET도 20회)과 키 누적(종목당 20키 × 2,800종목) 둘 다 피하려는 것. TTL 1년은 값의 수명이 아니라 **안 보는 종목의 키를 걷어내는 장치**다.
  - ⚠️ **`CLOSE_SETTLE_DAYS = 7`**: 금융위는 영업일+1 오후 1시에 확정 종가를 올린다. 갓 지난 기간말을 조회하면 아직 데이터가 없어 **직전 거래일 종가**가 잡히는데, 그걸 영구 캐시하면 틀린 값이 1년간 고정된다. 7일 지난 기간말만 저장하고 나머지는 다음 열람으로 미룬다.
- **구현 ②(동시성) — `QUOTE_CONCURRENCY = 20`**: 재무용 `DART_CONCURRENCY`(5)를 그대로 쓰던 것을 끊었다. 성격이 다른 두 API가 상수를 공유한 게 문제의 절반이었다. 동시 15콜 실측에서 총 시간이 최악 콜 하나(5.2초)와 같아 스로틀 흔적이 없음을 확인하고 1라운드화했다.
- **구현 ③(A안 — Suspense 분할 스트리밍)**: DART 1초 대 금융위 5~15초라는 10배 격차를, 예전에는 **재무 차트까지 시세에 묶어** 다 함께 기다렸다. 시세 의존 블록만 `<Suspense>`로 감싸고 `quotePromise`를 `await` 없이 만들어 넘긴다(호출은 한 번뿐이라 중복 조회 없음, `getAnalysisQuote`는 실패해도 null이라 reject되지 않는다).
  - **경계는 「연환산 탭까지 포함해」 판정했다** — 실적·현금흐름표·주당현금흐름은 전 탭이 DART 값만 쓰므로 재무용, 배당금&시가배당률은 **연환산 탭의 시가배당률이 종가로 계산**되므로 시세용. 이 판정 때문에 `AnalysisCharts`의 export를 `AnalysisFinancialCharts`/`AnalysisQuoteCharts` 둘로 갈랐다(같은 모듈이라 recharts 청크는 그대로 하나).
  - **화면 순서는 불변**(투자지표 → 주가 변동률 → 차트 4종 → 표). 그 사이에 재무 차트가 끼어 있어 경계가 3개로 나뉘었다 — 시세 블록을 하나로 합치면 순서가 무너진다.
  - 주요 재무지표 표는 **시세 쪽에 뒀다**. PER·PBR이 표의 핵심 지표라 `-`로 먼저 보이면 그게 곧 사용자가 말한 "오해"다.
  - `page.tsx`의 meta 문구에서 시세 출처 표기를 뺐다 — 이 한 줄 때문에 헤더가 시세를 기다렸다. 출처는 하단 각주에 이미 있고, 기준일은 투자지표 블록이 표시한다.
- **구현 ④(B안 — 경과 시간)**: `QuoteElapsed`(클라이언트) + `QuotePendingBlocks`(자리표시자 3종). **진행률(%)은 일부러 쓰지 않았다** — 서버 렌더 중의 진행을 클라이언트로 흘릴 표준 경로가 없어 예측값으로 채울 수밖에 없는데, 편차가 3배라 100%에 닿고도 안 끝나는 상황이 잦다(오해를 막으려다 되레 키운다). 대신 **실제 경과 초**만 보여주고, 3초 전에는 숨기며(캐시 히트 때 평소에도 느린 화면처럼 보인다) 8초를 넘기면 "처음 조회하는 종목은 10초 이상 걸릴 수 있습니다"를 덧붙인다. 매초 바뀌는 숫자는 `aria-hidden`(스크린리더가 시끄럽다), 자리표시자는 실제 블록과 같은 치수·`prefers-reduced-motion` 대응.
- **구현 ⑤(곁가지)**: `resolveName`을 `cache()`로 감쌌다 — `generateMetadata`와 본문이 같은 요청에서 각각 불러 161KB `market:stockMaster`를 두 번 읽고 있었다(Redis SDK라 fetch dedupe가 안 걸린다).
- **검증**: tsc·eslint·build 통과(라우트 29개). 실측 —
  - 확정 종가 캐시(000660, 캐시 삭제 후): **미스 12.6초/14콜 → 히트 285ms/0콜**, 14/14건 확보.
  - 동시성 5 vs 20(005930 15콜): **16.3초 → 10.8초**.
  - 금융위 편차가 커 첫 열람 절대시간은 시간대별로 5~15초를 오간다 — **캐시가 없는 첫 열람은 여전히 10초대가 가능**하고, 그래서 ③④가 함께 필요했다.
- **범위**: 종목분석 상세 화면과 그 read-through 캐시만. KIS·잡 6종·스케줄·알림 전부 불변. 새 Redis 키 1종(`analysis:closes:v1:*`).
- **상태**: **구현 완료(2026-07-29)**. 스트리밍 실동작(재무 차트가 시세보다 먼저 뜨는지·경과 시간 표시)은 배포 후 확인 대기.

### Phase 79 — 종목별 알림 off = 그 종목 4종 전부 차단 (Phase 73 배당 음소거 예외 폐기) (2026-07-29, 구현 완료)

- **요청 근거**: 사용자 — "전체 알림 설정에서 LG전자의 종목별 알림을 껐는데 배당 알림을 켜 두니 배당 푸시가 왔다. **종목별 알림을 끄면 (기기 알림을 뺀) 4종이 전부 꺼지게** 할 수 있는지 확인해 달라."
- **원인(코드 작성 전 회신)**: 버그가 아니라 **Phase 73에서 사용자 확정으로 넣은 예외**였다("종목 알림을 꺼도 배당 소식은 놓치고 싶지 않다"). 배당은 두 경로로 나가는데 둘 다 음소거를 보지 않았다 — 배당 공시(`feedAlerts.ts`, `prefs.dividend`만 확인)와 지급일 당일(`dividendAlerts.ts`, `getMutedSymbols` 조회 자체가 없음). 시세·공시·시장경보 3종은 정상 적용 중이었다.
- **정책(사용자 확정 — Phase 73 번복)**: 예외를 폐기하고 **4종 모두 `prefs.{종류}` AND 음소거를 통과해야 발송**. 두 축의 역할이 이제 완전히 직교한다 — 「알림 종류」는 **모든 종목에** 걸리는 전역 축, 종목별 토글은 그 위에 종목 단위로 걸리는 AND 게이트.
- **구현**:
  - `lib/alerts/feedAlerts.ts` — 배당 공시 루프에 `mutedSet` 가드 추가. 리포트 `dividendDisclosures`에 `mutedSkipped` 신설(다른 3종과 집계 형태 일치). 분류기 주석의 「음소거 예외」 설명 정리.
  - `lib/alerts/dividendAlerts.ts` — `getMutedSymbols` 재도입, 이메일 루프 안에서 `myDue` 건별 필터 + `mutedSkipped` 집계. **종목×지급일 전역 마커(`alerts:dividend:sent:*`)의 기록 순서는 손대지 않았다** — 마커는 사용자와 무관한 회차 단위 중복 차단 장치라, 음소거한 사용자가 있어도 다른 사용자 발송에 영향이 없다(마커는 어차피 발송 여부와 무관하게 먼저 찍힌다).
  - `lib/alerts/categories.ts` — 「배당」 설명에서 "종목별 알림을 꺼도 배당은 계속 받습니다" 삭제, 모듈 주석의 예외 서술을 4종 공통 규칙으로 교체.
  - `app/alerts/page.tsx` — 종목별 토글 아래 안내를 「배당은 예외」에서 "종목을 끄면 그 종목의 시세·공시·시장경보·배당 알림이 모두 멈춥니다 / 위 「알림 종류」는 모든 종목에 함께 적용됩니다"로 교체(두 축의 차이를 문구로 남김).
  - `app/dividends/page.tsx` — 「내 배당」 탭 배당 알림 카드 각주에 "종목별 알림을 끈 종목은 배당도 오지 않습니다" 한 줄 추가. 그 위 문장("다른 알림을 다 꺼도 이 항목만 켜 두면 배당은 받는다")은 **종류 축 이야기라 여전히 사실**이므로 유지.
  - `components/alerts/StockAlertToggles.tsx` — 컴포넌트 주석 갱신.
- **범위**: 알림 판정 2파일 + 문구 4곳. **Redis 스키마·키·잡 6종·스케줄·KIS 콜 전부 불변**, 새 저장값 없음. 되돌리려면 가드 2개를 지우면 된다.
- **검증**: tsc·eslint 통과.
- **상태**: **구현 완료(2026-07-29)**. 실동작(LG전자 음소거 상태에서 배당 공시·지급일 알림이 오지 않는지)은 다음 배당 이벤트 때 확인.

---

### Phase 80 — 종목분석 분기 시계열에서 진행 연도 누락 수정 (2026-07-29, 구현 완료)

- **요청 근거**: 사용자 — "지금 2026년 7월 29일인데 종목분석 상세화면에 아직 2026년 1분기·2분기 정보가 없다. 원래 없는 건지 문제인지 확인해 달라."
- **판정(코드 작성 전 회신)**: **1분기는 버그로 누락, 2분기는 원래 없는 게 맞다.**
  - DART 실측(삼성전자 00126380·SK하이닉스 00164779·카카오 00258801, 2026-07-29): 2026 **1분기보고서(11013)는 `status 000`·215~232행 존재**, 반기(11012)·3분기(11014)는 `status 013 조회된 데이타가 없습니다`. 반기보고서 법정 제출기한이 8/14라 지금 없는 것이 정상.
  - 원인은 `overview.ts:721`의 `annualYears.slice(0, OVERVIEW_QUARTER_YEARS)` — **연간 시계열의 연도 배열을 분기에 그대로 재사용**한 것. `latest = currentKstYear() - 1`은 **사업보고서 기준으로는 옳다**(2026 사업보고서는 2027년 3월 제출). 하지만 분기보고서는 당해 연도에 나오므로 이 배열을 쓰면 진행 연도가 후보에서 원천 제외된다 → `quarterYears = [2025, 2024, 2023]`, 2026은 **요청조차 하지 않았다**. 그 결과 분기 탭이 2025Q4에서 정지하고, 분기 롤링 합인 연환산(TTM) 탭도 함께 밀렸다.
  - 시세 쪽은 무관했다 — `quote.ts`의 분기말 종가는 **시세 최신 연도**(2026) 기준 4년치라 2026-03-31 종가를 이미 확보 중이었다. 재무만 늘리면 맞물린다.
  - `financials.ts`(재무제표 전문)는 연간만 다뤄 영향 없음.
- **구현**:
  - `lib/analysis/overview.ts` — 연도 배열 재사용을 끊고 **`buildQuarterJobs(확정연도[])`** 신설. 확정 사업연도 3년은 4개 분기를 전부 받고, **진행 중인 올해는 제출기한(`QUARTER_FILING_DEADLINES` 1Q=0515·반기=0814·3Q=1114)이 지난 분기만** 덧붙인다. 4분기는 사업보고서(이듬해 제출)라 진행 연도에 결코 없으므로 기한 표에서 제외. 오늘 기준 job 13개(= 12 + 2026Q1)로, 없는 보고서 3콜을 부르지 않는다. `collectQuarterly`는 `years` 대신 job 목록을 받는다.
  - **왜 "연도 4개 통짜"가 아니라 기한 컷오프인가** — 진행 연도를 통째로 붙이면 매 조회마다 아직 없는 보고서 최대 3개 × (재무제표+배당) = 6콜을 버린다. 기한이 갓 지난 시점에 아직 제출 안 한 회사를 부르는 건 남지만, 빈 응답은 기존 `rows.length > 0` 필터가 이미 걸러 낸다.
  - **주식총수 폴백** — 진행 연도는 사업보고서가 없어 `stockTotqySttus`가 비어 온다(2026Q1 실측: `합계` 행의 `istc_totqy`가 `"-"`). BPS·CFPS가 통째로 비지 않게 **직전 사업연도 말 주식수**로 대신한다. 확정 연도의 동작은 건드리지 않으려 `has(year)`로 갈랐다 — 확정 연도인데 값이 null이면 폴백 없이 그대로 null(조회 실패를 다른 해 값으로 덮지 않는다). 어차피 확정 연도에서도 한 해의 4개 분기가 연말 주식수를 함께 쓰고 있어 근사 수준이 같다.
  - **캐시 프리픽스 `analysis:overview:v1:` → `v2:`** — TTL이 30일이라 올리지 않으면 이미 캐시된 종목이 최대 한 달간 옛 시계열(2025Q4 정지)을 계속 내려준다. v1 키는 TTL로 자연 소멸.
  - `lib/analysis/quote.ts` — `QUOTE_QUARTER_YEARS = 4`의 근거 주석 갱신. 이제 overview 분기 범위가 「확정 3년 + 진행 연도」로 달력 연도 4개라 시세 쪽 4년과 **정확히 겹친다**(전에는 한 해 어긋난 것을 여유로 덮고 있었다). 값은 그대로.
- **범위**: 재무 리더 1파일 + 시세 주석 1줄. **KIS·잡 6종·스케줄·알림·화면 컴포넌트 전부 불변.** Redis는 키 1종의 프리픽스만 v2로. DART 콜은 종목당 24 → 26(분기 재무제표+배당 각 +1).
- **검증**: tsc·eslint 통과. job 생성 로직 복제 검증 — 오늘(0729) 기준 `2023Q1~2025Q4 + 2026Q1` 13개, 2026Q2~Q4 미포함. 2026Q1 실측값(삼성전자, 1분기는 누적=분기값): 매출 1,338,734억 · 영업이익 572,328억 · 지배지분자본 4,739,648억.
- **상태**: **구현 완료(2026-07-29)**. 배포 후 화면에서 분기 탭 최신이 25.4Q → 26.1Q로, 연환산 탭도 한 칸 밀려 나오는지 확인 필요.

---

### Phase 81 — 뉴스·공시에 「실적」 탭 신설 + 알림 종류 4종 → 5종 (2026-07-29, 구현 완료)

- **요청 근거**: 사용자 — "뉴스 공시 항목 상세화면에 실적 탭 하나 추가해서, 해당 회사 컨퍼런스콜에서 실적 발표하거나 그 전에 보도자료·발표로 실적이 공개되면 추가하는 게 가능한지 확인해 달라. 추가로 푸시 알림에 포함될 수 있는지(4종 → 5종?)." → 타당성 회신 후 "권장 범위는 다 넣어줘"로 전체 구현 승인.

#### 81.1 타당성 판정 (DART 실측, 2026-07-29)

- **보도자료·컨퍼런스콜을 따로 크롤링할 필요가 없다.** 공정공시 규정상 기관투자자·애널리스트에게 실적을 제공하기 전(또는 동시에) DART에 공시할 의무가 있어, **공정공시 접수 시각이 곧 실적 공개 시점**이다.
- 보고서명 vocabulary가 매우 정형적이다 (2026-05-01~05-20 거래소공시 전수 2,488건 실스캔): `연결재무제표기준영업(잠정)실적(공정공시)` 235건 · `기업설명회(IR)개최(안내공시)`+`기업설명회(IR)개최` 338건 · `영업(잠정)실적(공정공시)` 87건 · `결산실적공시예고` 25건 · `영업실적등에대한전망(공정공시)` 5건.
- **커버리지 한계(정직하게 기록)**: 1Q 시즌(4/1~6/15) 잠정실적 공정공시는 656건인데 정기보고서는 3,603건 — **잠정실적을 미리 내는 회사는 상장사의 20~25%(주로 대형·중견)**이고 나머지는 분기보고서가 최초 공개다. 그래서 정기공시(A)도 함께 수집한다.
- **컨퍼런스콜 내용 자체는 못 가져온다** — DART엔 IR 개최 일정·장소만 있고 가이던스 발언·Q&A는 없다. 그건 기존 뉴스 탭이 사후에 커버한다.

#### 81.2 왜 기존 공시 수집을 재사용할 수 없었나 (핵심 제약)

기존 `refreshDisclosures`는 **유형 무필터 최근 90일 상위 10건**이다. 대형주는 임원·주요주주 소유상황보고 등으로 이 창이 포화된다 — **삼성전자 90일 818건 실측**, 상위 10건에 실적 공시가 **한 건도 없다**(SK하이닉스 57건 중 7·9번째, NAVER 48건 중 15번째부터). 그래서 별도 수집 경로가 필수였고, `pblntf_ty=I`(거래소공시)를 걸면 **같은 조건이 15건으로 줄어 1페이지에 다 들어온다**. 이것이 이 Phase의 설계 근거 전부다.

#### 81.3 구현

- `lib/api/dart/client.ts` — `fetchDartDisclosures`에 `pblntfTy` 옵션 + 코드 상수 2종(`DART_PBLNTF_EXCHANGE`="I"/`DART_PBLNTF_PERIODIC`="A"). `fetchDartDocumentText`를 export로 승격. **`parseDartEarningsDetail`/`fetchDartEarningsDetail` 신설** — 잠정실적 원문의 `{계정명} 당해실적` 뒤 고정 7칸(당기·전기·전기대비·흑자적자전환·전년동기·전년동기대비·흑자적자전환)에서 0·4·5·6번째를 뽑는다.
- `lib/feeds/earnings.ts`(신규) — 실적 6유형 분류기. Redis·web-push 무의존 순수 모듈이라 잡·알림·화면이 공유한다.
- `lib/feeds/store.ts` — `market:earnings:{code}`(`EarningsItem`/`EarningsFigure`/`getEarningsSnapshots`/`setEarnings`/`earningsKey`).
- `lib/jobs/refreshFeeds.ts` — `refreshEarnings` 신설(5단계). 종목당 DART 2콜(I → A), 실적 유형만 남겨 접수번호 dedupe → 최신 10건. **잠정실적 원문 재파싱 방지**: 직전 스냅샷에 같은 접수번호가 `parsed`로 있으면 결과를 물려받고, 회차당 신규 원문 파싱은 20건 상한(첫 회차에 전 종목 zip을 한꺼번에 받지 않게).
- `lib/alerts/categories.ts` — `AlertCategory`에 `earnings` 추가(4종 → 5종). **마이그레이션 불필요** — `getAlertPrefs`가 기본값 위에 저장값을 덮는 구조라 기존 사용자는 자동으로 켬 상태가 된다(`store.ts` 주석이 이미 이 확장을 예고하고 있었다).
- `lib/alerts/store.ts` · `feedAlerts.ts` — `alerts:earnings:last:{code}` 커서 신설(공시 커서와 **분리** — 유형을 좁힌 별도 조회라 접수번호 흐름이 달라 공유하면 서로를 삼킨다) + 실적 발송 분기. 실적 6유형은 기존 공시 8유형과 키워드가 하나도 겹치지 않아 **배당(Phase 73) 같은 배타 분기가 필요 없다**.
- `lib/feeds/homeFeed.ts` — `getEarningsBoard`/`EarningsBoardItem`(공시 게시판 모델 + 유형·수치·대상기간·단위).
- `components/feeds/FeedTabsClient.tsx` — 4탭(뉴스·공시·**실적**·수출입) + `EarningsBoard`. 진입 탭은 서버가 `?tab=`을 파싱해 `initialTab`으로 주입(`useSearchParams` Suspense 경계를 안 만들려는 선택). 흑자·적자 전환은 증감 칸에 라벨로 상승/하락색.
- `app/feeds/page.tsx` — `searchParams` 수용 + 탭 화이트리스트 폴백.
- `lib/jobs/cleanupOrphanStocks.ts` — 고아 키 7종 → **9종**(`market:earnings:{code}`·`alerts:earnings:last:{code}`).

#### 81.4 파싱 견고성 — 실측으로 잡은 함정 2건

1. **`[기재정정]` 공시는 실적표가 아니라 "정정전/정정후" 나열이다.** 같은 정규식이 엉뚱한 7칸에 걸린다(실측 서울반도체 20260512900204: `["당기실적(2026.1Q)","235,094",…]`). → **칸 모양 검증**(금액 5칸은 숫자 모양, 전환 2칸은 라벨 모양) + **`matchAll`로 모양을 만족하는 첫 후보 선택**. 이 둘을 합쳐야 정정 공시에서도 정정후 값(238,169)을 정확히 집는다.
2. **흑자·적자 전환이면 DART가 증감율 칸을 비우고 7번째 칸에 라벨을 넣는다.** 그냥 두면 가장 중요한 신호(적자→흑자)가 `—`로 사라진다. → `turnaround` 필드로 잡아 증감 칸에 표시.

- 월간 IR 공시(HD 계열·오리온 등)는 표준 표가 전부 `-`이고 실제 수치는 자유 서식 표에 있다 — **표 없이 제목·링크만** 보여주는 게 맞다(억지 파싱 안 함).

- **범위**: DART 콜은 종목당 1 → 3(+잠정실적 원문, 신규 건만). 종목 50개·하루 15회차 기준 ~2,250콜로 DART 일 20,000콜 한도 대비 여유. **KIS·시세 잡·스케줄 전부 불변.**
- **검증**: tsc·eslint·`npm run build` 통과. 실 DART 데이터 검증 — ① 분류기: 실적시즌 전수에서 오탐 1건(`기타시장안내 (분기보고서 미제출 …)`) 발견 → `excludes`로 제거, 배당·증자 공시는 정상 미매칭 ② 파서: 잠정실적 원문 **133건 파싱**(5/1~5/12 70건 전건 표 추출 성공, 5/13~5/20 63건 중 59건 성공·3건은 월간 IR로 표 없음이 정상·1건은 원문 미제공) ③ 로컬 dev에서 4탭 렌더·`initialTab` 반영 확인.
- **상태**: **구현 완료(2026-07-29)**. 배포 후 16:49 KST 수동 트리거 1회로 데이터 적재 완료(13종목·원문 14건 파싱·커서 13종목 baseline). 저장값 교차검증 — 삼성전자 26.1Q 매출 1,338,734억·영업이익 572,328억으로 **Phase 80에서 DART 직접 조회로 확인한 값과 일치**.

#### 81.5 후속 — 실적 알림 범위를 3종으로 좁힘 (Phase 81-1, 2026-07-29)

- **요청 근거**: 사용자 — "실적 공시 알림은 전부는 아닐 거고 잠정실적과 확정실적만 알림 울려?" → 확인해보니 **6유형 전부 울리고 있었다**(사용자 예상과 불일치).
- **실측 판단 근거**(적재된 실데이터, 보유·관심 13종목 최근 90일 58건):

  | 유형 | 건수 | 비중 | 수치표 | 종목당 최다 |
  |---|---:|---:|---:|---|
  | IR | 24 | 41% | **0** | 6건(006800) |
  | 정기보고서 | 16 | 28% | 0 | 2건 |
  | 잠정실적 | 15 | 26% | 12 | 3건(005930) |
  | 실적예고 | 3 | 5% | 0 | 1건 |

  「IR」이 최대 소음원인데 **수치표가 담긴 건이 0건** — 내용이 "언제 어디서 IR을 연다"뿐이라 실적 정보가 없다. 「실적예고」도 같은 성격(발표 일정 안내).
- **결정(사용자 확정)**: 알림 = **잠정실적 · 정기보고서 · 실적변동** 3종. **화면 탭은 6유형 유지** — 일정 안내도 훑어보는 값은 있으니 알림에서만 뺀다. 「실적변동」은 이 표본에 0건이라 소음이 없으면서 뜨면 실적 급변 의무공시(매출·손익 30% 이상 변동·매출액 미달·자본잠식)라 남긴다.
- **구현**: `feeds/earnings.ts`에 `isAlertableEarnings` 신설 — **분류(화면)와 알림 범위를 분리**해 탭은 손대지 않는다. `feedAlerts.ts`는 **커서를 유형과 무관하게 전진**시키고(제외 유형을 매 회차 재평가하지 않게) 발송만 게이팅하며, 리포트에 `typeSkipped`를 신설해 몇 건이 유형으로 빠졌는지 드러낸다. `/alerts` 설명 문구도 실제 동작에 맞춰 수정.
- **검증**: tsc·eslint·build 통과. 실데이터 58건 재생 → 알림 31건 / 제외 27건(IR 24·실적예고 3). 알림 대상 31건 중 정정본은 3건(`[기재정정]사업보고서` 2·`[기재정정]잠정실적` 1) — 정정도 수치가 바뀐 것이라 남긴다.

---

### Phase 82 — 실적 탭 개편: 종목 선택 + 「잠정+확정」 결합 분기 실적 + IR 일정 (2026-07-30, 구현 완료)

- **요청 근거**: 사용자 — "실적 탭 목록을 아래로 내리고 위에 보유/관심 종목 선택 영역 + 종목별 차트를 넣고, 그 아래 컨퍼런스콜·보도자료 실적 발표 내용 영역을 추가할 수 있는지" → 타당성 회신(`summary.md`, 2026-07-29) 후 남은 결정 1건("차트 영역을 잠정만 / **잠정+확정 결합** / 링크만 중 무엇으로")에 **2안(잠정+확정 결합) 확정**.

#### 82.1 왜 결합인가 — 이 탭의 존재 이유는 "속도"다

요청받은 「종목 선택 → 매출액·영업이익 차트 + 표」는 `/analysis/{종목코드}`에 **이미 있다**(Phase 64·72·78·80). 그대로 만들면 같은 차트가 두 곳에 생긴다. 그런데 두 데이터의 **시점이 다르다**:

| | 최신 시점 (2026-07-30 기준) | 정확도 |
|---|---|---|
| 확정 (정기보고서, 종목분석이 쓰는 것) | 26.1Q | 감사·검토 완료 |
| 잠정 (영업(잠정)실적 공정공시) | **26.2Q** (7/7 접수) | 감사 전, "실제와 차이 가능" 명시 |

**약 5주 빠르다.** 그래서 확정 분기 시계열 뒤에 **확정에 아직 없는 분기만** 잠정으로 이어 붙이고 그 지점을 「잠정」으로 표시한다 — 중복이 아니라 보완이 된다. 확정분은 `analysis/overview.ts`(TTL 30일 read-through)를 **그대로 재사용**해 같은 DART 재무제표를 두 번 캐시하지 않는다.

#### 82.2 실측으로 확정한 파싱 근거 (2026-07-30, 실 DART)

1. **전기(전분기) 3칸은 이미 받아놓고 버리고 있었다.** 잠정실적 서식 7칸 중 Phase 81은 0·4·5·6번(당기·전년동기·전년동기증감·전환)만 썼는데, 1·2·3번이 **전기실적·전기대비증감율·전기 흑자적자전환**이다. 요청받은 "전분기 대비"는 **새 API 호출 0건**, 필드 3개 추가로 끝난다.
2. **IR 개최 공시는 완전 정형이다.** 보유·관심 16종목의 2026-01~07 IR 공시 **48건 전건**에서 `1. 일시 및 장소 / 3. 개최목적 / 4. 개최방법 / 6. 주요 설명회내용(요약) / 8. IR 자료 게재일시 + 관련 웹페이지`가 깨끗하게 뽑혔다. 시각 미정 건은 서식이 `2026-06-08 --:--`로, 값 없는 칸은 `-`로 채워 온다.
3. **단위·기간 분포** (2026-07-20~29 거래소공시 잠정실적 60건 표본): 단위는 백만원 45 · 억원 15(+삼성전자는 조원), 대상기간은 달력 분기 3개월 59건 · 비달력 1건(`03-01~06-30`). → 단위는 원/천원/만원/백만원/십억원/억원/조원만 **억원으로 환산**하고 목록 밖은 환산하지 않는다. 기간은 **달력 분기 3개월만** 분기 포인트로 받는다 — 누계(반기·9개월)를 분기값으로 오인하면 시계열이 통째로 뻥튀기된다.
4. **결합 산수 교차검증** (7종목 26.2Q): 확정·잠정을 억원으로 정규화한 뒤 계산한 전분기대비가 **원문 증감율 칸과 반올림 오차 내 일치**했다 — 005930 27.74%/27.74, 000660 50.86/50.9, 005380 7.13/7.1, 373220 15.33/15.3, 105560 3.45/3.45, 012330 4.91/4.91, 066570 0.43/0.4. LG에너지솔루션 영업이익은 전기 -2,078억 → 당기 1,133억으로 **흑자전환 라벨**까지 잡혔다.

#### 82.3 판정 — 됐던 것과 안 되는 것

| 요청 | 판정 |
|---|---|
| 정기보고서 = 확정실적인가 | ✅ 맞음 (숫자는 `fnlttSinglAcntAll` 별도 API — 이미 보유) |
| 목록을 아래로 · 종목 선택 영역 | ✅ **칩 링크**로 구현 (라디오 비권장 — 아래) |
| 차트 + 표 · 전분기·전년 대비 | ✅ 결합 시계열로 |
| 컨퍼런스콜 일정 · IR 자료 링크 | ✅ IR 공시에서 정형 추출 (48/48) |
| **QnA 전문·발언록·기자간담회 내용** | ❌ **불가** — 공개 API가 없다. 기업 IR 페이지에 PDF·음성으로만 올라오고 사이트 구조가 종목마다 다르다 |
| **뉴스 본문 스크랩** | ❌ **하지 않음** — 언론사 저작권·robots.txt 문제이고 DOM이 달라 유지보수가 계속 깨진다. 발표 내용은 기존 뉴스 탭이 사후에 커버한다 |

잠정실적 원문의 자유 서술란도 실측해보니 대부분 면책 문구였다(삼성전자 건은 "외부감사인의 회계감사가 완료되지 않은 상태…"가 전부, 전망 언급 없음) — 전망 텍스트 소스로는 쓸 수 없다.

#### 82.4 구현

- `lib/api/dart/client.ts` — `DartEarningsFigure`에 `qoqBase`·`qoqPct`·`qoqTurnaround` 추가(7칸 전부 사용). **`parseDartIrDetail`/`fetchDartIrDetail` 신설** — IR 공시 원문 → 일시·장소·목적·방법·요약·자료 게재일시·IR 웹페이지. 일시조차 못 잡으면 `null`을 돌려 제목·링크만 남긴다(억지 파싱 안 함).
- `lib/feeds/earnings.ts` — `hasIrSchedule`·`needsEarningsDocument`(원문을 받을 유형=잠정실적+IR) + **`EARNINGS_PARSER_VERSION`(=2)** 신설.
- `lib/feeds/store.ts` — `EarningsFigure`에 전기 3칸(옵셔널 — v1 저장분엔 없다), `EarningsIr` 타입, `EarningsItem.ir`. **`parsed: boolean` → `parsedV: number`** 로 교체.
- `lib/jobs/refreshFeeds.ts` — 재사용 조건을 `parsedV === EARNINGS_PARSER_VERSION`으로 바꿔 **파서가 올라가면 저장분이 알아서 다시 파싱된다**(옛 값은 버전이 없어 자연히 대상). IR 원문 파싱 분기 추가. 원문 예산 20 → **30**(파싱 대상이 잠정 15 + IR 24로 늘고 버전업 재파싱이 한 번 돌기 때문 — 회차당 +4.5초).
- **`lib/feeds/earningsFocus.ts`(신규)** — 결합의 본체. `getEarningsStockOptions`(보유→관심 순, 우선주는 `supported:false`)·`getEarningsFocus`(확정 `getAnalysisOverview` + 잠정 스냅샷 **병렬** 조회 → 단위 정규화 → 확정에 없는 분기만 덧붙임 → 최근 9분기). 확정을 못 구해도 IR 일정은 살려서 돌려준다.
- `components/feeds/EarningsFocusPanel.tsx`(+`EarningsFocusChart`/`Client`, CSS) — 칩 + 최신 분기 요약 3칸(전분기·전년동기 대비) + 차트 + 분기 표 + IR 일정 카드.
- `components/feeds/FeedTabsClient.tsx` — 실적 패널을 **서버 슬롯(`earningsFocus: ReactNode`)** 으로 받아 게시판 위에 놓는다. 게시판 아코디언에 전분기 2칸(가로 스크롤 래퍼)과 IR 일정 표기 추가.
- `app/feeds/page.tsx` — `?code=` 수용. **외부 입력이라 보유·관심 목록에 있고 `supported`인 코드만 통과**시키고, 미지정이면 첫 종목을 기본 선택한다. 실적 블록 조회는 `?tab=earnings`일 때만 한다.

#### 82.5 설계 결정 3건 (근거)

1. **선택 UI는 라디오가 아니라 링크 칩.** 라디오는 ① `onChange`가 필요해 Client Component가 강제되고 ② 종목 15개를 세로로 늘어놓으면 **선택 영역이 목록보다 길어져** "목록이 길어서 아래로 내리고 싶다"는 원래 요구와 정면으로 충돌한다(모바일 PWA라 더 심하다). 칩을 `?code=`로 만들면 이 프로젝트의 `?mode=`·`?period=`·`?tab=` 관례와 같고 서버에서 다 처리된다(AGENTS.md §3). 접근성은 `role="radiogroup"`+`role="radio"`/`aria-checked`로 라디오와 동등하다 — 시각적으로만 칩이다.
2. **탭 상태를 `useState` → `?tab=` 링크로 올렸다.** 실적 상단 블록은 서버에서만 만들 수 있는데(DART 조회 + `<Suspense>`), 탭이 클라이언트 상태면 **뉴스만 보고 나가는 방문에도** 그 블록을 미리 렌더해 둬야 한다. 탭을 URL로 올리면 서버가 필요한 탭만 조립하고, 덤으로 탭이 공유·북마크 가능해진다(`?tab=`은 §81에서 이미 있었다).
3. **우선주 칩은 비활성.** 삼성전자우·삼성화재우 등은 **DART 고유번호가 없어 재무·공시가 아예 없다** — 눌러도 빈 화면이라 `aria-disabled`로 막고 이유를 `title`로 단다.

- **범위**: 새 Redis 키 **없음**(`market:earnings:{code}` 값 모양만 확장) · 잡 6종·스케줄·KIS·알림 판정 전부 불변. DART 콜은 갱신 잡에서 회차당 원문 최대 +10건, 화면에서 종목 첫 조회 시 `analysis:overview:v2:*` 캐시 미스분(30일 TTL, 종목분석과 공유하므로 이미 본 종목은 0콜).
- **검증**: tsc·eslint·`npm run build` 통과. 실 DART 검증 — ① IR 파서 48/48 ② 잠정실적 단위·기간 60건 표본 분포 확인 ③ 결합 산수 7종목 교차검증(§82.2-4).
- **상태**: **구현 완료(2026-07-30)**. 배포 후 화면 확인 필요 — 전기 칸·IR 일정은 **갱신 잡이 파서 v2로 다시 파싱한 뒤**에 채워진다(회차당 30건 상한이라 몇 회차에 걸쳐 메워진다).

---

### Phase 83 — 실적 탭 선택 UI 개편 + 「내 배당」 DART 보완 + 시가배당률 정규식 (2026-07-30, 구현 완료)

- **요청 근거**: 사용자 — ① "실적 탭에서 칩 링크 대신 보유 탭·관심 탭을 일렬로 두고, 두 탭 오른쪽에 드롭다운으로 종목을 고르게. 선택한 종목의 실적 공시만 조회되게" (타당성 회신 후 **B안 = 목록은 클라이언트 필터** 확정) ② "삼성전자 현물배당 공시가 떴는데 배당 항목의 「내 배당」 탭에 반영이 안 됐다" ③ (②를 확인하다 발견) 시가배당률 정규식이 실서식과 어긋나 있던 것.

#### 83.1 실적 탭 — 칩 2줄 → 세그먼트 + 드롭다운 1줄

Phase 82가 라디오를 물린 근거는 "**종목 15개를 세로로 늘어놓으면 선택 영역이 목록보다 길어진다**"였다. **드롭다운은 접혀 있어 그 근거가 성립하지 않는다** — 오히려 칩 2줄이 한 줄로 줄어 원래 요구(목록을 위로)에 더 맞다. 그래서 이번엔 `<select>`를 쓴다.

- **그룹은 상태를 늘리지 않는다.** `?group=`을 새로 만들지 않고 **선택 종목이 속한 쪽이 곧 활성 세그먼트**다 — 상태가 하나뿐이라 URL과 어긋날 여지가 없고, 세그먼트를 누르면 그 그룹의 첫 종목으로 옮겨 간다. 고를 종목이 없는 그룹의 세그먼트는 `disabled`.
- **선택 상태가 클라이언트로 내려갔다(B안의 실체).** `?code=`가 여전히 정본이지만, `FeedTabsClient`가 `pickedCode`를 함께 들고 있어 드롭다운을 바꾸는 즉시 **아래 실적 목록이 걸러진다**. 서버 왕복(`router.push`)은 상단 블록(DART 조회 + `<Suspense>`)만을 위한 것이다. 뒤로가기 등으로 URL이 바뀌면 렌더 중 상태 조정으로 URL을 따른다.
- **목록 필터가 서버가 아니라 클라이언트인 대가**: 40건 컷이 전 종목 통합 기준으로 남는다(A안이었다면 종목당 40건이라 이력이 더 길었다). 대신 종목 전환이 즉각적이다 — 사용자가 이 트레이드오프를 확인하고 B안을 골랐다.
- **알림 링크를 같이 고쳐야 했다.** `feedAlerts.ts`의 실적 알림 URL이 `/feeds?tab=earnings`(종목 없음)였는데, 목록이 한 종목만 보여주게 된 이상 **알림을 눌러도 엉뚱한 첫 종목이 뜬다.** 같은 루프에 `event.symbolCode`가 있어 `&code=`를 붙이는 한 줄로 끝났다 — 이걸 놓쳤으면 알림이 조용히 무용지물이 될 뻔했다.

#### 83.2 「내 배당」 — 소스가 달라서 생긴 공백을 DART로 메운다

먼저 사실관계부터. 사용자가 말한 「현물배당 공시」(`20260730800137`)를 열어보니 **내용은 현물배당이 아니다** — 「현금ㆍ현물배당결정」은 서식 이름이고, `2. 배당종류 현금배당` / `현물자산의 상세내역 -` / **주당 374원 · 기준일 2026-06-30 · 지급 2026-08-28**인 평범한 분기 현금배당이었다.

**원인은 두 화면이 서로 다른 소스를 본다는 것이었다.**

| 화면 | 소스 | 그때 상태 |
|---|---|---|
| 공시 탭 | DART | 오늘자 배당결정 **이미 있음** |
| 내 배당 | KIS 예탁원 배당일정 | 같은 회차를 `per_sto_divi_amt=0`으로 줌 |

KIS를 직접 불러 확인한 실응답:

```
20260630 | 분기 | perShare= 0    | pay=(빈칸)   ← 오늘 공시된 그 회차
20260331 | 분기 | perShare= 372  | pay=2026/05/29
```

`buildDividendBlock`이 `per_sto_divi_amt > 0`(=확정분)만 남기므로 이 회차는 저장 단계에서 탈락한다(Redis 실조회로 `rounds`가 2026-03-31까지인 것 확인). **코드는 의도대로 동작했고 버그가 아니라 데이터 소스 시차다** — 이사회 결의가 그날이라 예탁원이 아직 못 따라온 것이고, 며칠 뒤면 저절로 채워진다.

그래도 방치하면 **지급일 알림(`dividendAlerts`)과 홈 배당 카드가 같은 `rounds`를 읽어** 8/28 지급일까지 예탁원 반영 속도에 묶인다. 그래서 보완한다.

- **소스 우선순위**: 예탁원이 본 소스, **예탁원에 아직 없는 기준일만** 공시로 메운다. 예탁원이 나중에 채우면 그쪽이 이긴다(정정도 그쪽이 반영). 겹침·정정본·무효값 3케이스를 합성 데이터로 확인했다.
- **DART 리스트 콜은 0건 늘었다.** 「현금ㆍ현물배당결정」은 **거래소공시(`pblntf_ty=I`)** 라 Phase 81이 실적용으로 이미 부르는 목록에 들어 있다(실측 확인). 그래서 `refreshEarnings`가 받아 둔 `rows`를 그대로 훑는다. 새로 드는 건 **새 배당결정 공시의 원문(zip) 1건**뿐이고, 파서 버전(`parsedV`)으로 굳혀 재사용한다.
- **원문 예산은 실적과 분리**(`DIVIDEND_DOC_BUDGET=8`) — 실적 시즌에 30건 예산을 다 쓰면 배당이 계속 밀린다. 종목당 90일 배당결정은 0~2건이라(실측 삼성전자·LG전자·현대차 각 1건) 정상 회차엔 0~1건만 쓴다.
- **저장은 별도 키**(`market:dividendDecision:{code}`). `market:stockInfo`에 끼워 넣으면 시세 잡의 SET 덮어쓰기에 매번 지워지고, `market:earnings`에 넣으면 실적 탭에 배당 공시가 섞인다. 고아 정리 잡 대상 키는 9종 → 10종.
- **한계 하나는 남는다**: 배당결정 공시는 보통주 법인 명의로 접수되고 **우선주(005935 등)는 DART 고유번호가 없어** 수집 대상에서 빠진다 — 우선주분은 예탁원이 채울 때까지 기다린다. 화면 각주에 명시했다.

#### 83.3 시가배당률 정규식 — 표본 8/8 실패하던 조용한 버그

Phase 44의 `/시가배당율?\(%\)/`는 "시가배당(%)" 또는 "시가배당**율**(%)"만 받는데, **실서식은 「시가배당률(%)」**(률)이다. 2026-07-15~30 배당결정 8건 전건에서 매치 실패했고, 주당배당금·기준일은 정상이라 **배당 순위표 폭배 툴팁의 「공식 시가배당률」만 조용히 비어 있었다.** `[율률]`로 고치니 8/8 정상(삼성전자 0.1·LG유플러스 1.8·HD현대 0.61 …).

#### 83.4 구현

- `lib/api/dart/client.ts` — `DartDividendDetail`(배당구분·지급예정일 추가) + `isDividendDecisionReport`·`parseDartDividendDetail`·`fetchDartDividendDetail` 분리 신설, `fetchDartDividendDecision`은 이들을 재사용(폭배 경로 동작 불변). 배당구분은 "분기배당"→"분기"로 정규화해 **예탁원 `divi_kind`와 어휘를 맞춘다**(병합 후 화면에서 섞여 보이지 않게).
- `lib/feeds/store.ts` — `DividendDecisionItem`·`StoredDividendDecisions`·`dividendDecisionsKey`·`getDividendDecisionSnapshots`·`setDividendDecisions`.
- **`lib/dividends/rounds.ts`(신규)** — `mergeDividendRounds`(순수) + `getDividendRoundsMap`(두 MGET 병렬, 공시 읽기 실패는 격리). 읽기 경로가 여기 하나로 모여 화면·알림이 규칙을 나눠 갖지 않는다.
- `lib/jobs/refreshFeeds.ts` — `collectDividendDecisions` 신설 + `refreshEarnings`가 같은 `rows`로 호출. 리포트에 `dividendDecisions` 추가하되 **`ok` 게이팅에서는 제외**(부가 소스 실패로 피드 파이프라인을 재실행시키지 않는다 — `tradeStats`와 같은 방침).
- `lib/dividends/summary.ts`·`lib/alerts/dividendAlerts.ts` — 회차 조회를 병합 리더로 교체(행에 `source` 추가).
- `lib/jobs/cleanupOrphanStocks.ts` — 새 키 수거 대상 등록.
- `app/dividends/page.tsx`(+CSS) — 공시로 메운 회차에 「공시」 배지(`title`로 사유) + 각주 1건 추가(출처·우선주 미적용).
- **`components/feeds/EarningsStockPicker.tsx`(+CSS, 신규 Client)** — 세그먼트 2 + 드롭다운.
- `components/feeds/EarningsFocusPanel.tsx`(+CSS) — 칩 제거, props 축소(`options` → `hasOptions`).
- `components/feeds/FeedTabsClient.tsx` — `pickedCode` 상태·`selectStock`·실적 목록 필터, `EarningsBoard`에 `stockName`(빈 안내 문구용).
- `app/feeds/page.tsx` — 선택지·선택코드를 `FeedTabsClient`에도 전달.
- `lib/alerts/feedAlerts.ts` — 실적 알림 URL에 `&code=`.

- **범위**: 새 Redis 키 **1종**(`market:dividendDecision:{code}`) · 잡 **6종·스케줄 불변** · **KIS 호출 0건 증가** · DART 리스트 콜 0건 증가(원문만 회차당 최대 +8) · 알림 판정 규칙 불변(대상이 병합 회차로 넓어질 뿐).
- **검증**: tsc·eslint·`npm run build` 통과. 실 DART/KIS/Redis 검증 — ① 배당결정 파싱+병합을 3종목(삼성전자·LG전자·현대차)에 실행해 **예탁원 회차는 그대로 두고 최신 1건씩만 `[dart]`로 붙는 것** 확인(삼성전자 3→4건, LG전자 2→3건, 현대차 4→5건) ② 겹침(예탁원 우선)·`[기재정정]`(접수번호 큰 쪽)·무효값(주당배당금/기준일 없음 → 버림) 3케이스 통과 ③ 시가배당률 정규식 표본 8/8 복구.
- **상태**: **구현 완료(2026-07-30)**. 배포 후 화면 확인 필요 — **「공시」 배지가 붙은 회차는 갱신 잡이 한 번 돈 뒤**에 나타난다(새 키라 첫 회차에 채워진다).

### Phase 84 — 실적 탭에 발표 안내(공시 원문) + 「실적 관련 보도」 (2026-07-30, 구현 완료)

- **요청 근거**: 사용자 — "잠정실적·확정실적을 발표하면 뉴스나 보도자료가 쏟아져 나오는데, 그 **실적 관련 멘트** 같은 것들을 실적 탭에 내용으로 추가할 수 있는지". 타당성 확인 결과를 보고한 뒤 **A+B 병행 승인**.

#### 84.0 먼저 확인한 것 — "멘트"의 출처는 DART가 아니다

잠정실적 공시 원문에 자유서술 필드가 있긴 하다(「4. 기타 투자판단과 관련한 중요사항」, 실측 4/4 존재). **그런데 내용이 전부 면책 문구다**:

> "한국채택국제회계기준에 따라 작성된 연결기준의 잠정 영업실적입니다. … 외부감사인의 회계감사가 완료되지 않은 상태에서 투자자들의 편의를 위해 제공되는 정보로서…"

해설도 전망도 없다. Phase 82가 `dart/client.ts`에 적어 둔 판단(**컨퍼런스콜 발언·Q&A는 DART에 없다**)이 그대로 유효하다. 그래서 **A(공시에서 더 뽑을 수 있는 정형 항목)와 B(뉴스 요약문)를 갈랐다** — 회사가 뭐라고 말했는지는 B에서만 나온다.

#### 84.1 A — 이미 받고 있는 원문에서 더 뽑는다 (추가 호출 0)

파서 v2는 실적표와 IR 서식만 봤는데, 같은 원문에 **안 보고 버리던 정형 항목**이 셋 더 있다. 원문(zip)은 이미 받고 있으므로 **DART 콜이 1건도 늘지 않는다**.

| 항목 | 실측 예 | 표본 25건 파싱률 |
|---|---|---|
| 「2. 정보제공내역」 일자·시간·행사명 | `2026년 7월 30일 공정공시 이후 / 2026년 2분기 경영실적설명회 (Conference Call)` | **서식 25/25**, 값이 있는 건 **5/25** |
| 「4. 기타…」 안의 IR 홈페이지 URL | `https://www.lgensol.com/kr/index` | 4/25 |
| 「3. 정정사유」(정정 공시) | `2026년 2분기 연결재무제표 기준 영업(잠정)실적(공정공시) 내용 정정` | 2/2 |

- **서식은 완전 정형인데 값이 비어 있는 게 다수다**(20%만 채움). 중소형주는 「-」나 "공정공시 후 수시제공"으로 낸다. 그래서 **파싱은 전건 하되 화면은 값이 의미 있을 때만 렌더**한다 — `-`·빈칸을 그대로 내보내면 25건 중 20건에 빈 줄이 생긴다.
- **날짜를 정규화하지 않는다.** 실측에 `2026년 7월 30일(목)`·`2026-07-30`·`공정공시 후 수시제공`·`공정공시 이후`가 섞여 있다. 자유서술 칸이라 파싱해서 재조립하려 들면 오히려 값을 망가뜨린다 — **원문 문자열 그대로** 보여준다.
- 일자 칸과 시간 칸에 **같은 문구가 중복**돼 오는 회사가 있다(콜마비앤에이치·한컴: 둘 다 "공정공시 후 수시제공"). 같으면 한 번만 표시한다.
- 이 값들의 쓸모: **IR 개최 공시를 따로 내지 않는 회사도** 잠정실적 공시 안에 컨퍼런스콜 일정을 적는다 — Phase 82의 IR 일정 카드가 비는 종목을 메운다.

#### 84.2 B — 실적 뉴스는 **요약문(description)**이 핵심인데 그걸 버리고 있었다

실측(2026-07-30, 마침 삼성전자 발표일)에서 네이버 뉴스 API `description`에 정확히 요청받은 멘트가 들어 있었다:

> "김재준 삼성전자 메모리사업부 전략마케팅실장(부사장)은 30일 2분기 실적발표회에서 '주요 5대 데이터센터 고객사와 이미 LTA를 완료했다'며…"

그런데 `naver/client.ts`가 `title`·`link`·`pubDate`만 담고 **`description`을 버린다.** 뉴스 탭이 제목만 쓰기 때문인데, 실적 보도에서는 그 한 문단이 본체다.

- **쿼리는 `"{종목명}" "영업이익"` + `sort=sim`.** 후보 6종을 실측 비교했다(제목에 실적 신호가 있는 비율):

  | 쿼리 | sort=date | sort=sim |
  |---|---|---|
  | `종목명` (뉴스 탭 현행) | 7/20 | — |
  | `종목명 실적` | 5/15 | 11/15 |
  | **`"종목명" "영업이익"`** | 9/15 | **15/15** |
  ...
  `sort=date`는 발표 직후 시황·정치 기사가 앞을 채운다(실측: "정부, 레버리지 ETF 규제", "충청권 392조 투자"). 실적 보도는 **최신순이 아니라 관련도순**으로 집어야 한다.
- **⚠️ 네이버는 쿼리에 종목명이 있어도 다른 종목 기사를 준다.** `현대차 2분기 실적`에 LG엔솔 기사가 3건 올라왔다. 현행 필터(제목+**요약** 포함)로는 안 걸러진다 → **제목 우선 점수제**로 정렬한다(제목에 종목명 +2, 제목에 실적어 +1, 요약에만 있으면 0점대). 0점은 버리고 점수순 → 최신순으로 상위 5건.
  - 제목 매칭을 **필수 조건으로 하지 않은 이유**: 실측에서 "삼전 2분기 영업익 89조"·"하이닉스 2분기 영업이익 60.5조"처럼 **축약형 제목**이 흔하다. 필수로 걸면 정작 핵심 기사가 탈락한다.
- **평상시 API 콜은 0이다.** 전 종목을 매 회차 조회하지 않고 **최근 `EARNINGS_NEWS_WINDOW_DAYS`(7일) 안에 실적 공시(잠정실적·정기보고서·실적변동)가 접수된 종목만** 부른다. 발표 시즌에만 종목당 1콜이 붙는다(일 15회차 × 소수 종목).
- **저작권 선**: 네이버가 주는 발췌(2~3줄) + **원문 링크**까지만 저장한다. 본문 스크랩은 하지 않는다(Phase 82에서 같은 이유로 접었던 선을 그대로 지킨다). 화면에 출처·발행일을 함께 단다.
- **오래된 보도는 화면에서 숨긴다.** 스냅샷은 다음 분기 발표 때 덮어써지므로 그 사이 3개월 묵은 기사가 남을 수 있다 — 기준 공시 접수일(`basisRceptDt`)을 함께 굳혀 두고 `EARNINGS_NEWS_MAX_AGE_DAYS`(45일)를 넘으면 렌더하지 않는다.

#### 84.3 구현

- `lib/api/dart/client.ts` — `DartEarningsBriefing` 신설, `parseDartEarningsDetail`이 `briefing`·`correctionReason`·`irUrl`을 함께 반환. URL은 `http(s)`만 통과(네이버 링크와 같은 방침 — 화면에서 `<a href>`로 나간다).
- `lib/feeds/earnings.ts` — `EARNINGS_PARSER_VERSION` 2 → **3**(저장분이 다음 회차부터 새 필드를 채우러 재파싱된다) + `isEarningsNewsTarget`(뉴스를 부를 유형 판정 — 알림 3종과 같은 기준).
- `lib/feeds/store.ts` — `EarningsBriefing`·`EarningsItem`에 3필드 추가 / **`EarningsNewsItem`·`StoredEarningsNews`·`earningsNewsKey`·`getEarningsNews`·`setEarningsNews`**.
- `lib/api/naver/client.ts` — `summary`(발췌) 반환 + `sort` 옵션. 뉴스 탭 저장분은 **무변경**(요약을 저장하지 않는다 — 값 크기가 10건×2~3줄만큼 늘 이유가 없다).
- **`lib/feeds/earningsNews.ts`(신규)** — 쿼리 조립·점수 필터·나이 컷이 전부 여기 순수 함수로 모인다(잡·화면이 규칙을 나눠 갖지 않게).
- `lib/jobs/refreshFeeds.ts` — 스텝 6 `refreshEarningsNews`. **`ok` 게이팅에서 제외**(부가 소스 — `tradeStats`·`dividendDecisions`와 같은 방침).
- `lib/feeds/earningsFocus.ts` — `EarningsFocus`에 `news`·`briefings` 추가. 확정 재무를 못 구한 경로에도 싣는다(IR 일정과 같은 취급).
- `lib/feeds/homeFeed.ts` — `EarningsBoardItem`에 새 3필드 전달.
- `components/feeds/EarningsFocusPanel.tsx`(+CSS) — 「실적 발표 안내」(A) + 「실적 관련 보도」(B) 섹션.
- `components/feeds/FeedTabsClient.tsx` — 아코디언에 발표 일시·행사명·정정사유.
- `lib/jobs/cleanupOrphanStocks.ts` — 새 키 수거 대상(10종 → 11종).

- **범위**: 새 Redis 키 **1종**(`market:earningsNews:{code}`) · 잡 **6종·스케줄 불변** · **KIS 호출 0건 증가** · **DART 콜 0건 증가**(A는 이미 받는 원문에서 뽑는다) · 네이버 콜은 **발표 후 7일 안의 종목만** 회차당 1콜 · 알림 판정 규칙 불변.
- **검증**: tsc·eslint·`npm run build` 통과. 실 API 검증 — ① 잠정실적 원문 표본 **25건**에서 정보제공내역 서식 25/25 파싱(값 있는 건 5건)·IR URL 4건·정정사유 2/2 ② 네이버 쿼리 후보 6종 정밀도 비교로 `"종목명" "영업이익"`+`sim` 채택 ③ 점수 필터를 실응답에 적용해 타 종목 기사 배제 확인.
- **상태**: **구현 완료(2026-07-30)**. 배포 후 확인 필요 — **A의 새 필드는 파서 v3 재파싱이 끝난 뒤**(회차당 30건 상한) 채워지고, **B는 다음 실적 공시가 접수된 종목부터** 뜬다(발표 시즌이 아니면 비어 있는 것이 정상).

### Phase 85 — 홈 원/달러 카드에 달러 인덱스 보조 한 줄 (2026-07-30, 구현 완료)

- **요청 근거**: 사용자 — 홈 원/달러 환율 카드의 요약에 **달러 인덱스도 함께** 보고 싶다. 단 **원/달러 수치가 메인**이고 달러 인덱스는 그 표시를 방해하지 않게 작게. 표시 방식 추천 요청.
- **조사·판단**:
  - **데이터는 이미 있다.** `refreshDxy`(§28)가 매 회차 환율 6종을 합성해 `market:detail:dxy`에 저장하고, 원/달러 **상세 하단**에 `DollarIndexSection`으로 이미 표시 중이다. **KIS 콜 0 증가**, 잡·스케줄 무변경. 홈에서 빠져 있던 이유는 `getDashboardData()`가 조회 키 목록에서 dxy를 뺀 것뿐 — `getMarketDetails`는 MGET 1회라 **Redis 왕복도 안 늘어난다**(키 1개 추가).
  - **표시 슬롯도 이미 있다.** §71에서 변동성 카드용으로 만든 `SummaryCard`의 `note`(대표값·등락 아래 보조 한 줄, `--text-caption-sm`·tertiary·색상 없음)가 "메인을 방해하지 않는 작은 한 줄"이라는 요구와 정확히 같은 용도다 → **컴포넌트·CSS 변경 0**.
  - 검토했지만 뺀 대안 2종: ① **대표값 줄 오른쪽 인라인 병기** — 400px 미만 1열 구간에서 메인 수치와 폭을 다투고 `SummaryCard` 구조를 고쳐야 한다. ② **`MarketCard`식 2행 리스트 전환** — 원/달러가 대표값 위계를 잃어 "원/달러를 메인으로"라는 요구와 정반대.
- **정책(사용자 확정)**: 추천안(`note` 한 줄) 승인.
- **구현**:
  - `types/indices.ts` — `IndexDashboardData`에 `dxy: IndexSnapshot | null`(파생 지표·실패 격리라 nullable).
  - `lib/indices/getDashboard.ts` — `OPTIONAL_KEYS`에 `"dxy"` 추가. **`asOf` 계산에는 넣지 않는다** — dxy는 실패해도 잡 `ok`에 영향 없는 파생 지표라(§28) 실패가 이어지면 낡은 `fetchedAt`이 남는데, 그걸 asOf 후보에 넣으면 **화면 전체의 「마지막 갱신」을 부수 지표 하나가 끌어내린다**. `fetchedAtByKey`(배지 판정)도 기존 `Exclude<…, "dxy" | …>` 유지 — §30에서 확립한 "잡 ok 게이팅 밖 지표는 배지 판정 제외" 관례를 그대로 따른다.
  - `components/indices/IndexDashboard.tsx` — `dollarIndexNote(dxy, usdKrwBasDt)` 헬퍼 신설. 문구는 `달러인덱스 98.42 (+0.21%)`, dxy가 null이면 **줄 자체가 없다**. `indexSummaryProps` 시그니처는 그대로 두고 스프레드 뒤에 `note`만 덧붙여 코스피·코스닥 카드에 영향이 없게 한다.
- **표기 결정 근거**:
  - **기준일이 어긋날 수 있다** — dxy는 통화쌍 6종의 기준일 **교집합**에서만 계산하므로(`dxy.ts`) 원/달러보다 기준일이 하루 밀릴 수 있다. 요약 카드는 `basDt`를 보여주지 않아 사용자가 두 값을 같은 날짜로 읽는다 → ~~**두 `basDt`가 다를 때만** 뒤에 `· 07/29 기준`을 붙인다~~ (**§85.1에서 제거** — 사용자가 화면 확인 후 기준일 표기를 빼기로 확정. 시차 자체는 그대로 존재한다).
  - **등락 색은 쓰지 않는다.** `note`는 무채색이 §71에서 정한 규칙이고, 여기서 예외를 만들면 원/달러 등락(색 있음)과 달러 인덱스 등락(색 있음)이 한 카드에서 경합해 "메인을 방해하지 않는다"는 목적 자체가 깨진다. 방향은 `formatChangeRate`의 부호로 드러낸다.
  - ~~**지표명은 「달러인덱스」(축약 아님).** 홈 `MarketCard`는 4행 폭 압박 때문에 축약(美 금리·WTI·GOLD·BTC)을 쓰지만(§34), 여기는 caption-sm 한 줄이라 폭 여유가 있고 `DXY`보다 한글 표기가 바로 읽힌다.~~ → **§85.1에서 `DXY`로 교체**(사용자 요청: 공식 명칭 영문 표기).
- **동선**: 카드를 누르면 가는 `/indices/usdkrw` **하단에 달러 인덱스 상세(카드+차트+근사치 각주)가 이미 있다**(§28) — 요약 한 줄에서 상세로 자연히 이어지므로 새 라우트가 필요 없다.
- **범위**: 새 Redis 키 0 · 잡 6종·스케줄 불변 · **KIS 콜 0 증가** · Redis 왕복 0 증가(MGET 키 1개) · CSS 변경 0.
- **검증**: tsc·eslint·`npm run build` 통과. **라이브 Redis 실측(2026-07-30 13:40 KST 회차)** — `usdkrw` `20260730 / 1,439.00 / −0.52%`, `dxy` `20260729 / 101.45 / +0.02%` → 카드 렌더 `원/달러 환율 · 1,439.00 · -7.50 (-0.52%) · 달러인덱스 101.45 (+0.02%) · 07/29 기준`.
- **실측이 바꾼 판단 — 기준일 병기는 예외가 아니라 장중 상시다**: 계획 단계에서는 기준일 어긋남을 "밀릴 수 있다"로 봤지만, 실측에서 **바로 어긋났다**(원/달러는 당일 행이 있는데 dxy는 전일). 통화쌍 6종 교집합 구조상 **장중에는 당일 dxy가 없는 것이 정상**이라, 날짜 꼬리표는 사실상 장중 내내 붙는다. 그래도 조건부 표기를 유지한다 — 이번 실측처럼 **원/달러는 하락(−0.52%)인데 달러 인덱스는 상승(+0.02%)으로 보이는** 상황이 시차 때문에 생기고, 날짜가 없으면 데이터 오류로 읽힌다. 마감 후 회차에는 두 기준일이 맞아 꼬리표가 사라진다.
- **폭 검토(문구를 축약하지 않은 이유)**: 12px(`--text-caption-sm`) 기준 이 문구는 좁은 2열(400~430px 화면, 카드 내부 ~170px)에서 두 줄이 될 수 있다. 그래도 「DXY」 축약(§34 홈 축약 관례)을 쓰지 않았다 — **2열에서 원/달러 카드의 짝은 4행짜리 `MarketCard`라 항상 더 높고**, 그리드 행 높이는 그쪽이 정하므로 보조줄이 한 줄 늘어도 **레이아웃에 영향이 0**이다. 폭을 아끼려 매번 읽을 때 인지 비용이 드는 약어로 바꿀 이유가 없다.
- **상태**: **구현 완료(2026-07-30)**. 문구는 아래 §85.1에서 조정됨.

#### 85.1 화면 확인 후 문구 조정 — 「달러인덱스」 → `DXY`, 기준일 제거 (2026-07-30, 구현 완료)

- **요청 근거**: 사용자 — 배포 화면 확인 후 "달러 인덱스 말고 **공식 명칭 영어로** 바꾸고 **기준일 빼줘**".
- **표기**: `DXY` 채택 — ICE 선물 티커이며 정식 명칭은 U.S. Dollar Index(USDX). 홈 카드 축약 관례(§34 美 금리·WTI·GOLD·BTC)와도 결이 맞고, 풀네임은 카드를 눌러 들어간 상세 하단 `DollarIndexSection`(「달러 인덱스」)에서 확인된다. 결과 문구 `DXY 101.45 (+0.02%)` — 한글 5자가 빠져 좁은 2열에서도 한 줄에 들어간다.
- **기준일 제거의 트레이드오프(사용자에게 고지 후 확정)**: §85가 날짜를 붙인 이유가 사라지는 게 아니라 **표시하지 않기로 한 것**이다. 장중에는 원/달러가 당일·dxy가 전일이 정상이라(6종 교집합, §28) **원/달러 하락과 DXY 상승이 동시에 보일 수 있고**, 이제 화면에 그 이유를 알리는 단서가 없다. 사용자가 배포 화면을 직접 보고 제거를 요청했으므로 그 결정을 따르되, 시차 사실은 `dollarIndexNote` 주석과 research.md에 남겨 다음 작업자가 이를 버그로 오해하지 않게 했다.
- **구현**: `dollarIndexNote(dxy)` — 인자에서 `usdKrwBasDt`가 빠지고 `formatBasDtLabel` import도 제거(비교·병기 로직 삭제). 나머지(nullable 처리·`note` 슬롯·asOf/배지 제외)는 §85 그대로.
- **검증**: tsc·eslint·`npm run build` 통과. 라이브 Redis 실측(2026-07-30) 렌더 — `원/달러 환율 · 1,439.00 · -7.50 (-0.52%) · DXY 101.45 (+0.02%)`.
- **상태**: **구현 완료(2026-07-30)**. 배포 후 확인 필요 — 홈 원/달러 카드에 `DXY …` 보조줄이 뜨는지(기준일 없음).

---

### Phase 86 — 홈 코스피/코스닥 카드에 거래대금·수급 보조 블록 (개·외·기 3열 + 연속 거래일) (2026-07-30, 구현 완료)

- **요청 근거**: 사용자 — "코스피/코스닥 카드 요약에 거래대금·투자자별 순매수금액을 표시하기로 했는데 안 보인다"는 지적에서 출발. 확인 결과 **§69에서 홈은 명시적으로 제외**(「홈 요약 카드는 폭이 좁아 제외」)했고 상세 화면에만 있었다. 사용자가 홈에도 얹기로 확정하면서 ① **코스피/코스닥을 한 카드로 합쳐 한 줄에 담을 수 있는지** 확인 요청 → 확인 후 **통합은 취소**(기존 반폭 2카드 유지), ② 라벨은 「개·외·기」 1자 축약, ③ 두 번째 줄에 **(전일대비 대금, 연속 거래일 수)**.
- **선행 확인 — §70은 코드가 아니라 데이터 대기였다**: 사용자는 "실데이터가 확보 안 돼 보류된 것"으로 기억했는데, `pending-commits.md`의 [확인] Phase 70 항목(2026-07-29 경과: "아직 전일 슬롯이 안 쌓여 미반영")이 그것이다. Phase 70은 커밋 `81430b5`로 구현이 끝나 있었고, 이번에 baseline 42슬롯이 실제로 쌓여 **코드 변경 없이** 「전일 14:00 대비」로 동작함을 실측 확인했다(사용자 화면 확인 완료).
- **폭 실측 (Geist 숫자·라틴 + Apple SD Gothic Neo 한글 메트릭으로 문자열 폭 계산 — Geist에는 한글 글리프가 없어 시스템 폴백)**:
  - **가장 좁은 지점은 360px이 아니라 430px 뷰포트다.** 홈 그리드가 400px에서 2열로 바뀌며 카드 내부가 302px → **169px**로 급감한다(360/390px은 1열이라 302/332px). 3열이면 **열당 53.7px**.
  - **카드 통합안 검토 후 취소**: 전폭(`grid-column: 1 / -1`)이면 카드 내부 422px@480px이지만, 두 지수를 좌우 2열로 나누면 열당 208px로 현행 194px과 사실상 같아(§67과 동일 계산) 이득이 14px뿐이다. 실이득은 **지수당 1행씩 전폭을 가로로 쓰는** 표 형태뿐인데, 한 줄에 지수·등락률·거래대금·개·외·기를 전부 넣으면 라벨을 헤더로 올리고 11px로 줄여야 360px에서 겨우 통과(309px→283px)하고 **대표 지수값의 28px 큼직함을 잃는다**. 사용자가 통합 취소를 선택.
  - **채택 배치**: 반폭 카드 유지 + 3열 그리드(상단 순매수 금액, 하단 전일 대비 증감 + 연속 거래일 수). 11px(`--text-micro`).
  - **줄바꿈은 설계로 수용**: 채택 문구를 다시 재보니 430px에서 실데이터 대부분이 열폭을 넘는다(`+3624억 2일` 61.6px · `외 +7335억` 57.1px vs 53.7px). 억 4자리는 6글자로 45px이 필요해 라벨까지 담으면 물리적으로 여유가 없다. 폰트를 10px로 더 줄이거나(토큰에 없는 크기) 억 표기를 조로 뭉개는(`7335억`→`0.7조`, 정보 손실) 대신 **접히게 두는 쪽을 택했다** — 증감·연속·값을 각각 `nowrap` 조각으로 감싸 **숫자 중간이 쪼개지지 않고 조각 경계에서만** 두 줄이 된다. 390px 이하(열당 98~108px)에서는 전부 한 줄이다.
- **정책(사용자 확정)**: 배치는 3열 상하 정렬(위아래가 같은 주체), 금액은 **억/조 자동 전환 + 쉼표 생략**.
  - **조 단위로 통일하지 않은 이유**: 코스닥 수급은 수백억대라 조로 쓰면 「−0.0조」가 되어 정보가 사라진다(실측 코스닥 개인 −515억). 1조 이상만 조 소수 1자리, 미만은 억원 정수.
  - **색상은 순매수 금액에만.** 거래대금 줄과 하단 증감·연속은 무채색 tertiary — 카드의 대표 색상은 지수 등락률이어야 한다(§85 DXY 보조줄 관례). 다만 3열 값은 순매수/순매도 부호가 정보의 핵심이라 예외로 색을 준다.
- **구현**:
  - `lib/format/krw.ts` — `formatFlowCompact(millionWon, signed)` 신설. 최상위 단위 1개·쉼표 없음. 기존 `formatEokAxis`(차트 축)는 억 단위에도 소수를 붙이고 부호 옵션이 없어 재사용 불가. **억 반올림이 1조 경계를 넘으면(9,999.5억↑) 조로 승격**해 `10000억` 같은 5자리를 막는다.
  - `lib/indices/marketFlow.ts` 신설 — `pickBaselineSlot`을 `MarketFlowSummary`에서 **이동**(상세·홈이 같은 비교 기준을 쓰도록 공용화), `computeFlowStreak`(같은 부호 연속 거래일, 20거래일 창 소진 시 `capped`), `buildHomeIndexFlow`(상세 §69·§70과 같은 스냅샷·기준으로 홈용 요약 조립).
  - `types/indices.ts` — `HomeIndexFlow`·`HomeIndexFlowInvestor` 신설, `IndexDashboardData`에 `kospiFlow`·`kosdaqFlow`(nullable).
  - `lib/indices/getDashboard.ts` — detail MGET과 **병렬로** 수급 4키(`market:investor:*` 2 + `market:investorIntraday:*:baseline` 2)를 읽는다. `asOf`는 **각 지수의 자기 `fetchedAt`**을 쓴다 — 화면 전체 asOf(가장 오래된 시각)를 쓰면 다른 지표가 지연될 때 엉뚱한 슬롯과 비교한다.
  - `components/indices/IndexFlowNote.tsx`(+`.module.css`, 신규) — 서버 컴포넌트. 거래대금 한 줄 + 3열. 1자 라벨의 원래 이름(개인·외국인·기관계)과 전일 대비 기준·연속 의미는 `.srOnly`로 남긴다(MyStocksCard와 같은 패턴).
  - `components/indices/SummaryCard.tsx` — `flow?: ReactNode` 슬롯 추가(문자열 한 줄인 `note`로는 그리드를 담을 수 없다). 위 여백은 앞 요소(`.change`의 margin-bottom 8px)가 그대로 갖는다.
  - `components/indices/IndexDashboard.tsx` — `indexSummaryProps`에 `flow` 선택 인자. 원/달러·변동성 등 다른 `SummaryCard` 호출부는 무변경.
- **연속 거래일 수의 한계(코드 주석·이 문서에 기록)**: ① **당일 행부터 세므로 장중에는 확정값이 아니다** — 당일 누적 부호가 마감까지 뒤집히면 연속일수도 바뀐다(상세 3열이 당일 값을 쓰는 것과 같은 성질). ② 저장 창이 **20거래일**이라 그보다 긴 연속은 `20일+`로 캡된다. ③ 당일 값이 정확히 0이면 순매수도 순매도도 아니라 표기를 생략한다.
- **범위**: 새 Redis 키 0 · 잡 6종·스케줄 불변 · **KIS 콜 0 증가**(저장된 스냅샷만) · Redis 왕복 순증 4키(detail MGET과 병렬).
- **검증**: tsc·eslint·`npm run build` 통과. **라이브 Redis 실측(2026-07-30 14:30 회차)** — 실제 모듈(`buildHomeIndexFlow`+`formatFlowCompact`)을 실데이터로 실행해 렌더 문자열 확인: 코스피 `거래대금 31.5조 (-7.9조)` / `개 -1.5조·+3624억 2일` `외 +7335억·+1.7조 1일` `기 +7434억·-2.1조 4일`, 코스닥 `거래대금 3.9조 (-1.6조)` / `개 -515억·+3127억 2일` `외 +1454억·-1632억 3일` `기 -974억·-1522억 1일`. 기준 라벨 「전일 14:30」로 §70 슬롯 정렬 확인. 포맷터 경계값(9,999.49억→`9999억` / 9,999.5억→`1조` / 30.0조→`30조` / 0→`0억`)과 `computeFlowStreak` 엣지(전부 동일 부호→capped, 당일 0→null, 중간 0으로 끊김, 부호 전환, 빈 배열) 검증.
- **상태**: **구현 완료(2026-07-30)**. 배치는 아래 §86.1에서 바뀜(실기기 확인 결과).

#### 86.1 실기기(아이폰 17) 접힘 해소 — 라벨을 열 머리로 + 값 10.5px + 「일」→「D」 (2026-07-30, 구현 완료)

- **요청 근거**: 사용자 — 배포 화면 확인 결과 PC는 괜찮은데 **아이폰 17에서 한 열이 네 줄로 접혔다**(`기` / `-1111억` / `-1188억` / `3일`). 요청 2건: ① 개·외·기 라벨은 그대로 두고 **숫자·단위만 0.5pt 축소** ② 「일」을 「D」로.
- **폭 병목 정정 — 최악은 430px이 아니라 400~402px이다**: §86에서 430px(열당 53.7px)을 병목으로 봤지만, 실제 최악은 **2열 전환 바로 위 구간**이다. 홈 그리드가 `min-width: 400px`에서 2열이 되므로 400px에서 카드 내부가 332px → 154px로 급감하고 **열당 48.7px**, 402px(=아이폰 17 논리 폭)에서 **49.0px**이다. 후보 뷰포트에 402px을 넣지 않아 놓쳤다.
- **요청 2건만으로는 안 풀린다(실측)**: 라벨 11px 고정·값만 축소한 조합을 전부 재보니 —

  | 조합 | 최악 상단 | 최악 하단 | 49px 대비 |
  | --- | --- | --- | --- |
  | §86 현행 (11px, 「일」) | 55.3px | 74.0px | 둘 다 접힘 |
  | 값 10.5px + 「일」 | 53.4px | 70.8px | 둘 다 접힘 |
  | 값 10.5px + 「D」 | 53.4px | 69.0px | 둘 다 접힘 |
  | 값 10px + 「D」 + gap 2px | 49.5px | 63.9px | 여전히 접힘 |

  근본 원인은 폰트가 아니라 **한 줄에 두 요소를 넣는 구조**다 — 억 4자리 증감(약 40px) + 연속(약 14px)은 어떤 크기로도 49px에 안 들어간다.
- **정책(사용자 확정)**: **라벨을 열 머리 행으로 올려** 각 칸이 값 하나만 갖게 한다(3열 × 4행: 라벨 / 순매수 / 전일대비 / 연속). 가장 긴 칸이 **40.6px**로 떨어져 400px에서도 8px 여유. 요청 2건(값 10.5px·「D」)도 그대로 반영. 검토 후 버린 대안: **조 단위 통일** — 3줄이 유지되고 실데이터는 들어가지만 코스닥 수급(−515억·+1454억)이 `-0.1조`·`+0.1조`로 뭉개지고 십조대(−12.3조)는 하단이 다시 접힌다(63.2px).
- **구현**:
  - `IndexFlowNote.tsx` — `InvestorCell`이 라벨·값·증감·연속 **4개 형제 span**을 세로로 갖는다(기존 `cellTop`/`cellSub` 2단 구조 폐기). `formatStreak`는 `2D`/`20D+`. **값이 없는 칸은 `—`로 자리를 지킨다** — 비워 두면 그 열만 행이 밀려 열끼리 어긋난다.
  - `IndexFlowNote.module.css` — `.grid`는 그대로 3열이고 각 `.cell`이 4행을 세로로 담는다. `.cell > *`에 `nowrap`(§86의 `.piece` 조각 단위 nowrap을 대체 — 이제 칸마다 값이 하나라 조각을 나눌 필요가 없다). `.label`은 `--text-micro`(11px) 유지, `.cellValue`·`.cellSub`는 **10.5px 직접 값**(토큰에 없는 크기 — 좁은 열 폭을 아끼는 국소 조정이라 라벨과 함께 움직이지 않는다).
- **범위**: 컴포넌트 1개 + CSS 1개. 데이터·타입·리더·잡 전부 불변.
- **검증**: tsc·eslint·`npm run build` 통과. 4행 배치의 모든 칸을 400px·402px 열폭(48.7·49.0px)에 대해 재계산 — 라벨 9.5px · 값 최대 39.9px · 증감 최대 40.6px · 연속 최대 26.6px, **전부 통과**.
- **상태**: **구현 완료(2026-07-30)**. 배치는 아래 §86.2에서 다시 바뀜.

#### 86.2 주체를 행으로 돌린 테두리 없는 표 + 라벨 풀네임 복원 (2026-07-30, 구현 완료)

- **요청 근거**: 사용자 — "개·외·기 부분을 표로 만들어 **안 보이는 테두리**를 활용해 3×2로, **최대한 여백을 줄이면** 될 것 같은데 확인해 달라". 이어 거래대금을 **맨 아래로** 옮기고 하단을 `(증감,연속)` 괄호 표기로 두는 그림 제시.
- **표를 쓰자는 착안이 옳았다**: 그때까지 쓰던 CSS 그리드는 `repeat(3, minmax(0,1fr))`로 **열을 균등 분할**해, 짧은 열(`개 -1.5조` 40px)이 남긴 폭을 긴 열(`외 +7335억` 53px)이 쓰지 못했다. 표(`table-layout: auto`)는 내용에 맞춰 배분한다.
- **다만 3열 × 2행은 여백을 0으로 해도 불가(실측)** — 열 폭 자동 배분이 도움이 안 되는 이유는 **세 열이 모두 긴 값**을 가져 내줄 폭이 없기 때문이다.

  | 조합 | 코스피 | 코스닥 | 최악 | 카드 내부 154px 대비 |
  | --- | --- | --- | --- | --- |
  | 사용자안 `(+3624억,2D)` | 178.4px | 181.6px | 228.0px | −24 ~ −74px |
  | 쉼표 제거 | 179.5px | 183.1px | 229.6px | −25 ~ −76px |
  | 괄호까지 제거 | 168.0px | 165.9px | 212.3px | −14 ~ −58px |
  | 값 9.5px + 괄호 유지 | 163.4px | 165.1px | 207.1px | −9 ~ −53px |
  | 하단을 증감만(연속 삭제) | 148.8px | 146.3px | 156.3px | 최악 −2.3px |

  **상단 행만으로 이미 넘친다** — `개 -1.5조`(57) + `외 +7335억`(52) + `기 -1111억`(51) = 160px > 154px. 하단이 무엇이든, 괄호를 없애든, 폰트를 낮추든 무관하다. 즉 3열을 유지하려면 라벨을 값에서 떼야 하고(§86.1), 라벨을 값 옆에 붙이려면 **주체를 행으로 돌려야 한다**.
- **정책(사용자 확정)**: 주체를 **행**으로 둔 3행 × 4열 표(주체 / 순매수 / 전일대비 / 연속) + **거래대금은 표 아래**. 라벨은 "여유가 있으면 풀네임"이라는 지시에 따라 실측 후 **개인·외국인·기관계**(3자)로 복원 — §86에서 1자로 줄인 것을 되돌렸다.
- **폭 실측(400px 기준, 라벨 11px·값 10.5px·열 간격 3px)**:

  | 라벨 | 코스피 | 코스닥 | 최악 | 최악 여유 |
  | --- | --- | --- | --- | --- |
  | **3자(개인·외국인·기관계)** | 131.8px | 127.2px | 143.8px | **+10.2px** |
  | 2자(개인·외인·기관) | 122.3px | 117.7px | 134.3px | +19.7px |
  | 1자(개·외·기) | 112.8px | 108.2px | 124.8px | +29.2px |

  3자 라벨이 29px(한글 11px ≈ 9.7px/자)이라 예상보다 짧아 풀네임에도 여유가 남았다. 갭을 4px 대신 3px로 두어 최악 여유를 7.2 → 10.2px로 넓혔다.
- **구현**:
  - `lib/indices/marketFlow.ts` — `HOME_INVESTOR_COLUMNS` → `HOME_INVESTOR_ROWS`, 라벨을 풀네임으로.
  - `components/indices/IndexFlowNote.tsx` — `<table>`(테두리·배경 없음) + `<th scope="row">` 주체 라벨. 데이터 표라 시맨틱도 맞다. 열 의미는 `<caption className={srOnly}>`와 셀 안 `.srOnly`로 준다. 거래대금 `<p>`를 표 **뒤로** 이동. `FULL_LABELS` 매핑 삭제(라벨이 곧 풀네임).
  - `IndexFlowNote.module.css` — `.grid`(3열 그리드) 폐기, `.table`/`.label`/`.value`/`.sub`. 셀 간격은 `padding-left: 3px`(첫 열 0)로 주어 표 폭이 **내용 + 간격만** 차지하게 한다. `th`의 기본 굵기·가운데 정렬 해제.
- **줄 수**: §86.1의 4행 + 거래대금 = 5줄 → **3행 + 거래대금 = 4줄**로 한 줄 줄었다.
- **범위**: 컴포넌트 1개 + CSS 1개 + 라벨 상수. 데이터·타입·리더·잡 전부 불변.
- **검증**: tsc·eslint·`npm run build` 통과. 라이브 Redis 실측(2026-07-30 15:30 회차) — 코스피 `개인 -1.5조 / +6339억 / 2D`·`외국인 +8583억 / +2.1조 / 1D`·`기관계 +6007억 / -2.7조 / 4D` + `거래대금 35.4조 (-10.3조)`, 코스닥 `개인 -1006억`·`외국인 +2216억`·`기관계 -1263억` + `거래대금 4.4조 (-2조)`.
- **상태**: **구현 완료(2026-07-30)**. 글자 크기·연속 표기는 아래 §86.3에서 되돌림.

#### 86.3 글자 크기 기본 복원 + 연속 표기 「D」 → 「일」 (2026-07-30, 구현 완료)

- **요청 근거**: 사용자 — "1pt 줄였지? **다시 기본 사이즈로** 바꾸고 `1D`를 **`1거래일`**로". §86.2에서 주체를 행으로 돌려 폭 여유가 생겼으니 §86.1의 두 축약(값 10.5px·라틴 `D`)을 되돌리자는 것.
- **실측 — 둘 중 하나만 가능하다** (라벨 11px·갭 3px 고정, 카드 내부 154px):

  | 조합 | 코스피 | 코스닥 | 최악 | 여유 |
  | --- | --- | --- | --- | --- |
  | A 요청대로 11px + 「거래일」 | 157.7px | 155.5px | 169.8px | **−1.5 ~ −15.8** |
  | **B 11px + 「일」(채택)** | 138.6px | 136.4px | 150.8px | **+3.2 ~ +17.6** |
  | C 10.5px + 「거래일」 | 152.2px | 150.1px | 163.8px | 최악 −9.8 |
  | D 11px + 「거래일」 + 갭 2px | 154.7px | 152.5px | 166.8px | 최악 −12.8 |

  「거래일」은 한글 3자라 연속 열이 16px → 35px으로 **두 배 넘게** 늘어난다. 실데이터로도 코스피가 3.7px 모자라고, 연속일수가 두 자리(`10거래일`)만 돼도 확실히 접힌다. 갭을 0으로 해도 최악은 못 막는다.
- **정책(사용자 확정)**: **B안** — 기본 크기(`--text-micro` 11px) 복원 + 「3일」. 사용자 요청 중 "기본 사이즈"와 "D 제거"를 둘 다 만족한다. 제시했지만 고르지 않은 대안 2종: ① 카드를 전폭으로 돌리면 「거래일」이 들어가지만(내부 342px) 코스피·코스닥이 위아래로 배치돼 홈이 길어진다 ② 표에 열 머리 행(순매수/전일대비/연속)을 추가하면 147px로 들어가지만 한 줄 늘어난다.
- **구현**: `IndexFlowNote.tsx`의 `formatStreak`가 `2일`/`20일+`, `.module.css`의 `.value`·`.sub`가 `10.5px` → `var(--text-micro)`. 라벨·값이 같은 크기가 되어 `.label`은 색(tertiary)으로만 구분된다.
- **여유가 3.2px까지 줄었다**: 이 배치에서 더 늘릴 여지는 없다. 이후 표기를 늘리려면 위 대안 2종 중 하나로 배치를 바꿔야 한다.
- **검증**: tsc·eslint·`npm run build` 통과.
- **상태**: **구현 완료(2026-07-30)**. 배포 후 아이폰 17에서 3행이 각각 한 줄로 유지되는지 재확인 필요.

---

### Phase 87 — 지수 상세 순서 재배치(차트 우선) + 「거래대금 · 수급」 접힘 기본 (2026-07-30, 구현 완료)

- **요청 근거**: 사용자 — 상세 화면에서 **차트가 맨 처음**에 있었으면 좋겠고, 그 위에 있던 「거래대금 · 수급」은 홈 요약과 겹쳐 없앨까 하다가 아까워서 고민 중. 아이디어로 **차트의 거래량/거래대금 토글에서 「거래대금」을 고르면 차트가 통째로 거래대금 정보로 바뀌는 방식**을 제안하고 의견·가능성을 물음.
- **검토 의견(사용자에게 회신한 내용)**:
  - **토글로 차트 전체를 교체하는 안은 권하지 않았다** — ① 지수 선이 함께 사라져 상세 화면의 주 정보인 7거래일 추이를 잃는다(현재 토글은 `Bar`의 dataKey만 바꾼다) ② 거래대금 막대는 **7거래일 시계열**이고 요약 블록은 **오늘 한 시점**이라 서로 대체재가 아니다 ③ 같은 컨트롤이 "막대 종류"와 "화면 종류"를 겸해 예측이 어렵다.
  - **정보 중복 실측**: 「거래대금 · 수급」은 §86 홈 카드(`IndexFlowNote`)의 **부분집합**이다 — 거래대금·3주체 순매수·증감은 양쪽에 있고 **연속 거래일 수는 홈에만** 있다. 상세 블록의 고유 가치는 억원 풀표기 정밀도와 「전일 HH:MM 대비」 라벨 정도이고, 시계열은 「일별 수급」 탭이 이미 커버한다. 즉 없애도 손실은 작다.
  - 대안 3종 제시 — A(토글이 「거래대금」일 때만 차트 **아래에** 요약 노출) · B(삭제) · C(차트만 위로, 요약은 그대로).
- **정책(사용자 확정)**: **A안 승인하되 노출 조건은 토글이 아니라 접기/펼치기**. 즉 차트를 카드 바로 아래로 올리고, 요약은 **차트 아래에서 접힘 기본**으로 둔다. 토글 연동은 넣지 않았다 — 토글과 접힘 두 메커니즘이 같은 블록을 제어하면 상태가 충돌하고(토글로 나타난 걸 접으면? 다시 토글하면 펼쳐지나?) 사용자가 마지막에 지시한 것은 접기/펼치기였다.
- **구현**:
  - `components/indices/IndexDetailScreen.tsx` — 차트 `<section>`을 `MarketFlowSummary` **위로** 이동. 그 외 블록 순서·데이터 흐름 불변.
  - `components/indices/MarketFlowSummary.tsx` — `<section>`+`<h2>` → **`<details>`+`<summary>`**. 네이티브라 JS가 필요 없고 **서버 컴포넌트로 남는다**(`holdings/DailyHistoryList`와 같은 패턴, §29). 접힌 채로도 거래대금 값은 보이도록 `summary`에 금액을 함께 두고, 펼치면 아래 `.tradingRow`가 같은 값에 날짜·시각·증감을 더한다(요약 → 상세 점층).
  - `MarketFlowSummary.module.css` — `.summary`(마커 제거·줄 전체 클릭)·`.summaryValue`·`.chevron`(펼치면 180° 회전)·`.card[open] .summary`(펼쳤을 때만 아래 간격).
- **범위**: 컴포넌트 2개 + CSS 1개. **새 데이터·새 키·KIS 호출 0**, 클라이언트 번들 증가 0(네이티브 `<details>`).
- **검증**: tsc·eslint·`npm run build` 통과.
- **상태**: **구현 완료(2026-07-30)**. 배포 후 확인 필요 — 차트가 먼저 오는지, 접힘 상태에서 거래대금 금액이 보이는지, 펼침이 매끄러운지.

---

### Phase 88 — 글로벌 지표 화면 전면 개편: 6섹션 32종 숫자 표 + 차트 전면 제거 (2026-07-31, 구현 완료)

- **요청 근거**: 사용자 — `/indices/market`을 환율·농산물·에너지·귀금속·비철금속·유가·세계증시 **7섹션 다항목 표**로 리뉴얼 요청(열 수 4·5·6열 지정). 타당성 실측 결과를 `summary.md`(2026-07-30)로 보고했고, 그 §6 「확인이 필요한 결정」 4건에 사용자가 답한 것이 이 Phase의 사양이다.
- **실측 근거(요약, 상세는 summary.md)**:
  - **`S`(금선물) 카테고리는 죽은 피드다** — §15.1이 "output2가 빈다"고만 적었던 그 계열인데 **값 자체가 2023년에 멈춰 있다**(금 1,928.60 vs 정상 4,097.00 · WTI 105.24로 2026-07-11 실측과 한 자리도 안 변함). 마스터의 `E`접두 32종(백금·팔라듐·납·니켈·주석·설탕·대두·소맥…)이 **전부 이 카테고리에만** 있어 요청 항목 다수가 코드는 있는데 못 쓴다. `N`으로 같은 코드를 넣으면 `rt_cd=0`이지만 값이 `0.00`.
  - **해외선물 경로는 거래소 시세 신청이 막혀 있다** — NYMEX `EGW00551`·CBOT `EGW00552`("SUB거래소 신청 계좌가 아닙니다"). ICE만 열려 가스오일·설탕은 `rt_cd=0`이지만 **전일 정산가 한 값**뿐이고(`output2`는 파라미터 6조합 모두 빈 응답) 월물 롤오버 로직도 필요하다.
  - **환율 ISO 코드가 뒤바뀌어 있다** — `FX@INR`이 인도네시아 루피아(18,082) · `FX@IDR`이 인도 루피(95.798). 마스터 한글명은 값과 일치하고 알파벳만 반대다. **상식대로 `IDR`을 쓰면 인도 값이 들어온다.**
  - **기준일이 권역마다 다르다** — 아시아·유럽은 당일, **미국은 전일**(갱신 창이 09:00~18:40이라 구조적). 섹션 헤더에 기준일 하나를 붙일 수 없어 **행마다 기준일 열**을 둔다(§85.1 DXY와 같은 문제).
  - **요청하신 4·5·6열은 이 앱 폭에서 성립하지 않는다** — 480px/360px 기준 타일 폭이 6열 64.0/**44.0px** · 5열 78.4/**54.4px** · 4열 100.0/**70.0px**인데 값 한 줄(`51,594.14`)의 필요 폭이 **58.5px**다. §86.2의 "3열×2행 불가"와 같은 벽.
- **정책(사용자 확정 2026-07-31, summary.md §6 4문항)**:
  1. **환율은 KIS 원값 그대로** — 원화 교차환율 변환 없음. 방향이 반대인 유로·영국·호주는 라벨 아래 `달러/유로` 식으로 **방향을 명기**해 오독을 막는다.
  2. **가능한 것만으로 섹션 축소** — 에너지 선물(0/3)은 **섹션 자체 삭제**해 **7섹션 → 6섹션 32종**. ICE 해외선물만으로 되는 설탕·가스오일도 **제외**(신규 API 계열 + 전일 정산가 한 값 + 롤오버라 나머지와 품질·구현 성질이 다름). NYMEX·CBOT 시세 신청과 오피넷 키 발급은 **사용자 판단으로 보류** — 신청이 되면 별도 Phase로 다룬다.
  3. **섹션별 표 1개** — 항목명 좌 / 숫자 우(`.numeric`) / 등락률만 방향 색 / 행마다 기준일. 첫 섹션 펼침 + 나머지 `<details>` 접힘(§87 관례).
  4. **기존 콘텐츠는 미국 10년물·비트코인만 유지** — WTI·금 카드 삭제, **미니 차트 4종 전면 삭제**. 「일별 기록」 접힘은 **유지**(차트를 지우면 시계열을 볼 유일한 수단이고, 그것도 표라서 새 섹션과 성질이 같다).
- **최종 32종**:

  | 섹션 | 항목 | 제외된 요청 항목 |
  |---|---|---|
  | 세계 증시 12 | 다우 `.DJI` · S&P500 `SPX` · 나스닥종합 `COMP` · 반도체 `SOX` · VIX / 상해종합 `SHANG` · 항셍 `HK#HS` / 니케이225 `JP#NI225` / 유로STOXX50 `SX5E` · 독일DAX `GR#DAX` · 영국FTSE `GB#FTSE` / 대만가권 `TW#WT` | TOPIX·러셀2000 (마스터에 코드 없음) |
  | 환율 8 | 미국 `X/FX@KRW` · 중국 `FX@CNY` · 유로 `FX@EUR` · 일본 `FX@JPY` · 인도네시아 **`FX@INR`** · 영국 `FX@GBP` · 브라질 `FX@BRL` · 호주 `FX@AUD` | — |
  | 유가 3 | WTI `N/WTIF` · 두바이유 `N/DUBAIF` · 브렌트유 `N/BRENTF` | 휘발유·경유·LPG (KIS 전 경로 부재 → 오피넷 키 보류) |
  | 귀금속 3 | **국내 금 `J/M04020000`**(185,990원, 국내 현재가 API) · 국제 금 `N/GOLDLNPM` · 은 `N/SLVRLN` | 백금·팔라듐 (S 전용 + NYMEX 차단) |
  | 비철금속 3 | 구리 `N/LMECOC` · 아연 `N/LMEZINC` · 알루미늄 `N/LMEALC`(합금 NASAAC 아님) | 납·니켈·주석 (S 전용, LME는 KIS 미취급 → 신청 대상조차 없음) |
  | 농산물 3 | 옥수수 `N/CHICORN` · 커피 `N/COFFE` · 면화 `N/COTTON` | 설탕·대두·대두유·소맥 (S 전용/CBOT 차단) · 쌀 (코드 없음) |

- **갱신 비용·게이트(사용자 확정)**: 신규 **26콜/회차**를 **하루 3회차(09:00·15:40·18:15)만** 갱신 → **+78콜/일**. 매 회차면 +1,092콜/일인데 해외 지수·상품은 전일 종가가 하루 한 번 바뀔 뿐이라 10분 주기는 낭비다. **신규 잡 라우트를 만들지 않고**(AGENTS.md §2 "잡 6종" 규칙 유지) `refreshMarketData` 안에 **회차 시각 게이트**를 둔다.
  - 재사용으로 콜을 줄인 근거 — 환율 8종 중 `FX@EUR·JPY·GBP`는 **`refreshDxy`가 이미 받아오고**(`KIS_DXY_COMPONENTS`), `FX@KRW`는 `usdkrw` 기존이라 **신규는 CNY·INR·BRL·AUD 4콜뿐**이다(summary.md는 5로 산정했으나 실제 4). WTI·국제 금도 기존 `oil`·`gold` 스냅샷 재사용으로 0콜. 내역: 환율 4 + 증시 12 + 비철 3 + 농산물 3 + 두바이·브렌트 2 + 은 1 + 국내 금 1 = **26**.
  - 재사용 조건 — `refreshDxy`가 현재 6종 원응답을 내부에서 버리므로 **`Map<code, raw>`를 반환하도록 고쳐** 표 빌드에 넘긴다. dxy·oil·gold는 매 회차 도니 게이트 회차에도 최신값이다.
- **구현 전 코드 실측(2026-07-31)** — 카탈로그를 쓰기 전에 **32종 전부 라이브 확인**(읽기 전용, Redis 캐시 토큰 재사용). 잘못된 코드는 배포 후 영구 결측이 되고 summary.md에 VIX·스위스 등 일부 코드가 기록돼 있지 않았다.
  - **VIX 코드는 `N/VIX`** (17.09, 23행) — 후보로 넣어본 `SPVIX`·`VX`는 `rt_cd=0`이면서 값 `0.00`·0행. 이 "성공 응답인데 값이 0" 패턴이 S 카테고리와 같아서, 매퍼에서 **종가 0을 실패로 다루도록** 했다(표에 `0.00 (0.00%)`이 남지 않게).
  - 32종 전부 정상 — 국내 금 `J/M04020000` 187,820원(+0.98%) · 은 58.0550 · 구리 13,795 · 옥수수 468.40 등. **기준일이 실제로 갈렸다**(아시아·원/달러 20260731 vs 미국·유럽 20260730) — 행별 기준일 표기의 근거를 그대로 재확인.
  - `M04020100`(미니금 99.99_100g)도 정상이나 카탈로그에는 1kg만 넣었다.
- **구현**:
  - `lib/api/kis/constants.ts` — `KIS_GLOBAL_TABLE_SECTIONS`(섹션 6 × 항목별 출처·라벨·단위·소수점 자리) + `GLOBAL_TABLE_ROUND_MINUTES`·`GLOBAL_TABLE_ROUND_WINDOW_MINUTES`. 출처는 4종 — `overseas`(1콜)·`dxyPair`(§28 응답 재사용)·`detail`(`market:detail:{oil|gold}` 재사용)·`domestic`(국내 현재가). `KIS_OVERSEAS_INDICATOR` 주석의 "S 계열 사용 금지" 근거를 **"값이 2023년에 정지"로 격상**해 다시 시도하지 않게 못 박았다.
  - `lib/api/kis/client.ts` — `fetchKisOverseasDailyByCode(marketDivCode, code)` 신설, 기존 `fetchKisFxPairDaily`는 이 함수에 위임(중복 제거). `types.ts`에 `prdy_vrss` 필드 명시(국내 금 전일 대비).
  - `lib/market/store.ts` — 새 키 `market:globalTable` 하나. 항목마다 detail 키를 32개 만들지 않았다 — 차트·일별을 안 쓰고 값·등락·기준일만 필요해서 화면이 GET 1회로 완결된다.
  - `lib/indices/globalTable.ts`(신규) — `isGlobalTableRound`(게이트) + `buildGlobalTableSections`. `parseNum`/`applyKisSign` 경유(§2), **항목 단위 실패 격리 + 직전 스냅샷 행 이어받기**(`staleAt`) — 3회차만 갱신하므로 한 회차 실패로 행이 사라지면 최대 반나절 빈다. 이어받을 값도 없는 항목만 표에서 빠진다.
  - `lib/jobs/refreshMarketData.ts` — `refreshDxy`가 통화쌍 원응답 맵을 함께 반환하도록 고쳐(중간 실패 시 그때까지 받은 것만) 환율 3종 재사용, KOSPI `basDt` 계산을 파이프라인 앞으로 끌어올려 국내 금 기준일과 `tradingDay` 판정이 공유. 리포트에 `globalTable: { ok, skipped?, fresh?, stale?, dropped? }`.
  - `components/indices/GlobalTableSection.tsx` + CSS(신규) — 서버 컴포넌트, 네이티브 `<details>`(§87 패턴, 클라이언트 번들 증가 0). 4열 + 항목명 아래 단위·호가 방향, 이어받은 행은 기준일 뒤 `*`.
  - `app/indices/market/page.tsx` — 미국 10년물·비트코인 2카드(차트 없음, 일별 기록 유지) + 6섹션(첫 섹션 펼침). 헤더 「마지막 갱신」은 카드 2종 기준이고 **표는 자기 갱신 시각을 따로** 적는다(§85 dxy와 같은 이유 — 하루 3회차를 섞으면 10분마다 도는 카드까지 낡아 보인다).
  - `lib/format/index.ts` — `formatFixed(value, digits)`. 환율 4자리·지수 2자리·국내 금 정수가 한 표에 섞인다.
  - **삭제**: `BtcChartClient`·`BtcLineChart`(+CSS) 3파일 — 이 화면 전용이라 파일째 제거. `IndexChartClient`·`IndexLineChart`는 **남겼다**(지수 상세·`DollarIndexSection`이 계속 쓴다). **`oil`·`gold` 수집도 유지** — `getDashboard.ts:20`의 `OPTIONAL_KEYS`가 홈 카드에서 읽어 삭제하면 홈이 깨지고, 새 유가·귀금속 표가 그 스냅샷을 재사용한다(추가 콜 0).
- **콜 수 정정**: summary.md는 환율을 5콜로 산정했으나 실제 **4콜**이다(`FX@EUR·JPY·GBP`는 달러 인덱스 태스크가 이미 받아오고 `FX@KRW`는 `usdkrw` 기존). 브렌트유가 +1콜이라 **합계 26콜 / +78콜/일**은 그대로다.
- **검증**: tsc·eslint·`npm run build` 통과. 32종 코드 라이브 실측 통과(위). 게이트 시각 540·940·1095는 `staleness.ts`의 `SCHEDULE_MINUTES`에 실재하는 슬롯임을 확인 — 없는 시각을 넣으면 표가 영구 미갱신이 된다.
- **최초 시딩(2026-07-31 12:26 KST)**: 배포 후 수동 트리거로 즉시 시딩했다. 이 과정에서 **`?force=true`가 표 게이트를 우회하지 못하는 결함**을 발견 — force는 KIS 허용 창만 우회해서 게이트 창(09:00·15:40·18:15) 밖 수동 호출은 `skipped`로 통과하고, 새 키는 다음 회차(최대 반나절)까지 빈 채로 남았다. force의 원래 용도가 「디버깅·최초 시딩」이므로 이 게이트도 함께 우회하게 넓혔다(`refreshMarketData(trigger, { force })` → `refreshGlobalTable(..., force)`). QStash 경로는 그대로 force를 무시한다. **결과: `globalTable: { ok: true, fresh: 32, stale: 0, dropped: [] }`** — 32종 전부 실값 저장 확인(다우 52,208.06 / 국내 금 187,680원 / 은 58.05 / 옥수수 468.40 …), 기준일도 의도대로 갈렸다(아시아·원/달러 20260731 · 미국·유럽 20260730).
- **상태**: **구현 완료(2026-07-31)**. 배포 후 확인 필요 — ① 첫 09:00/15:40/18:15 회차에 표가 채워지는지 ② **360px에서 4열이 한 줄에 들어가는지**(계산상 최장 라벨 「필라델피아 반도체」 99px + 값 73 + 등락률 45 + 기준일 38 + 갭 24 = 279px로 내부 폭 304px에 겨우 들어간다 — 넘치면 라벨 축약) ③ 첫 섹션만 펼쳐져 있는지 ④ 환율 「달러/유로」 방향 표기가 오독되지 않는지. **§89에서 표 → 타일 그리드로 재개편했다.**

---

### Phase 89 — 글로벌 지표 화면 타일 그리드 재개편: 27종 5구획, 값 큼직 + 등락률 아래 (2026-07-31, 구현 완료)

- **요청 근거**: 사용자 — §88의 4열 표(32종)가 **항목이 많아 보기 불편**하다. 항목을 추리고 **값을 크게, 전일 대비율은 값 아래 작게** 두는 타일 배치로 바꾸고, **기준일 열은 빼고**, **전 구획을 펼친 상태**로.
- **정책(사용자 확정 2026-07-31, 4문항)**:
  1. **환율 4열은 값 폰트 `clamp()` 자동 조절** — 360px에서 값 폰트 상한이 12.5px이라 "크게"가 성립하지 않는다. 넓은 화면에서만 커지게 두고 좁은 화면은 현행 크기로 수렴시킨다. (버린 대안: 소수 자릿수 축소 → 환율 정밀도 손실 / 2열 4행 → 요청한 4열 2행이 아님)
  2. **국기는 이모지** — 추가 자산 0개, 폭 절약. **Windows 크롬·엣지는 국기 이모지를 렌더링하지 않아 「US」 같은 문자로 보인다**(iOS·안드로이드·맥은 정상). 이 앱이 모바일 PWA 위주라 감수한다. 거슬리면 `public/flags/*.svg` 8개로 교체하는 선택지가 남는다.
  3. **화면에서 빠지는 5종(다우존스·VIX·독일 DAX·영국 FTSE·대만 가권)은 카탈로그에서 삭제** — 수집도 중단해 콜을 줄인다. 되살리려면 카탈로그에 항목을 되돌리면 된다.
  4. **기준일은 전 구획에서 제거** — 귀금속/비철금속도 포함(요청 원문 「기준일 해줘」를 확인).
- **최종 구성 — 상단 6타일 + 4구획 27종** (§88의 6섹션 32종에서 재편):

  | 구획 | 열×행 | 항목 | §88 대비 |
  |---|---|---|---|
  | 상단(구획 밖) | 3×2 | 미국 10년물 · 비트코인(USD) · 반도체 `SOX` · 두바이유 · 브렌트유 · WTI | 카드 2개 + 「국제 유가」 섹션 흡수 |
  | 세계 증시 | 3×2 | 나스닥 종합 · S&P 500 · 상해 종합 · 항셍 · 니케이 225 · 유로 STOXX 50 | 12 → 6 (반도체는 상단으로) |
  | 전세계 환율 | 4×2 | 미국 · 중국 · 유로 · 일본 · 인도네시아 · 영국 · 브라질 · 호주 | 8 유지 + 국기 이모지 |
  | 귀금속 · 비철금속 | 3×2 | 국내 금 · 국제 금 · 은 · 구리 · 아연 · 알루미늄 | 두 섹션 병합 |
  | 농산물 | 3×1 | 옥수수 · 커피 · 면화 | 3 유지 |

- **폭 실측(§88과 같은 방식)** — 내부 폭은 `v − 32`(container padding) `− 24`(card padding), `--layout-max-width: 480px`이라 v>480이면 424px 고정. 타일 gap 8px · 타일 좌우 padding 4px · 문자 폭 0.542em(§88의 「`51,594.14` 9자 = 58.5px @12px」에서 역산).
  - **타일 내부 폭** = 3열 `v/3 − 34.67` · 4열 `v/4 − 28` → @360px 88.0 / 62.0px, @480px 125.3 / 92.0px.
  - **값 폰트 상한** = 3열(9자 `51,594.14`) `v/14.634 − 7.108` → @360 **17.5px** · @402 20.4px · @480 25.7px. 4열(8자 `1,387.50`) `v/17.344 − 6.456` → @360 **14.3px** · @402 16.7px · @480 21.2px.
  - **채택 `clamp()`**: 3열 `clamp(15px, 4.8vw, 24px)` · 4열 `clamp(12px, 3.85vw, 20px)`. 기울기는 **최악 지점인 360px에서 상한에 닿도록** 잡았고(3열 4.86vw·4열 3.97vw가 이론 상한), 최대값은 **컨테이너가 480px에서 고정된 뒤에도 vw가 계속 커지는 것**을 막는 캡이다 — 캡이 없으면 넓은 데스크톱에서 값이 타일을 넘는다.
  - 상단 6타일도 3열 값을 쓴다(최장 8자 `6,127.45`라 여유). **비트코인 USD만 정수 표기로 바꿨다** — `118,432.50`은 10자라 3열 상한이 14.8px로 떨어지는데, 11만 달러 단위에서 센트 자리는 정보가 없다.
- **단위 문구를 짧게 줄였다** — 표(가로) 때는 라벨 옆에 여유가 있었지만 타일(세로)에서는 88px 안에서 줄바꿈된다. `USD/온스 · LBMA 런던` → `USD/온스`, `원/g · KRX 99.99_1kg` → `원/g`, `센트/부셸 · 시카고` → `센트/부셸`, `필라델피아 SOX` → `SOX`. **잘라낸 설명은 전부 구획 각주로 옮겨** 정보가 사라지지 않게 했다.
- **기준일 제거로 생긴 두 구멍을 각주로 메웠다**:
  - 미국·유럽 지수가 **항상 전일 종가**라는 단서 → 세계 증시·상단 각주에 문장으로 남겼다(§88은 행마다 기준일을 적어 눈으로 알 수 있었다).
  - 조회 실패로 직전 값을 이어받은 행의 `*` 표시 → **값 뒤**로 옮겼다(`title="직전 회차 값"` 유지).
- **갱신 주기가 섞이는 상단 6타일** — 미국 10년물·비트코인은 `market:detail`(10분), 반도체·유가 3종은 `market:globalTable`(하루 3회차)이다. **`us10y`·`btcUsd`를 카탈로그에 넣지 않았다** — 넣으면 `detail` 재사용이라 콜은 0이지만 표 스냅샷에 갇혀 **10분 갱신이 하루 3회로 퇴화**한다. 화면에서 두 출처를 합쳐 그리드 하나로 그리고, 주기 차이는 각주로 알린다.
- **콜 수**: overseas 21(상단 3 + 증시 6 + 환율 5 + 금속 4 + 농산물 3) + domestic 1 = **22콜/회차 · 66콜/일**. §88의 실제값 27콜/81콜에서 **−5콜/회차**.
  - **§88 기록 정정** — 그 문서는 26콜이라 적었으나 카탈로그상 `FX@KRW`가 `overseas`라 실제로 호출된다(환율은 4가 아니라 5콜). 실제는 27콜/81콜이었다.
- **구현**:
  - `lib/api/kis/constants.ts` — `KIS_GLOBAL_TABLE_SECTIONS`를 6섹션 32종 → **5섹션 27종**으로 재편(`highlights`·`worldIndices`·`fx`·`metals`·`agriculture`). 환율 항목에 `flag`(이모지) 추가, `unit` 축약, 각주 재작성. 삭제 5종은 주석에 근거를 남겨 다시 넣을 때 판단할 수 있게 했다.
  - `components/indices/GlobalTile.tsx`(+CSS, 신규) — 타일 하나(라벨·단위·값·등락률). 값 폰트는 **`var(--tile-value-size)` 상속**으로 받는다 — CSS Modules는 파일마다 클래스 해시가 달라 부모가 자식 클래스를 직접 못 고르지만, **CSS 변수는 상속되므로** 그리드 쪽에서 열 수에 맞는 크기를 내려보낼 수 있다. 값은 포맷된 문자열로 받는다(정수·2자리·4자리가 섞이고 상단은 `formatIndex`도 쓴다).
  - `components/indices/GlobalTileSection.tsx`(+CSS, 신규) — `<details open>` + 3열/4열 그리드. **접기 기능 자체는 남겼다**(요청은 "펼친 상태"이지 "접기 제거"가 아니고, 네이티브 `<details>`라 비용이 0이다).
  - **삭제**: `GlobalTableSection.tsx`(+CSS) — 표 폼 전체.
  - `app/indices/market/page.tsx` — 상단 3×2 그리드(detail 2 + `highlights` 4) + 4구획. `btcKrw`·일별 기록(비트코인·미국 10년물) 제거.
  - `lib/format/btc.ts` — `formatBtcChange` **삭제**(이 화면이 유일한 사용처였다). `formatBtcValue`는 홈 `MarketCard`가 계속 쓴다.
  - **유지**: `btcKrw` 수집(업비트 호출이라 KIS 콜과 무관, 홈 카드 복구 여지) · `IndexDailyList`(지수 상세가 계속 쓴다) · `market:globalTable` 저장 스키마(열 수는 화면이 섹션 id로 정하므로 스냅샷에 표현 정보를 넣지 않았다).
- **배포 후 첫 회차까지 화면이 비는 구간이 있다** — 섹션 id가 바뀌어(`oil`·`preciousMetals`·`baseMetals` → `highlights`·`metals`) 저장된 스냅샷의 옛 id와 맞지 않는다. 화면은 **저장된 섹션을 그대로 그리므로** 재시딩 전까지 옛 6섹션이 3열로 나오고 상단 4타일이 빈다. §88과 같이 **수동 `?force=true` 트리거로 즉시 재시딩**한다(force가 회차 게이트도 우회하는 것은 §88 마지막에 고친 그대로).
- **검증**: tsc·eslint·`npm run build` 통과.
- **상태**: **구현 완료(2026-07-31)**. 배포 후 확인 필요 — ① 수동 재시딩 후 상단 6타일이 다 차는지 ② **360px에서 4열 환율 값이 타일을 넘지 않는지**(가장 빠듯한 지점) ③ 국기 이모지가 기기에서 국기로 보이는지 ④ 전 구획이 펼쳐져 있는지 ⑤ 다음 정규 회차에 값이 갱신되는지. — ③은 §90에서 SVG 이미지로 교체해 해소.

### Phase 90 — 세계 증시 4열 2행(다우존스 복귀 + 코스피) + 국기 SVG 교체 (2026-07-31, 구현 완료)

- **요청 근거**: 사용자 — ① 주요 지표에 반도체·국제 유가 3종을 올릴 것 ② 세계 증시에서 VIX·독일·영국·대만을 빼고 코스피를 넣어 **4열 2행**을 맞출 것 ③ 전세계 환율 국기를 이미지로 할 수 있는지.
- **①·②의 절반은 이미 §89에 있었다** — 요청 시점(14:39 KST)에 화면이 §88 배치로 보인 것은 코드가 아니라 **스냅샷 때문**이다. §89 배포가 14:17이고 표는 하루 3회차(09:00·15:40·18:15)만 갱신되므로, 화면은 09:00 회차에 저장된 옛 6섹션 32종을 그대로 그리고 있었다. 반도체·유가 3종의 상단 이동과 VIX·독일·영국·대만 삭제는 §89에서 끝난 작업이라 **§90에서 새로 한 일이 아니다**.
- **정책(사용자 확정 2026-07-31, 2문항)**:
  1. **8번째 칸은 다우존스 복귀** — §89에서 뺐던 `.DJI`를 되살린다(KIS +1콜/회차 = 하루 +3콜). 버린 대안: 코스닥 추가(0콜이지만 「세계 증시」 제목과 어긋남) / 한 칸 비우기.
  2. **코스피는 10분 주기** — 카탈로그에 넣지 않고 화면이 `market:detail:kospi`를 직접 읽어 구획 첫 자리에 끼운다. §89 상단 6타일과 같은 구조다(카탈로그에 넣으면 콜은 0이지만 하루 3회차 스냅샷에 갇혀 10분 갱신이 퇴화한다). 대가로 **한 구획 안에 갱신 주기가 섞이므로** 각주 첫 문장에 명시했다.
- **세계 증시 최종 8종** — 코스피(10분) · 다우존스 · 나스닥 종합 · S&P 500 · 유로 STOXX 50 / 상해 종합 · 항셍 · 니케이 225. 국내 → 미국 → 유럽 → 아시아 순이라 1행이 국내+미국, 2행이 유럽+아시아로 갈린다.
- **4열 값 폰트를 다시 잡았다(9자 기준)** — §89의 4열 `clamp`는 환율 최장값 `1,387.50`(8자) 기준이었는데, 세계 증시가 4열이 되며 `44,123.45`(9자)가 같은 열 수를 쓴다. 8자 기준 3.85vw는 @360px에서 13.9px라 9자를 그리면 타일을 **0.9px 넘긴다**.
  - 값 폰트 상한(4열·9자) = `v/19.512 − 5.166` → @360px **12.7px** · @402 15.4px · @480 이상 18.9px(내부 폭 92px 고정).
  - 채택: `clamp(10px, 3.5vw, 18px)`. 기울기는 360px에서 상한에 닿도록(이론 상한 3.53vw), **하한 10px은 320px(9자 상한 10.7px)에서도 넘치지 않는 값**이다. 환율도 같은 크기를 쓰게 되어 8자 기준일 때보다 약간 작아지지만, 4열 두 구획의 값 크기가 어긋나지 않는 이득이 있다.
- **국기: 이모지 → 로컬 SVG** (§89 정책 2의 유보 조건을 실행). Windows 크롬·엣지가 국기 이모지를 「US」 문자로 렌더링하는 문제가 이유다.
  - `public/flags/{us,cn,eu,jp,id,gb,br,au}.svg` 8개 + `LICENSE` — `flag-icons` 7.5.0(MIT)의 4x3 SVG를 복사했다. **패키지는 의존성에 넣지 않았다**(자산 8개만 필요하고, 런타임 코드가 아니라 정적 파일이다). 합계 약 12KB(브라질이 7KB로 가장 크다 — 별자리).
  - 카탈로그 `flag`가 이모지 문자에서 **소문자 2자 국가 코드**로 바뀌었다(`"us"`). 값이 `public/flags/{flag}.svg` 경로에 들어가므로, 화면에서 `/^[a-z]{2}$/`로 한 번 더 막는다 — 값은 카탈로그 상수에서만 오지만 **Redis 스냅샷을 거쳐** 오기 때문이다. 어긋나면 국기만 빠지고 라벨은 그대로 나온다.
  - `next/image`를 쓰지 않았다 — SVG는 `dangerouslyAllowSVG` 없이는 최적화 대상이 아니고, 1KB 안팎 정적 파일에 최적화 파이프라인을 태울 이유가 없다. `<img width=16 height=12>`로 CLS를 막고 `no-img-element` 규칙만 국소 해제했다. 장식이라 `alt=""`(국가명은 옆 라벨이 읽어준다).
  - 타일 CSS에 `box-shadow: 0 0 0 0.5px var(--color-border)` 한 겹 — **일본기처럼 바탕이 흰 국기가 배경에 묻히는 것**을 막는다. 라이트/다크 각각 정의된 토큰이라 함께 뒤집힌다.
- **콜 수**: overseas 22(상단 3 + 증시 7 + 환율 5 + 금속 4 + 농산물 3) + domestic 1 = **23콜/회차 · 69콜/일**(§89 22/66에서 다우존스만큼 +1/+3). 코스피는 이미 도는 `market:detail`이라 0콜.
- **구현**: `lib/api/kis/constants.ts`(다우존스 복귀·순서 재배치·`flag` 코드화·각주) · `app/indices/market/page.tsx`(`kospi` MGET 3키로 확장, `kospiRow`를 구획 앞에 끼움, `FOUR_COLUMN_SECTIONS`에 `worldIndices` 추가, 각주 27→28종) · `components/indices/GlobalTile.tsx`(+CSS, `<img>` 국기) · `GlobalTileGrid.module.css`(4열 clamp 재계산) · `types/indices.ts`·`lib/indices/globalTable.ts`·`lib/jobs/refreshMarketData.ts`(주석 종수·콜 수).
- **스냅샷 이어받기는 이번엔 안전하다** — 구획 id가 그대로라(`worldIndices`) 다우존스만 새 항목이다. 첫 회차에 그 하나가 실패하면 이어받을 값이 없어 빠지고(7타일), 나머지 7종은 §89 스냅샷을 그대로 이어받는다.
- **검증**: tsc·eslint·`npm run build` 통과.
- **상태**: **구현 완료(2026-07-31)**. 배포 후 확인 필요 — ① 재시딩(또는 15:40 회차) 후 세계 증시가 **코스피 포함 8타일 4열 2행**인지 ② **360px에서 4열 9자 값(`44,123.45`)이 타일을 넘지 않는지** ③ 환율 국기가 8종 다 뜨는지(특히 다크 모드에서 일본기 테두리) ④ 코스피만 10분마다 움직이는 것이 각주로 납득되는지 ⑤ 「유로 STOXX 50」 라벨이 4열에서 줄바꿈돼도 열이 어긋나지 않는지.

### Phase 91 — 지표 화면 각주를 「설명」 토글로 (공용 `NoteDisclosure`) (2026-07-31, 구현 완료)

- **요청 근거**: 사용자 — 글로벌 지표 상세의 항목란 하단 부연설명을 **원할 때만 펼쳐 보게** 해달라(설명 버튼 등). 이어서 "글로벌 지표뿐 아니라 **각 항목들의 상세화면 카드별 각주**도 동일하게".
- **§89·§90에서 각주가 무거워진 것이 배경이다** — 단위 축약으로 잘라낸 거래소·기준 설명, 기준일 열 제거로 옮겨 온 「미국·유럽은 전일 종가」 단서, 갱신 주기가 섞인다는 설명이 전부 구획 각주로 모였다(§89·§90). 값을 크게 보려고 만든 타일 아래에 4~5줄짜리 회색 문단이 붙어 정보 밀도가 되돌아갔다.
- **채택안: 네이티브 `<details>` 중첩** (버린 대안 2가지)
  - **`popover` + `popovertarget`**(「?」 아이콘 → 오버레이): 자리를 아예 안 먹지만, CSS anchor positioning 지원이 고르지 않아 하단 시트/중앙 모달로 고정해야 하고 dim·닫기 스타일이 따라붙는다. iOS Safari 17+ 요구도 생긴다.
  - **`'use client'` 토글 버튼**: AGENTS.md §3(Recharts만 Client)에 어긋나고, 각주 10지점에 클라이언트 경계가 생긴다.
  - `<details>`는 **클라이언트 번들 0 · 서버 컴포넌트 유지 · 키보드/스크린리더 기본 지원**이고 §87 `MarketFlowSummary`·§89 구획 카드가 이미 쓰는 폼이다. 구획 카드(`<details open>`) 안에 또 `<details>`가 들어가지만 유효한 HTML이고, 자식은 `<summary>` 바깥(본문)이라 부모 토글의 클릭 영역과 겹치지 않는다.
- **공용 컴포넌트 `components/ui/NoteDisclosure`** — `<details>` + 「ⓘ 설명 ▾」 요약줄 + 각주 본문. 여백은 사용처마다 달라 `className`을 받아 합친다(폰트·색·펼침 회전은 컴포넌트가 쥔다). 라벨은 기본값 "설명"으로 **10지점 전부 통일** — 자리마다 다르게 부르면 같은 조작이 다른 것처럼 보인다.
- **적용 기준 — "왜/어디서/무슨 기준"은 접고, "이 표를 읽는 법"은 남긴다.**
  - **접음(화면 10지점 / 코드 7지점)**: 글로벌 지표 주요 지표 각주 · 구획 각주 4개(`GlobalTileSection` 1지점) · 갱신 주기 푸터 / 지수 상세 3화면 공용 푸터 `dataNotice`(`IndexDetailScreen` 1지점) / 달러 인덱스 근사치 각주 / 코스피 변동성 계산식 푸터 / 수출입 상세 출처·오차 푸터.
  - **남김(5곳)**: `MarketFlowSummary`의 부호 범례 · `MarketCapRankingTable`의 기준 한 줄 · `IndexDetailScreen` 일별 수급 탭의 `sectionNote` · 수출입 상세 `cardNote` 2개(품목/국가 표의 집계 기준·조작 안내). **표 값 자체를 오독하게 만드는 문장들**이라 접으면 안 된다.
  - **홈 대시보드 `DataAsOfFooter`는 손대지 않았다** — 요청 범위가 상세화면이고, 홈은 카드마다 각주가 없어 푸터 한 곳뿐이다. 필요하면 같은 컴포넌트를 한 줄로 끼우면 된다.
- **푸터의 「마지막 갱신」·「기준일」 줄은 그대로 뒀다** — 각주가 아니라 데이터의 나이 표시라, 접으면 staleness를 숨기는 셈이 된다(§11.10-B 배지와 같은 이유).
- **구현**: `components/ui/NoteDisclosure.tsx`(+CSS, 신규) · `app/indices/market/page.tsx`(+CSS, 2지점) · `components/indices/GlobalTileSection.tsx`(+CSS) · `components/indices/IndexDetailScreen.tsx`(+CSS) · `components/indices/DollarIndexSection.tsx`(+CSS) · `app/indices/kospi-volatility/page.tsx`(+CSS) · `app/indices/trade/[yyyymm]/page.tsx`(+CSS). 각 CSS의 각주 클래스는 **여백만 남기고** 폰트·색을 컴포넌트로 옮겼다.
- **데이터·잡은 무변경** — 각주 문구는 `KIS_GLOBAL_TABLE_SECTIONS`(카탈로그) → Redis 스냅샷 → 화면으로 이미 흐르고 있어 렌더 단계만 바뀐다. 스냅샷 스키마·콜 수·갱신 주기 영향 없음.
- **검증**: tsc·eslint·`npm run build` 통과.
- **상태**: **구현 완료(2026-07-31)**. 배포 후 확인 필요 — ① 글로벌 지표 5개 카드에 「설명」이 접힌 채 뜨고 눌러 펼쳐지는지 ② 구획 카드를 접었다 펴도 안쪽 설명 상태가 꼬이지 않는지 ③ 지수 상세 3종·변동성·수출입 푸터의 「설명」 ④ 다크 모드에서 요약줄 대비 ⑤ 터치 타깃이 작지 않은지(요약줄 상하 여백 4px).

---

### Phase 92 — 장중 수급 슬롯 영구 축적(날짜별 아카이브) + 수동 회차 격리 + 맥미니 SQLite 미러 (2026-07-31, 구현 완료)

- **요청 근거**: 사용자 — 2026-07-31 코스피 일별 수급이 기록적이었는데 **시간별로 어떻게 7조에 이르렀는지** 알고 싶다, 그리고 그 기록을 근거로 **매수세 폭등 시그널을 사전에 푸시 알림**으로 받고 싶다(임계값을 확률적·수학적으로).
- **실측(2026-07-31 코스피, 저장된 슬롯 43개 직접 조회)**: 외국인 종일 **+7.241조**. **09:10 한 슬롯에 2.605조(종일의 36%)**, 09:20 누적 53%, 10:00 누적 71%로 **개장 1시간에 대부분 끝났다**. 14:10 피크 7.812조 후 되돌림(15:40 종가 동시호가에서 −0.338조). 전일(7/30) 동시각은 0.265조라 **09:10 시점에 이미 10배 차이**. 일별로는 직전 19거래일 평균 −0.580조·표준편차 1.974조 대비 **z ≈ 3.96**(19일 내 최대 2.621조의 2.8배).
- **분석 지표 결론**: 절대액보다 **누적 순매수 ÷ 누적 거래대금**을 시그널로 쓴다(오늘 40.8%→16.3% / 전일 7.8%→2.4%, 5~7배 유지). 시장 규모에 안 휘둘려 임계값 수명이 길다. 단 **시간대별로 임계값이 달라야** 한다 — 09:10 슬롯은 시가 단일가가 통째로 들어가 구조적으로 비율이 튄다(전일도 09:10이 그날 최대였다).
- **막힌 지점 = 이 Phase의 이유**: 시간대별 임계값을 확률로 정하려면 과거 분포가 필요한데 **KIS는 과거 거래일의 시간대별 수급을 주지 않고**(§70 실측), 당일 키는 매일 덮어써져 **표본이 당일+전일 2개뿐**이었다. 지금 임계값을 정하면 통계가 아니라 하루치 오버피팅이다. 그래서 **먼저 쌓는다.**
- **채택안: Redis 날짜별 아카이브(원천) + 맥미니 SQLite 미러(복제본), 삭제 없음** (버린 대안 2가지)
  - **10년 롤오버**(10년마다 맥미니로 이관 후 Redis 리셋, 분석 시 병합): 사용자 제안. **해결하는 문제가 없어 폐기** — 롤오버 근거였던 "읽기 부담"은 `MGET` 100키 배치 정도였고, 분위수를 파생 키에 캐시하면 10분마다 도는 판정은 **1키만** 읽어 원본이 1년치든 10년치든 상시 비용이 같다. 반면 `noeviction`이 지켜 주는 데이터를 **수동으로 지우는 리스크**와 경계 구간 중복·누락 검증 코드가 영구히 따라붙는다.
  - **미러 단독**(Redis 아카이브 없이 맥미니가 매일 당일 키 수확): Redis 코드 변경 0이지만 **맥미니가 원천**이 된다 — 하루라도 놓치면 그날이 영구 소실(재발급 경로 없음). 맥미니 24시간 가동이어도 감수할 이유가 없다.
- **용량 실측(2026-07-31)**: Redis 254키 **1.37MB** / `maxmemory` **64MB** · `noeviction`. 슬롯 1개 103B, 하루 2시장 8.7KB, **연 약 2.1MB → 약 29년치 여유**. 지금 무거운 키(`dividendRanking` 221KB·`analysis:overview` 181KB·`stockMaster` 153KB)는 전부 고정 크기 스냅샷이라, **누적 증가하는 키는 이 아카이브가 사실상 유일**해진다. 압축(배열 포맷 42%·gzip 26%)은 **하지 않는다** — 29년을 60년으로 늘리자고 기존 `IntradayFlowSlot`과 다른 스키마를 만들면 변환 코드와 버전 분기가 영구히 붙는다.
- **아카이브는 회차마다 쓴다** — 계획 단계에서는 "마지막 회차(18:15) 1회 복사"(하루 2 SET)였으나, **그 회차가 실패한 날이 통째로 비고 지나간 거래일은 복구가 불가능**하다. 회차마다 당일 키와 함께 SET해도 하루 84회 남짓이라(잡은 이미 회차당 수십 개 명령을 쓴다) 실패 내성을 사는 편이 낫다. 멱등이라 재실행도 안전.
- **수동 회차 격리 — `trigger`로 가른다(시각으로 가르지 않는다)**: `?force=true` 수동 트리거가 정규 회차 사이 비정형 시각에 슬롯을 남긴다(실측 — 7/31 `1226` 슬롯 1개, 7/30은 42개 전부 정각). **hhmm 그리드 판별은 두 방향으로 틀린다** — ① 수동 호출이 정각에 떨어지면 기존 `filter((s) => s.hhmm !== hhmm)`가 **정규 슬롯을 조용히 덮어쓴다** ② 지연된 정규 회차는 멀쩡한 데이터인데 비정각이다. `verifyJobRequest`가 이미 `"qstash"`/`"manual"`을 반환하고 그 값이 `refreshMarketData(trigger, ...)`까지 와 있어 **인자 하나만 더 넘기면** 정확히 갈린다. 수동분은 버리지 않고(사용자 확정) `:manual:{날짜}`로 격리 — 날짜가 키에 들어가 그 자체로 아카이브다.
- **QStash 지연분은 trigger로 못 거른다**(걸러서도 안 된다 — 정상 데이터). 분석 단계에서 **가까운 10분 버킷에 귀속**시켜 흡수한다. 저장은 원본 그대로 둔다.
- **키 3종(신규 2)**: `market:investorIntraday:{시장}:archive:{YYYY-MM-DD}`(정규, TTL 없음) · `:manual:{YYYY-MM-DD}`(수동) · 기존 당일 키·`:baseline`은 **무변경**이라 화면 읽기 경로는 한 줄도 안 바뀐다.
- **미러 스크립트 `scripts/mirror-intraday-flows.mjs`**: SCAN+MGET → SQLite(`node:sqlite` 내장, **의존성 0**). PK `(market, trading_date, hhmm, source)`로 멱등, 기본은 미보유 날짜+최근 3일만 받고 `--full`로 전량 재적재. **읽기 전용 토큰**(`UPSTASH_REDIS_REST_READONLY_TOKEN`) 우선 사용 — SCAN·MGET만 하므로 쓰기 권한이 필요 없고 맥미니가 뚫려도 원본을 못 지운다. `foreign`이 SQL 예약어라 컬럼명은 `foreign_net`. launchd 샘플(`com.jusik.mirror-intraday.plist`, 매일 19:00)과 사용법·분석 시 주의는 `scripts/README.md`.
- **검증**: tsc·eslint 통과. 미러는 실제 스키마 모양의 임시 키 2개(`1970-01-01`)를 Redis에 넣어 **왕복 검증** — 정규/수동이 `source`로 갈려 적재되고, `tradingValue` 없는 슬롯은 `NULL`, 재실행 시 스킵, `--full` 시 재적재해도 3행 유지(멱등). 검증 후 임시 키 삭제(DBSIZE 254 복원).
- **상태**: **구현 완료(2026-07-31)**. 배포 후 다음 정규 회차에서 확인 — ① `archive:{오늘}` 키가 시장별로 생기는지 ② 수동 `?force=true` 응답에 `intradayFlows.manual: true`가 뜨고 정규 당일 키의 슬롯 수가 **안 늘어나는지** ③ 맥미니 launchd 첫 실행 로그.
- **후속(별도 Phase)**: 분위수 파생 키 + **그림자 모드**(발송 없이 "울렸을 것"만 기록, 60~80거래일) → 임계값 확정 → 알림 카테고리 6번째 추가. **기존 5종과 달리 시장 전체 알림이라 종목별 음소거 축과 맞지 않는다** — Phase 73 배당 예외가 Phase 79에서 폐기된 전례가 있으니 축을 처음부터 정리하고 들어간다. 발송 지연은 슬롯 주기상 **5~15분**(7/31 09:10 슬롯의 `fetchedAt`이 09:15:01)이라 초단타 진입용이 아니라 조기 인지용이다.

---

### Phase 93 — 일별 수급 표 날짜 행 펼침(시간대별 슬롯) + 슬롯 10주체 확장 (2026-07-31, 구현 완료)

- **요청 근거**: 사용자 — 코스피/코스닥 상세의 **일별 수급 목록에서 날짜를 클릭하면 시간대별로 펼쳐** 보고 싶다. 아카이브(§92)가 아직 1거래일뿐이라 "데이터가 쌓인 뒤에 화면을 붙이자"고 제안했으나, **미리 구현해 두면 쌓이는 대로 자동으로 채워진다**는 판단으로 지금 진행하기로 확정.
- **§92가 만든 것을 화면으로 꺼내는 Phase다** — `:archive:{날짜}` 키가 이미 "날짜 → 그날 슬롯 묶음" 모양이라 조회는 날짜당 GET 1회로 끝난다. 표는 20거래일을 보여주는데 아카이브는 **2026-07-31부터**라, 당분간 펼칠 수 있는 행은 아래쪽 1~2개뿐이다(KIS가 과거 시간대별 수급을 주지 않아 소급 불가, §70·§92).
- **결정 1 — 슬롯에 기관 세부 7종 추가**(사용자 확정). 지금 `IntradayFlowSlot`은 개인·외국인·기관계 + 거래대금뿐이라 펼친 표가 위 일별 표(10주체)와 어긋난다. `refreshIntradayFlowSlots`가 쓰는 `row`(`InvestorFlowRow`)에 **이미 10주체가 다 들어 있어 KIS 호출은 0**이고 저장 필드만 는다. 슬롯 103B → 약 250B, 연 2.1MB → 약 5MB(64MB 한도 대비 12년치 이상). **소급 불가라 미루면 그 기간이 영구히 3주체로 남는다**는 점이 지금 넣는 이유.
  - 7종은 **옵셔널 필드**로 넣는다. 필수로 선언하면 이미 저장된 7/31 슬롯(3주체)이 타입과 어긋난다. 화면은 필드가 없는 날짜를 "이 날은 개인·외국인·기관계만 기록됨" 한 줄과 함께 3열로 렌더한다.
- **결정 2 — 펼친 표는 누적과 증분을 나란히**(사용자 확정). 슬롯은 "그 시각까지의 누적"이라 그대로 늘어놓으면 §92 실측의 핵심(09:10 한 구간에 종일의 36%)이 안 보이고, 증분만 두면 특정 시각의 누계를 잃는다. **증분 = 그 슬롯 − 직전 슬롯**(첫 슬롯은 누적과 같다)이라 저장은 그대로 두고 화면에서 계산한다.
- **채택안: 어포던스는 축적 시작일 상수로, 슬롯 본문은 클릭 시 Server Action** (버린 대안 2가지)
  - **상세 진입 시 20일치 MGET 선로딩**: 클릭이 즉각적이지만 20일 × 43슬롯 × 10주체 ≈ **215KB(raw)** 가 상세 페이지마다 RSC 페이로드로 실린다. 대부분 펼치지 않을 데이터다.
  - **네이티브 `<details>` 서버 렌더**(§91 `NoteDisclosure` 패턴): 클라이언트 번들 0이지만 위와 같은 페이로드를 HTML로 지고, 표 안에서 행 단위 펼침을 만들려면 `<td colspan>` 안의 `<details>`가 되어 "날짜 행 클릭"이라는 조작과 어긋난다.
  - 채택안은 **초기 페이로드 0 · 클릭한 날짜만 Redis GET 1회**다. 표를 `'use client'`로 돌리는 것은 같은 탭 묶음의 `FiRankingTable`이 이미 쓰는 폼이고(§50), Server Action이 **Redis 스냅샷만 읽으므로** AGENTS.md §2(화면·Server Action은 KIS 직접 호출 금지)에 부합한다. 한 번 받은 날짜는 클라이언트 상태에 남겨 재요청하지 않는다.
  - **어포던스 판정에 키 존재 확인을 쓰지 않는다** — 20키를 EXISTS/MGET로 훑으면 선로딩과 같은 읽기를 하고 버리는 셈이다. 상수 `INTRADAY_ARCHIVE_SINCE`(= `2026-07-31`) 이전 날짜 행은 펼침 표시를 아예 그리지 않고, 이후 날짜만 열되 잡이 실패해 빠진 날은 펼쳤을 때 "기록 없음"으로 처리한다(드문 케이스에만 헛클릭).
- **`:manual:{날짜}`는 읽지 않는다** — 비정형 시각이라 정규 시계열에 섞으면 시각 간격이 어긋나 보인다(§92의 격리 이유와 같다).
- **거래대금 열도 함께 보여준다** — 슬롯에 이미 있고, §92가 시그널로 지목한 **누적 순매수 ÷ 누적 거래대금**을 눈으로 확인할 수 있는 자리가 된다.
- **펼친 표는 값 하나를 두 줄(누적 위·증분 아래)로 쓴다** — 주체 10종 × 2를 열로 펼치면 22열이라 못 읽는다. 열 수를 시각+10주체+거래대금 = 12로 묶어 위 일별 표(11열)와 폭이 어긋나지 않게 했고, 폭이 좁아 값 표기는 `formatEokFromMillion`("8,507억 5,400만원") 대신 §86의 `formatFlowCompact`("29.7조"·"-382억")를 쓴다. 색은 **누적에만** 준다(두 줄 다 색이면 구분이 사라진다).
- **증분은 직전 슬롯과의 차, 첫 슬롯은 누적 그대로.** 직전 슬롯에 그 주체가 없으면(스키마 확장 회차를 걸친 하루) `—`로 비운다 — 누적을 그대로 쓰면 증가분을 통째로 부풀린다.
- **`INTRADAY_ARCHIVE_SINCE`는 `market/store.ts`가 아니라 `types/indices.ts`에 뒀다** — 클라이언트 컴포넌트가 읽는 값이라 store에 두면 `@upstash/redis`를 물고 있는 모듈을 import하게 된다. 빌드 산출물(`.next/static/chunks`)에 `upstash` 문자열이 없는 것으로 경계를 확인했다.
- **구현**: `types/indices.ts`(`IntradayFlowSlot` 7필드 옵셔널 + `INTRADAY_ARCHIVE_SINCE`) · `lib/jobs/refreshMarketData.ts`(슬롯 조립에 7필드 복사) · `app/indices/actions.ts`(신규 — `auth`+허용 이메일·시장·날짜 형식/범위 검증 후 `getIntradayArchive` 1회) · `components/indices/InvestorFlowTable.tsx`(+CSS, `'use client'` 전환·펼침 행·중첩 표) · `components/indices/IndexDetailScreen.tsx`(`market` 전달 + 안내 문구) · `scripts/mirror-intraday-flows.mjs`(SQLite 컬럼 7개, 기존 DB는 `PRAGMA table_info` → `ALTER TABLE ADD COLUMN`) · `scripts/README.md`.
- **잡 스케줄·콜 수·기존 키 형식은 무변경** — 슬롯 필드만 늘고, 화면 읽기 경로는 Server Action 하나가 추가된다.
- **검증**: tsc·eslint·`npm run build` 통과. 미러는 **실 Redis를 읽어 사본 DB 2개로 왕복 검증** — ① 새 DB에 17컬럼으로 생성되고 7/31 86슬롯이 적재되며 세부 7컬럼이 `NULL`(확장 전 데이터라 정상), 외국인 09:10 = 2,605,061백만원으로 §92 실측과 일치 ② Phase 92 스키마의 기존 DB 사본에 컬럼 7개가 뒤에 붙고 **행 수 86 유지**, `--full` 재실행에도 86 유지(멱등). 클라이언트 번들에 Redis 미유입 확인.
- **상태**: **구현 완료(2026-07-31)**. 배포 후 확인 필요 — ① 7/31 행에만 펼침 표시가 뜨고 그 이전 19행에는 없는지 ② 7/31을 펼치면 **3주체 + 거래대금**으로 뜨고 "개인·외국인·기관계만 기록됐습니다" 안내가 붙는지 ③ **8/3 회차부터 10주체**로 펼쳐지는지(스키마 확장 첫 거래일이라 09:00 전후 회차가 3주체·10주체로 섞이면 그 경계 슬롯의 증분이 `—`가 되는 것이 정상) ④ 가로 스크롤 시 시각 열이 고정되는지 ⑤ 다크 모드에서 펼친 영역(`--color-bg`)과 상위 행의 대비 ⑥ 미러 재실행 시 8/3분에 세부 7컬럼이 채워지는지.

### Phase 94 — 배당 회차 병합을 회차 단위 → 필드 단위 보완으로 (2026-07-31, 구현 완료)

- **요청 근거**: 사용자 — 홈 배당 요약 카드에 **삼성전자 배당정보가 나왔다가 사라졌다**. 원인 조사 후 "혼합으로 해달라"로 방식 확정.
- **원인은 §83 병합 규칙의 사각지대다**(실 Redis 스냅샷 2건으로 확인). `market:dividendDecision:005930`은 26.2Q를 **374원·기준일 2026-06-30·지급일 2026-08-28**로 갖고 있는데, `market:stockInfo:005930`의 예탁원 `rounds`가 07-31 09:15Z 갱신에서 **같은 기준일을 금액만 채우고 지급일은 공란**(`payDate: null`)으로 새로 넣었다. `mergeDividendRounds`는 "예탁원에 없는 **기준일**만 공시로 메운다"였으므로 그 기준일이 `covered`로 잡혀 공시가 이미 알던 지급일이 버려졌고, `payDate !== null && payDate >= today`로 거르는 홈 카드(`getDividendCardSummary`)에서 회차가 빠졌다. 즉 **예탁원이 "일부만 채운" 중간 상태에서 이미 확보한 정보가 후퇴하는** 구조였다.
- **§83이 이 상태를 못 본 이유** — 07-30 실측 당시 예탁원은 같은 회차를 `per_sto_divi_amt=0`(미확정)으로 줬고, 0원 회차는 `buildDividendBlock`에서 걸러져 `rounds`에 아예 없었다. "예탁원에 있다/없다"의 두 상태만 관측한 것이다. 하루 뒤 금액만 확정된 세 번째 상태가 나타났다.
- **채택안: 같은 기준일이면 예탁원 회차를 살리되 빈 지급일만 공시로 채우고 `source: "mixed"`** (사용자 확정 "혼합"). 금액·종류는 **예탁원 우선 유지** — 정정을 반영하는 쪽이 예탁원이라 값 신뢰도의 본 소스는 그대로 둔다.
  - **보완 대상이 지급일 하나뿐인 것은 나머지에 메울 구멍이 없기 때문이다** — 기준일은 대조 키이고, 금액은 0원(미확정)이면 예탁원 회차 자체가 만들어지지 않으며(위 §83 경로), 종류(`divi_kind`)는 예탁원이 항상 준다. 그래서 "필드 단위 보완"의 실제 적용면은 `payDate` 하나다.
  - 버린 대안 — **공시를 통째로 우선**(예탁원에 payDate가 없을 때 회차 자체를 공시 것으로 교체): 금액·종류까지 공시 값이 되어 예탁원 정정 반영 경로를 잃는다. **홈 카드 필터에서 payDate null을 허용**: 지급일 없는 회차가 "다가오는 지급일" 목록에 정렬 기준 없이 섞인다(카드의 존재 이유가 지급일 순 4건이다).
- **알림도 같은 병합을 읽어 자동으로 고쳐진다** — `evaluateDividendAlerts`는 `payDate === today` 회차를 찾는데(`lib/alerts/dividendAlerts.ts`), 지급일이 null이면 8/28 당일에도 알림이 나가지 않는다. 병합에서 채워지므로 별도 수정이 없다.
- **화면 표기는 배지를 하나 더 둔다** — `dart`(회차 통째 공시)는 기존 「공시」, `mixed`는 「지급일 공시」. 같은 배지를 쓰면 "회차 전체가 미확정 소스"로 읽혀 실제보다 신뢰도를 낮게 보이게 한다. 각주에 한 문장 추가.
- **구현**: `lib/dividends/rounds.ts`(`source`에 `"mixed"` 추가 · `mergeDividendRounds`가 공시 맵을 먼저 만들어 예탁원 회차와 기준일로 대조) · `lib/dividends/summary.ts`(`DividendScheduleRow["source"]`를 `ScheduledDividendRound["source"]` 참조로) · `app/dividends/page.tsx`(배지 분기 + 각주). 스냅샷 스키마·잡·KIS 호출·라우트 6종 **무변경** — 읽기 시 병합 규칙만 바뀌어 **다음 갱신을 기다릴 필요 없이 배포 즉시 반영**된다.
- **검증**: tsc·eslint·`npm run build` 통과.
- **상태**: **구현 완료(2026-07-31)**. 배포 후 확인 — ① 홈 배당 카드에 삼성전자 26.2Q(주당 374원, 지급 2026-08-28)가 **다시 뜨는지** ② `/dividends?mode=schedule`에서 그 회차가 「지급일 공시」 배지 + 지급일 2026-08-28로 뜨는지(기존 「공시」 배지 회차와 섞이지 않는지) ③ 예탁원이 지급일을 채운 뒤 배지가 사라지고 값이 그대로인지.

### Phase 95 — 변동성 월 지표의 기준월을 「당월 고정」에서 「기록이 있는 마지막 달」 폴백으로 (2026-08-01, 구현 완료)

- **요청 근거**: 사용자 — 코스피 변동성 카드의 **월 평균 줄이 사라졌고** 상세의 **일별 기록 목록도 없어졌다**. 조사 후 "달이 넘어갔어도 아직 영업일이 시작된 게 아니면 전월 기준으로 그대로 보여달라"로 확정.
- **원인은 회귀가 아니라 월 경계 + 주말이다**(실 Redis `kospiVolatility:history` 115건으로 확인). 조사 시점이 2026-08-01(토)이라 **8월 기록이 0건**인데, 카드 요약과 상세 목록이 둘 다 `today.slice(0,7)`를 기준월로 **고정**하고 있었다. `getVolatilityCardSummary`는 `monthly.find(month === "2026-08")`가 `undefined`라 `currentMonthAvg`·`monthOverMonthDiff`를 모두 `null`로 냈고, `IndexDashboard`의 `monthParts`가 빈 배열이 되어 `note` 자체를 넘기지 않아 `SummaryCard`의 `note !== undefined` 가드에서 줄이 통째로 빠졌다. 상세는 `records.filter(startsWith("2026-08"))`가 빈 배열이라 섹션이 렌더되지 않았다. **월별 평균 차트만 남은 이유**는 그쪽이 처음부터 "기록이 있는 최근 6개월"(`aggregateMonthlyAverages`)이라 당월 고정이 아니었기 때문이다.
- **매달 반복되는 구조였다** — 1일이 주말이면 2~3일, 평일이어도 최소 그날 09:00 회차 전까지 대표값(최신 거래일 변동폭)만 남고 월 지표가 비는 상태가 된다. 대표값 17.78%는 떠 있는데 월평균만 없는 화면이 사용자에게 "빠진 것"으로 읽힌 것이 그대로 이 현상이다.
- **채택안: 기준월 폴백 = 당월 기록이 1건이라도 있으면 당월, 없으면 기록이 있는 마지막 달** (사용자 확정). 카드·상세가 같은 헬퍼(`resolveVolatilityBaseMonth`)를 쓰게 해서 둘이 어긋나지 않게 했고, 이때 카드 3줄(최신 변동폭·전일 대비·월 지표)과 상세 목록·차트 마지막 점이 모두 같은 달(7월)을 가리켜 **기준이 오히려 일관해진다**.
  - **당월 첫 거래일이 오면 즉시 당월로 전환된다** — 8/3(월) 09:00 회차에 8월 기록 1건이 생기면 기준월은 8월이 되고 월평균은 그 하루값, 전월 대비는 7월 평균(7.47%)과의 차가 된다. 표본 1건이라 값이 튀는 점을 짚었으나, **7월 중에도 7월 진행분 평균 vs 6월이었으므로 기존 설계 그대로**라는 사용자 판단으로 특별 처리 없이 둔다(누적 며칠 전까지 폴백 유지 안을 버린 이유).
  - 버린 대안 — **최근 N거래일 롤링 평균**: 월 경계 문제는 사라지지만 차트(월별 막대)와 카드의 집계 단위가 달라져 "전월 대비"의 의미가 흐려진다. **당월 0건일 때 월 지표 줄을 placeholder로 채우기**: 빈 줄이 뜨는 것뿐이라 정보량이 늘지 않는다.
- **라벨에 기준월을 반드시 박는다** — 폴백 상태에서 기존 문구 `월평균 7.47%`를 그대로 두면 8월 평균으로 오독된다. `7월 평균 7.47% · 전월 +2.30%p`처럼 월을 쓰고, 상세 섹션 제목도 `당월 일별 기록` → `7월 일별 기록`으로 **당월일 때도 같은 표기**로 통일했다(상태에 따라 문구가 바뀌면 그 자체가 읽는 부담이다).
- **타입 필드명을 의미에 맞췄다** — `currentMonthAvg`(당월 평균, nullable) → `baseMonth`+`baseMonthAvg`(non-null). 기록이 1건이라도 있으면 `aggregateMonthlyAverages`가 반드시 1개월 이상을 내므로 폴백 후에는 null이 나올 수 없고, 카드에서 `null` 분기를 지울 수 있다. `monthOverMonthDiff`만 전월 기록이 없을 때 `null`로 남는다.
- **구현**: `types/indices.ts`(`VolatilityCardSummary`의 `currentMonthAvg` → `baseMonth`·`baseMonthAvg`) · `lib/indices/volatility.ts`(`resolveVolatilityBaseMonth`·`monthLabel` export, `getVolatilityCardSummary`가 폴백 기준월 사용) · `components/indices/IndexDashboard.tsx`(월 라벨 표기) · `app/indices/kospi-volatility/page.tsx`(일별 목록 기준월 폴백 + 제목·각주·metadata 문구).
- **데이터·잡·키는 무변경** — `getVolatilityHistory`가 이미 전체 기록을 읽고 있어 KIS 호출·스냅샷 스키마·라우트 6종이 그대로다. **읽기 시 기준월 선택만 바뀌므로 다음 갱신 회차를 기다릴 필요 없이 배포 즉시 반영된다.**
- **검증**: tsc·eslint·`npm run build` 통과. 실 Redis 115건 기준 기대값 — 기준월 2026-07(22건), 평균 7.47%, 전월(6월 5.17%) 대비 **+2.30%p**.
- **상태**: **구현 완료(2026-08-01)**. 배포 후 확인 — ① 홈 카드에 `7월 평균 7.47% · 전월 +2.30%p`가 뜨는지(한 줄 폭이 아이폰에서 접히지 않는지 — §86.1 전례) ② 상세에 「7월 일별 기록」 22건이 뜨는지 ③ **8/3(월) 첫 회차 후 기준월이 8월로 넘어가고** 라벨이 「8월 평균」·「8월 일별 기록」으로 바뀌는지. — **①② 사용자 확인 완료(2026-08-01)**, ③은 8/3에 자동 전환.

### Phase 96 — 글로벌 지표 타일의 항목 이름 11px → 13px (2026-08-01, 구현 완료)

- **요청 근거**: 사용자 — §90 확인 과정에서 "국기 카드뿐 아니라 **글로벌 지표 상세화면의 모든 카드마다 각 항목 이름의 글씨 크기를 좀 더 크게**".
- **왜 작았나**: §89에서 표를 타일로 바꾸며 **값을 키우는 데 폭 예산을 전부 썼다**(값 `clamp(15px, 4.8vw, 24px)`). 라벨은 §88 표 시절의 `--text-micro`(11px)를 그대로 물려받아, 값만 커지고 "무엇의 값인지"는 오히려 더 안 읽히는 상태가 됐다.
- **구현**: `components/indices/GlobalTile.module.css`의 `.label`을 `--text-micro`(11px) → **`--text-caption`(13px)** 한 줄. **상단 6타일과 4구획 27종이 전부 같은 `GlobalTile`을 쓰므로 한 곳 수정으로 화면 전체가 따라온다**(§89에서 타일을 컴포넌트 하나로 모아둔 덕).
- **13px을 고른 근거**: 위계가 `단위 10px → 라벨 13px → 값 15~24px bold`로 유지된다. 12px은 체감 차이가 작고, 14px 이상이면 4열 환율 타일(@360px 내부 62px)에서 두 줄 라벨이 늘어 행 높이가 들쭉날쭉해진다.
- **레이아웃 영향 없음**: 라벨은 `word-break: keep-all`이고 행 높이는 그 행에서 가장 높은 타일이 정하므로, 「인도네시아」가 두 줄이 되어도 열은 어긋나지 않는다(§89 설계 그대로). 값 폰트·`clamp` 산식·국기 SVG(16×12)는 손대지 않았다.
- **검증**: tsc·eslint 통과. CSS 토큰 교체 한 줄이라 데이터·잡·스냅샷 무변경 — **배포 즉시 반영**.
- **상태**: **구현 완료(2026-08-01)**. 배포 후 확인 — ① 4열 환율 타일의 라벨 두 줄이 늘어 타일 높이가 들쭉날쭉해 보이지 않는지 ② 국기(16×12 고정)가 13px 라벨 옆에서 작아 보이지 않는지(거슬리면 `.flag`의 `vertical-align`·크기 미세조정).

### Phase 96.1 — 커진 것은 항목명이 아니라 구획 제목이어야 했다 (2026-08-01, 구현 완료)

- **요청 근거**: 사용자 — "**항목명이 아니라 컬럼명**을 크게 해달라는 뜻이었다. 크게 하되 **굵게는 하지 마라**. 항목명은 원래대로 복원". §96이 요청을 잘못 짚었다.
- **"컬럼명"이 무엇인지 먼저 확정했다** — 타일 그리드에는 표 헤더가 없어 후보가 갈렸다(구획 카드 제목 / 타일 안 단위 줄 / 지수 상세의 표 헤더). 화면 스케치를 보여 주고 확인 → **구획 카드 제목**(「주요 지표」·「세계 증시」·「전세계 환율」·「귀금속 · 비철금속」·「농산물」).
- **⚠️ `--text-body`는 정의된 적이 없는 토큰이었다** — 이 작업 중 발견. `var(--text-body)`를 쓰던 **3곳**(글로벌 지표 카드 제목 `page.module.css` · 구획 제목 `GlobalTileSection` · `FeedTabsClient`)이 **폴백 없이 상속값 16px으로 렌더**되고 있었다. 값이 우연히 의도와 같아 눈에 띄지 않았을 뿐이라, `tokens.css`에 **실효값 그대로 `--text-body: 16px`을 명시**했다(세 곳 모두 동작 불변).
- **구현**:
  - `styles/tokens.css` — `--text-section-title: 18px` 신설(페이지 제목 20px과 본문 16px 사이가 비어 구획 제목이 본문 크기로 눌려 있었다) + 위의 `--text-body: 16px` 정의.
  - `components/indices/GlobalTileSection.module.css` `.title` — 16px semibold → **18px regular**.
  - `app/indices/market/page.module.css` `.cardTitle`(「주요 지표」) — 같은 값으로 맞춤. 아래 구획들과 같은 위계라 어긋나면 첫 카드만 다른 종류로 보인다.
  - `components/indices/GlobalTile.module.css` `.label` — `--text-caption`(13px) → **`--text-micro`(11px) 원복**(§96 되돌림).
- **크기는 올리고 굵기는 낮춘 이유**: 요청 그대로이기도 하고, **커진 만큼 굵기까지 올리면 제목이 타일 값보다 먼저 읽혀 §89에서 값을 키운 취지가 뒤집힌다.** 페이지 제목(20px)도 §36에서 같은 이유로 regular라 방향이 일관된다.
- **검증**: tsc·eslint·`npm run build` 통과. CSS 토큰만 바뀌어 데이터·잡·스냅샷 무변경 — **배포 즉시 반영**.
- **상태**: **구현 완료(2026-08-01)**. 배포 후 확인 — ① 구획 제목 5개가 커지고 굵기는 오히려 옅어졌는지 ② 제목이 커져도 **타일 값이 여전히 먼저 읽히는지** ③ 항목명이 §96 이전 크기로 돌아왔는지.

### Phase 96.2 — 용어 정정: 키울 것은 타일 라벨, 줄일 것은 구획 제목 + 「N종」 제거 (2026-08-01, 구현 완료)

- **요청 근거**: 사용자 — 배포 화면을 보고 "요청대로 안 된 것 같다". ① **항목명**(=구획 제목)은 "홈화면의 다른 항목의 상세화면처럼 **굵기는 빼고 보통으로**, 크기도 줄여라" ② **컬럼명**(=**코스피·미국·두바이유** 등 타일 라벨)은 "굵기는 하지 말고 **좀 더 크게**" ③ **「8종」 같은 문구 삭제**.
- **§96 → §96.1 → §96.2를 오간 것은 용어 매핑이 어긋나서다.** §96에서 타일 라벨을 키웠고(맞았다), §96.1에서 "항목명이 아니라 컬럼명"이라는 정정에 따라 라벨을 되돌리고 구획 제목을 키웠는데(뒤집혔다), 이번에 **구체적 예시(코스피·미국·두바이유)**가 나오면서 확정됐다 — 사용자의 「컬럼명」 = **타일 라벨**, 「항목명」 = **구획 제목**. 확인 질문에 스케치를 붙였는데도 어긋난 이유는 선택지 라벨을 내 용어로 적었기 때문이다. **다음부터 이런 확인은 화면의 실제 문자열을 예시로 제시한다.**
- **구현**:
  - `components/indices/GlobalTile.module.css` `.label` — 11px medium·secondary → **13px regular·primary**. 굵기를 낮추면서 크기를 올렸고(요청 그대로), **색을 primary로 올려** 구획 제목과 크기가 같아진 자리를 색으로 갈랐다.
  - `components/indices/GlobalTileSection.module.css` `.title` — 18px regular·primary → **13px regular·secondary**. 앱 전역 섹션 제목 관례(`--text-caption`+secondary — 지수 상세·종목 상세·배당·보유·달러 인덱스 등 6곳 이상)를 따르되 **굵기만 semibold → regular**로 낮췄다.
  - `app/indices/market/page.module.css` `.cardTitle`(「주요 지표」) — 같은 값으로 맞춤.
  - `components/indices/GlobalTileSection.tsx` — **`{rows.length}종` 배지 제거**(+`.count` 클래스 삭제). 타일이 눈앞에 다 보이는 배치라 세어 줄 이유가 없다.
  - `styles/tokens.css` — **`--text-section-title` 삭제**. §96.1에서 신설했으나 이번에 두 사용처가 모두 `--text-caption`으로 옮겨 가 사용처가 0이 됐다(미사용 토큰을 남기지 않는다). `--text-body`는 피드 탭이 계속 써서 유지.
- **최종 위계**: 구획 제목 13px regular secondary → 타일 라벨 13px regular **primary** → 단위 10px tertiary → **값 clamp 15~24px bold primary**. 제목과 라벨이 같은 크기지만 **색으로 앞뒤가 갈리고**, 타일 안 위계는 여전히 값이 쥔다(§89의 취지 유지).
- **검증**: tsc·eslint·`npm run build` 통과. CSS·마크업만 바뀌어 데이터·잡·스냅샷 무변경 — **배포 즉시 반영**.
- **상태**: **구현 완료(2026-08-01)**. 배포 후 확인 — ① 코스피·미국·두바이유 같은 **항목 이름이 커지고 굵기는 빠졌는지** ② 구획 제목(「세계 증시」 등)이 **작고 조용해졌는지** ③ 「8종」이 사라졌는지 ④ 값이 여전히 가장 먼저 읽히는지.

### Phase 97 — 실적 보도 발췌를 마지막 온전한 문장까지로 다듬기 (2026-08-01, 구현 완료)

- **요청 근거**: 사용자 — §84 확인 중 "실적 관련 보도가 **요약 내용이 아니라 기사 본문을 중간에서 자른 정도**다. 요약 단어로 바꾸고 싶은데 기능적으로 안 되어 보인다". 선택지 3안을 제시하고 **1안(문장 경계에서 자르기) 채택**.
- **"요약이 안 되는" 진짜 이유는 재료다** — 발췌는 네이버 뉴스 API의 `description`인데 이것은 요약문이 아니라 **검색어 주변을 잘라낸 스니펫**이라 문장 중간에서 시작·종료한다. §84.2에서 **본문 스크랩은 하지 않기로** 저작권 선을 그었으므로(원문 링크 + 네이버가 주는 발췌까지), 요약을 생성할 재료 자체가 이 2~3줄뿐이다.
  - **그래서 LLM 요약(2안)을 권하지 않았다** — 2~3줄을 1줄로 줄이는 셈이라 정보가 오히려 준다. 진짜 요약을 하려면 "본문을 수집할 것인가"라는 별개 결정이 선행돼야 하고 그때 저작권 선을 다시 그어야 한다. **제목만 표시(3안)**는 §84.2에서 "그 한 문단이 본체"라 판단해 넣은 것을 되돌리는 셈이라 버렸다.
- **채택안 — 끝의 불완전한 조각만 버린다**(`tidyNewsSummary`). 요약은 아니고 정리다. 화면 각주("발췌·링크만 표시합니다")는 그대로 유효하다.
  - **앞쪽은 건드리지 않는다** — 한국어는 문장이 중간부터 시작했는지 판정할 단서가 없다(영어의 대문자 같은 것이 없다). 잘못 자르면 멀쩡한 첫 문장을 날린다. 선행 말줄임표만 지운다.
  - **말줄임표를 먼저 걷어낸다** — 실데이터는 `…`가 아니라 **마침표 3개**이고, 온전한 문장 뒤에 붙으면 `썼다....`처럼 4개가 연달아 나온다. 이걸 먼저 지우지 않으면 마지막 점이 문장 종결로 잡혀 아무것도 안 잘린다(첫 구현에서 실제로 그랬고 실데이터 확인에서 잡았다).
  - **소수점은 문장 끝이 아니다** — 종결 판정을 `[.!?](?=\s|$)`로 둬서 뒤가 숫자인 `1.5조`·`60.5조`·`7.5%`가 자연히 빠진다.
  - **역효과 가드 2종** — 다듬은 결과가 `MIN_TIDIED_CHARS`(40자) 미만이거나 **원문의 절반 미만**으로 줄면 원문을 그대로 둔다. 첫 문장이 짧은 기사에서 정리가 오히려 정보를 깎는 경우다.
- **저장이 아니라 읽기 시점에 적용한다**(`visibleEarningsNews`) — ① 이미 저장된 스냅샷에도 **즉시** 적용돼 다음 발표 시즌을 기다리지 않아도 된다 ② 스냅샷에는 원문이 남아 방침을 되돌리거나 규칙을 바꿔도 재수집이 필요 없다. 선별 점수(`scoreEarningsArticle`)는 수집 시점에 **원문 발췌**로 매기므로 다듬기가 어떤 기사를 고르는지에는 영향을 주지 않는다.
- **구현**: `lib/feeds/earningsNews.ts` — `tidyNewsSummary` 신설(+`SENTENCE_END`·`MIN_TIDIED_CHARS`), `visibleEarningsNews`가 반환 직전 `summary`에 적용. 화면·잡·스토어·타입 무변경.
- **검증**: tsc·eslint·`npm run build` 통과. **실 Redis 스냅샷 5종목 25건으로 실측** — 대부분 12~50자가 줄며 문장 끝으로 정리됐다(예: `…분기배당도 결정했다. 보통주와 우선주...` → `…분기배당도 결정했다.` 118→106자, `…역대 최대치를 달성했다. 특히 DS(반도체)부문의 경우 같은시기 89조원대...` → `…달성했다.` 128→97자). 종결부호가 하나도 없는 스니펫 1건은 말줄임표만 제거되고 본문은 보존됐고, 가장 많이 줄어든 건도 124→62자로 절반 가드 안이다.
- **상태**: **구현 완료(2026-08-01)**. 배포 후 확인 — 실적 탭 「실적 관련 보도」 발췌가 문장 끝에서 끝나는지(현재 스냅샷 11종목에 즉시 적용). 남는 잡음(`ⓒ스카이데일리` 같은 매체 표기, 부제·리드가 섞인 앞머리)은 **스니펫 자체의 성질**이라 이 작업 범위가 아니다.

### Phase 98 — 실적 탭 각주 2곳도 「설명」 토글로 (2026-08-01, 구현 완료)

- **요청 근거**: 사용자 — §84 확인을 마치며 "실적 탭에도 각주 설명이 있는데 **다른 항목처럼 버튼으로** 바꿔달라". §91이 지표 상세 10지점을 접힘 토글로 바꿀 때 **실적 탭은 범위 밖이었다**(그 Phase는 지표 화면만 훑었다).
- **대상 2곳** — 둘 다 `EarningsFocusPanel`의 `.caption` 문단이고 **설명형**이라 §91의 접기 기준에 그대로 맞는다:
  - **재무 기준 각주**(분기 표 아래) — "{연결/별도}재무제표 기준 · 확정 실적은 정기보고서(DART), 「잠정」 표시 분기는 영업(잠정)실적 공정공시에서 왔습니다" + 최신 잠정치 접수일·단위·감사 전 수치 고지.
  - **뉴스 출처 각주**(실적 관련 보도 아래) — "네이버 뉴스 검색 결과이며 발췌·링크만 표시합니다…".
- **구현**: `components/feeds/EarningsFocusPanel.tsx` — 두 `<p className={styles.caption}>`을 **`NoteDisclosure`로 교체**(라벨은 기본 "설명" 그대로 — 자리마다 다르게 부르면 같은 조작이 다른 것처럼 보인다는 §91 방침). `EarningsFocusPanel.module.css` — 사용처가 0이 된 **`.caption` 삭제**(폰트·색은 이제 `NoteDisclosure`가 쥔다).
- **여백은 넘기지 않았다** — 두 각주 모두 부모가 `flex column` + `gap`(`.panel` 12px · `.newsBlock`)이라 간격을 부모가 준다. §91에서 `className`으로 여백을 받게 만든 건 카드 안 각주와 푸터 첫 줄의 간격이 달라서였고, 여기는 해당이 없다.
- **`NoteDisclosure`는 서버 컴포넌트**라 이 교체로 **클라이언트 번들이 늘지 않는다**(네이티브 `<details>`). 실적 탭 상단 블록도 서버 컴포넌트 그대로다.
- **검증**: tsc·eslint·`npm run build` 통과. 마크업만 바뀌어 데이터·잡·스냅샷·KIS 호출 무변경 — **배포 즉시 반영**.
- **상태**: **구현 완료(2026-08-01)**. 배포 후 확인 — ① 분기 표 아래와 「실적 관련 보도」 아래에 「ⓘ 설명 ▾」이 **접힌 채** 뜨고 눌러 펼쳐지는지 ② 실적 게시판 아코디언(`<details>`) 안에서도 중첩이 꼬이지 않는지 ③ 다크 모드 대비.
