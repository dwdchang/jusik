...
- **표기 결정 근거**:
  - **기준일이 어긋날 수 있다** — dxy는 통화쌍 6종의 기준일 **교집합**에서만 계산하므로(`dxy.ts`) 원/달러보다 기준일이 하루 밀릴 수 있다. 요약 카드는 `basDt`를 보여주지 않아 사용자가 두 값을 같은 날짜로 읽는다 → **두 `basDt`가 다를 때만** 뒤에 `· 07/29 기준`을 붙인다(같을 때는 군더더기라 붙이지 않는다).
  ...
  - **지표명은 「달러인덱스」(축약 아님).** 홈 `MarketCard`는 4행 폭 압박 때문에 축약(美 금리·WTI·GOLD·BTC)을 쓰지만(§34), 여기는 caption-sm 한 줄이라 폭 여유가 있고 `DXY`보다 한글 표기가 바로 읽힌다.
...
- **상태**: **구현 완료(2026-07-30)**. 배포 후 확인 필요 — 홈 원/달러 카드에 보조줄이 뜨는지, 장 마감 후 회차에서 기준일 꼬리표가 사라지는지.
