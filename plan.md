...
- **상태**: **구현 완료(2026-07-31)**. 배포 후 확인 필요 — ① 첫 09:00/15:40/18:15 회차에 표가 채워지는지 ② **360px에서 4열이 한 줄에 들어가는지**(계산상 최장 라벨 「필라델피아 반도체」 99px + 값 73 + 등락률 45 + 기준일 38 + 갭 24 = 279px로 내부 폭 304px에 겨우 들어간다 — 넘치면 라벨 축약) ③ 첫 섹션만 펼쳐져 있는지 ④ 환율 「달러/유로」 방향 표기가 오독되지 않는지.
