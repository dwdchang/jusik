...
### Phase 42 — 일별 기록 지수↔수급 토글 (2026-07-20, **보류** — 키움 REST API 키 발급 대기)

- **요청 근거**: 사용자 요청 — 일별 기록에 토글을 넣어 지수/수급 전환, 수급 항목은 개인·외국인·기관(연기금·금투 등)·거래량·거래대금.
- **조사 결과 (2026-07-20) — 공개 API 3종 모두에서 시장 전체 수급 확보 불가**:
  1. **KIS**: 시장별 투자자매매동향(일별) `FHPTJ04040000`은 `rt_cd=0`으로 정상 응답하고 **스키마도 요청과 정확히 일치**한다 — `prsn_`(개인)·`frgn_`/`frgn_reg_`/`frgn_nreg_`(외국인·등록/미등록)·`orgn_`(기관계)·`scrt_`(금투)·`ivtr_`(투신)·`fund_`(연기금)·`pe_fund_`(사모)·`bank_`·`insu_`·`mrbn_`(종금)·`etc_corp_`(기타법인) 각 수량·금액. 그러나 **전 필드 값이 0**. `FID_INPUT_ISCD`(0001/1001)×`ISCD_1`(KSP/KSQ)×`ISCD_2` **48개 조합**, 날짜도 6·7월 여러 구간으로 전수 시도했으나 모두 0. 시세용 `FHPTJ04030000`도 동일하게 0. 종목 단위 `FHKST01010900`은 **정상 동작**(삼성전자 30거래일, 개인/외국인/기관 3분류 매수·매도·순매수 × 수량·금액)이나 지수코드(`U`/`0001`)로는 `ERROR INVALID FID_COND_MRKT_DIV_CODE`.
  2. **KRX OpenAPI**(`data-dbg.krx.co.kr`): 키 없이도 401(존재)/404(부재)가 구분돼 카탈로그 확인 가능 — 투자자 관련 경로 후보 14종 **전부 404**. 공식 목록도 지수 5·주식 7·증권상품 3·채권 3·파생 6·일반상품 3·ESG 3 = **33종 전부 시세 계열, 투자자별 없음**. 유료 등급도 없음(KRX 유료는 Data Marketplace 데이터셋 구매이며 API 아님).
  3. **data.go.kr 금융위**: 증권 계열 전체(주식·지수·증권상품·일반상품 시세, KRX상장종목정보, 주식발행정보, 공시정보)가 시세/기본정보뿐. 카탈로그를 API 타입으로 검색해도 투자자별 거래실적 API **없음**. 전부 무료라 유료 우회로도 없음.
  4. **KRX 웹 스크래핑(pykrx 방식)**: `getJsonData.cmd`를 pykrx와 동일한 헤더·파라미터·세션 쿠키로 재시도했으나 **대조군인 지수 시세 조회까지 전부 `LOGOUT` 400**. 현재 차단 상태이며 비공식 경로라 프로덕션 비권장.
- **대안 후보 (외부 소스)**:
  - **키움 REST API — 채택**. 무료, REST 기반이라 macOS/Linux/Vercel 동작(구 OpenAPI+의 Windows OCX 제약 없음). `ka10051`(업종별투자자순매수 — 코스피 종합 업종코드로 사실상 시장 전체)·`ka10058`(투자자별일별매매종목)·`ka10063`(장중)·`ka10066`(장마감후). 인증 구조가 KIS와 거의 같아 기존 잡·토큰 캐시 패턴 재사용 가능. **조건: 키움 계좌 개설 + 앱키/시크릿 발급**.
  - LS증권 OPEN API — REST+WebSocket, 무료, 일 5,000회, "[주식] 투자자" 카테고리 존재. 계좌 필요.
  - 코스콤 오픈API — `/v3/market/investors/{kospi|kosdaq}/investors`로 **시장 전체** 투자자 매수/매도 수량·대금 제공(KRX 직결 원본). 단 **과거 일별 조회 불가**(전일·당일 15:30 이후만)라 히스토리를 직접 축적해야 하고, 가공·축적에 **정보이용 라이선스 계약 필요**(유료).
- **미검증 리스크 (구현 착수 전 반드시 실측할 것)**: `ka10051`의 실제 응답 필드에 연기금·투신 세부가 들어오는지, 코스피 종합 업종코드로 시장 전체가 집계되는지는 **계좌가 있어야 호출 가능해 확인하지 못했다.** KIS처럼 "스키마는 오는데 값이 0"일 가능성을 배제할 수 없으므로, **키 발급 직후 값 실측을 선행**하고 그 결과로 이 Phase의 구현 계획을 확정한다.
- **병행 확인 (저비용)**: KIS `FHPTJ04040000` 0값은 시세 이용 신청 누락 등 계정 권한 문제일 가능성이 남아 있다. **KIS 고객센터 문의로 풀리면 신규 소스·잡·키가 전부 불필요**해지므로 키움 착수 전 확인할 가치가 있다.
- **차선 (수급을 다른 성격으로 대체할 경우)**: `foreign-institution-total`(FHPTJ04400000)은 **정상 동작 확인** — 종목별 외국인·기관·투신·연기금·보험 순매수 상위 30건. "시장 일별 수급"은 아니지만 KIS만으로 구현 가능. 또는 개인/외국인/기관 30거래일 수급을 **종목 상세**(`/holdings/[symbolCode]`·`/watchlist/[symbolCode]`)에 `FHKST01010900`으로 추가.
- **상태**: **보류** — 사용자가 키움 REST API 키를 직접 발급한 뒤 재개 예정(2026-07-20 결정). Phase 41(거래량 차트)은 이 Phase와 무관하게 선행 가능.
